...
public class PdfViewerPreferencesImp implements PdfViewerPreferences {
	private static final PdfName[] VIEWER_PREFERENCES = {
	...
	public void setViewerPreferences(final int preferences) {
		...
		if ((preferences & viewerPreferencesMask) != 0) {
			this.pageLayoutAndMode = ~viewerPreferencesMask & this.pageLayoutAndMode;
	...
	public void addToCatalog(final PdfDictionary catalog) {
		...
		if ((this.pageLayoutAndMode & PdfWriter.PageLayoutSinglePage) != 0) {
			catalog.put(PdfName.PAGELAYOUT, PdfName.SINGLEPAGE);
		} else if ((this.pageLayoutAndMode & PdfWriter.PageLayoutOneColumn) != 0) {
			catalog.put(PdfName.PAGELAYOUT, PdfName.ONECOLUMN);
		} else if ((this.pageLayoutAndMode & PdfWriter.PageLayoutTwoColumnLeft) != 0) {
			catalog.put(PdfName.PAGELAYOUT, PdfName.TWOCOLUMNLEFT);
		} else if ((this.pageLayoutAndMode & PdfWriter.PageLayoutTwoColumnRight) != 0) {
			catalog.put(PdfName.PAGELAYOUT, PdfName.TWOCOLUMNRIGHT);
		} else if ((this.pageLayoutAndMode & PdfWriter.PageLayoutTwoPageLeft) != 0) {
			catalog.put(PdfName.PAGELAYOUT, PdfName.TWOPAGELEFT);
		} else if ((this.pageLayoutAndMode & PdfWriter.PageLayoutTwoPageRight) != 0) {
			catalog.put(PdfName.PAGELAYOUT, PdfName.TWOPAGERIGHT);
		}

		// Page Mode
		catalog.remove(PdfName.PAGEMODE);
		if ((this.pageLayoutAndMode & PdfWriter.PageModeUseNone) != 0) {
	...
	public static PdfViewerPreferencesImp getViewerPreferences(final PdfDictionary catalog) {
		...
		if (obj != null && obj.isDictionary()) {
			...
			for (final PdfName element : VIEWER_PREFERENCES) {
				obj = PdfReader.getPdfObjectRelease(vp.get(element));
				preferences.addViewerPreference(element, obj);
			}
		}
		return preferences;
	}
