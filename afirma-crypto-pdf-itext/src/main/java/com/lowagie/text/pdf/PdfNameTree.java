...
class PdfNameTree {
    ...
    public static PdfDictionary writeTree(final HashMap items, final PdfWriter writer) throws IOException {
        ...
        int skip = leafSize;
        final PdfIndirectReference kids[] = new PdfIndirectReference[(names.length + leafSize - 1) / leafSize];
        for (int k = 0; k < kids.length; ++k) {
            int offset = k * leafSize;
            final int end = Math.min(offset + leafSize, names.length);
            final PdfDictionary dic = new PdfDictionary();
        ...
        while (true) {
            ...
            skip *= leafSize;
            final int tt = (names.length + skip - 1 )/ skip;
            for (int k = 0; k < tt; ++k) {
                int offset = k * leafSize;
