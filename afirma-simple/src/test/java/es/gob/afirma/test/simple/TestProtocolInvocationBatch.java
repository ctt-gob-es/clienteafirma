...
public class TestProtocolInvocationBatch {
	...
	private static final String LINE = "afirma://batch?" //$NON-NLS-1$
		...
		+ "stservlet=http://comparece.dipucr.es:8080/afirma-signature-storage/StorageService&" //$NON-NLS-1$
		+ "rtservlet=http://comparece.dipucr.es:8080/afirma-signature-storage/RetrieveService&" //$NON-NLS-1$
		+ "dat=" + Base64.encode(SAMPLE_BATCH_XML.getBytes(), true) + "&" //$NON-NLS-1$ //$NON-NLS-2$
		+ "batchpostsignerurl=" + "http://demo.com:8080/afirma-server-triphase-signer/BatchPostsigner" + "&" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		+ "batchpresignerurl=" + "http://demo.com:8080/afirma-server-triphase-signer/BatchPresigner" //$NON-NLS-1$ //$NON-NLS-2$
	;
