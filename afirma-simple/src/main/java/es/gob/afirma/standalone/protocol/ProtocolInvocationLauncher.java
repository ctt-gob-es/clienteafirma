...
public final class ProtocolInvocationLauncher {
	...
	/**
	 * Par&aacute;metro de entrada con la versi&oacute;n del protocolo que se va a
	 * utilizar.
	 */
	...
	public static String launch(final String urlString, final int protocolVersion, final boolean bySocket) {
		...
		// Por defecto, usaremos la version de protocolo proporcionada para la
		// operacion,
		// anque se extraera de la URL de llamada en caso de se una peticion de apertura
		// de
			...
			try {
				...
				if (params.getFileId() != null) {
					...
					try {
						...
						ProtocolInvocationLauncherErrorManager
								.showError(ProtocolInvocationLauncherErrorManager.ERROR_RECOVERING_DATA);
						return ProtocolInvocationLauncherErrorManager
						...
						ProtocolInvocationLauncherErrorManager
								.showError(ProtocolInvocationLauncherErrorManager.ERROR_DECRYPTING_DATA);
						return ProtocolInvocationLauncherErrorManager
				...
				try {
					...
					if (e.getErrorCode() == ProtocolInvocationLauncherBatch.getResultCancel()) {
						...
						sendErrorToServer(ProtocolInvocationLauncherErrorManager.getErrorMessage(e.getErrorCode()),
								params.getStorageServletUrl().toString(), params.getId());
					}
				...
				ProtocolInvocationLauncherErrorManager
						.showErrorDetail(ProtocolInvocationLauncherErrorManager.ERROR_PARAMS, e.getMessage());
				return ProtocolInvocationLauncherErrorManager
		...
		// Se solicita una operacion de guardado
			...
			try {
				...
				ProtocolInvocationLauncherErrorManager
						.showError(ProtocolInvocationLauncherErrorManager.ERROR_OBSOLETE_APP);
				return ProtocolInvocationLauncherErrorManager
				...
				ProtocolInvocationLauncherErrorManager
						.showError(ProtocolInvocationLauncherErrorManager.ERROR_LOCAL_ACCESS_BLOCKED);
				return ProtocolInvocationLauncherErrorManager
			...
			try {
				...
				// traves del servidor intermedio
					...
					if (e.getErrorCode() == ProtocolInvocationLauncherSign.getResultCancel()) {
						...
						sendErrorToServer(e.getErrorCode() + ": " + e.getMessage(), //$NON-NLS-1$
								params.getStorageServletUrl().toString(), params.getId());
					} else {
						sendErrorToServer(ProtocolInvocationLauncherErrorManager.getErrorMessage(e.getErrorCode()),
								params.getStorageServletUrl().toString(), params.getId());
				...
				LOGGER.log(Level.SEVERE, "Error. El usuario ha cancelado la seleccion del area de firma visible: " + e, //$NON-NLS-1$
						e);
				ProtocolInvocationLauncherErrorManager.showErrorDetail(
						ProtocolInvocationLauncherErrorManager.ERROR_VISIBLE_SIGNATURE, e.getMessage());
				return ProtocolInvocationLauncherErrorManager
						.getErrorMessage(ProtocolInvocationLauncherErrorManager.ERROR_VISIBLE_SIGNATURE);
			} catch (final Exception e) {
				LOGGER.log(Level.SEVERE, "Error en los parametros de firma: " + e, e); //$NON-NLS-1$
				ProtocolInvocationLauncherErrorManager.showError(ProtocolInvocationLauncherErrorManager.ERROR_PARAMS);
				return ProtocolInvocationLauncherErrorManager
		...
		// Se solicita una operacion de recuperacion de logs
			...
			try {
				final UrlParametersToGetCurrentLog params = ProtocolInvocationUriParser
						.getParametersToGetCurrentLog(urlString);
	...
	/**
	 * Env&iacute;a un mensaje de error al servidor intermedio e interrumpe la
	 * espera declarada en este servidor.
	 * 
	 ...
	 * @return Valor del par&aacute;metro de versi&oacute;n ('v') o el valor '1' si
	 *         no est&aacute; definido.
	 */
