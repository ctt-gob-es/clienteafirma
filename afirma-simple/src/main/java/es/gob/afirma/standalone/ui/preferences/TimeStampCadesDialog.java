...
final class TimeStampCadesDialog extends JDialog {

	private static final long serialVersionUID = 7679748306298466053L;
	...
	static void startTimeStampCadesDialog(final Frame parent) {
		final TimeStampCadesDialog tsd = new TimeStampCadesDialog(parent);
	...
	private void loadConfiguration() {
		...
		final String url = PreferencesManager.get(
			PreferencesManager.PREFERENCE_CADES_TIMESTAMP_TSA_URL,
			"" //$NON-NLS-1$
		);
		...
		final String policy = PreferencesManager.get(
			PreferencesManager.PREFERENCE_CADES_TIMESTAMP_STAMP_POLICY,
			"" //$NON-NLS-1$
		);
		...
		final String usr = PreferencesManager.get(
			PreferencesManager.PREFERENCE_CADES_TIMESTAMP_TSA_USR,
			"" //$NON-NLS-1$
		);
		...
		final String pwd = PreferencesManager.get(
			PreferencesManager.PREFERENCE_CADES_TIMESTAMP_TSA_PWD,
			"" //$NON-NLS-1$
		);
		...
		final String oid = PreferencesManager.get(
			PreferencesManager.PREFERENCE_CADES_TIMESTAMP_EXTENSION_OID,
			"" //$NON-NLS-1$
		);
		...
		final String extValue = PreferencesManager.get(
			PreferencesManager.PREFERENCE_CADES_TIMESTAMP_EXTENSION_VALUE,
			"" //$NON-NLS-1$
		);
	...
	void SaveConfiguration() {
		...
		PreferencesManager.putBoolean(
			PreferencesManager.PREFERENCE_CADES_TIMESTAMP_CERT_REQUIRED,
	...
	boolean isValidUrl() {
		if (getTsaUrlText() != null && !getTsaUrlText().trim().isEmpty()) {
			try {
				...
				Logger.getLogger("es.gob.afirma").info("URL errónea: " + e); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
		return false;
	}

	private TimeStampCadesDialog(final Frame parentFrame) {
		super(parentFrame);
