...
final class RestoreConfigWindows implements RestoreConfig {
	...
	public void restore(JTextArea taskOutput) throws IOException, GeneralSecurityException {
		...
		if (isAdmin()) {
			...
			try {
				...
				// Además, certutil no informa con algún error si hay algun problema.
	 ...
	 */
	private void configureChrome(JTextArea taskOutput) {
	 ...
	 */
	private boolean installRootCAWindowsKeystore(JTextArea taskOutput, Certificate cer) {
	 ...
	 */
	private void restoreProtocolRegistry(JTextArea taskOutput) {
	 ...
	 */
	private Certificate restoreCertificateWindows(JTextArea taskOutput) throws IOException, GeneralSecurityException {
