...
public final class SignPdfDialog extends JDialog implements PdfLoaderListener, SignPdfUiPanelListener {
	...
	private SignPdfDialog(final Frame parentFrame, final SignPdfDialogListener spdl, final boolean signatureVisible,
			final boolean stampVisible, final Boolean customAppearance) {
		super(parentFrame);
	...
	public static void getVisibleSignatureExtraParams(final boolean isSign, final byte[] pdf, final Frame parentFrame,
			final boolean signatureVisible, final boolean stampVisible, final Boolean customAppearance,
	...
	public static JDialog getVisibleSignatureDialog(final boolean isSign, final byte[] pdf, final Frame parentFrame,
		...
		final JDialog dialog = new SignPdfDialog(parentFrame, spdl, signatureVisible, stampVisible, customAppearance);
		dialog.setPreferredSize(getPreferredDimensionToSignatureDialog());
		final Point cp = GraphicsEnvironment.getLocalGraphicsEnvironment().getCenterPoint();
		dialog.setLocation(cp.x - (int) dialog.getPreferredSize().getWidth() / 2,
				cp.y - (int) dialog.getPreferredSize().getHeight() / 2);
	...
	public void nextPanel(final Properties p, final BufferedImage im) {
		if (checkSignatureDimensions(p)) {
			if (customAppearance != null && !customAppearance) {
				positionSelected(p);
			} else {
	...
	private boolean checkSignatureDimensions(Properties extraParams) {
		boolean res = true;
		Double lowerLeftX = Double
				.valueOf(extraParams.getProperty(PdfExtraParams.SIGNATURE_POSITION_ON_PAGE_LOWER_LEFTX));
		Double lowerLeftY = Double
				.valueOf(extraParams.getProperty(PdfExtraParams.SIGNATURE_POSITION_ON_PAGE_LOWER_LEFTY));
		Double upperRightX = Double
				.valueOf(extraParams.getProperty(PdfExtraParams.SIGNATURE_POSITION_ON_PAGE_UPPER_RIGHTX));
		Double upperRightY = Double
				.valueOf(extraParams.getProperty(PdfExtraParams.SIGNATURE_POSITION_ON_PAGE_UPPER_RIGHTY));
		Double dimensionX = Math.abs(upperRightX - lowerLeftX);
		Double dimensionY = Math.abs(upperRightY - lowerLeftY);
		boolean limitSizeCondition = dimensionX < LOWER_LIMIT_X || dimensionY < LOWER_LIMIT_Y;
		boolean limitAreaCondition = (dimensionX * dimensionY) < AREA_LIMIT;
		if (limitSizeCondition || limitAreaCondition) {
			String msg = "El área seleccionada para mostrar la firma tienen un tamaño demasiado pequeño."; //$NON-NLS-1$
			String title = "Tamaño inválido"; //$NON-NLS-1$
			AOUIFactory.showErrorMessage(this.activePanel, msg, title, // $NON-NLS-1$
