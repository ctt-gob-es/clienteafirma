...
import es.gob.afirma.signature.AOCertVerifier;
    ...
    CertificateInfo(final X509Certificate cert, final String description, final AOCertVerifier ocsp, final Icon i, final String iTooltip) {
    	...
    	if (description == null || "".equals(description)) {  //$NON-NLS-1$
        	if (cert == null) {
        		...
        		this.descriptionText = "<html>" + ((Platform.OS.MACOSX.equals(Platform.getOS())) ? Constants.HTML_SALTO_LINEA : "") + Messages.getString("CertificateInfo.1") + ": <a href=\"#\">" + AOUtil.getCN(cert) + "</a>. " + Messages.getString("CertificateInfo.2") + ": <a href=\"#\">" + AOUtil.getCN(cert.getIssuerX500Principal().toString()) + "</a>" + "</html>"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$ //$NON-NLS-8$ //$NON-NLS-9$
        	}
        }
        else {
            this.descriptionText = "<html>" + ((Platform.OS.MACOSX.equals(Platform.getOS())) ? Constants.HTML_SALTO_LINEA : "") + "<a href=\"#\">" + description + "</a>" + "</html>"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
        }
