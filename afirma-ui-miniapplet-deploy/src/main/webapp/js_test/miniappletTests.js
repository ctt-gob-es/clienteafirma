...
//--------------------------------------------------------------------------------------------
 var MiniAppletTests = {
	...
	/** URL del servicio que recoge los resultados de las operaciones. */
	serviceTestUrl : "http://localhost:8080/afirma-test-server/RegisterService",
	
	/**
	 * Compone el mensaje con el resultado de la operacion y lo envia al servidor
	 * encargado de recoger los resultados.
	 */
	notifyResultTest : function (testId, result, msg) {
	
		var params = "test=" + testId +
					 "&result=" + result +
					 "&miniapplet_version=" + MiniApplet.VERSION +
					 "&browser=" + MiniAppletTests.getBrowser() +
					 "&browser_version=" + MiniAppletTests.getBrowserVersion() +
					 "&os=" + MiniAppletTests.getOs() +
					 "&os_version=" + MiniAppletTests.getOsVersion() +
					 "&java_version=" + MiniAppletTests.getJavaVersion() +
					 "&java_arch=" + MiniAppletTests.getJavaArch();

		if (msg != undefined && msg != null && msg.trim().lenght > 0) {
			params += "&msg=" + msg;
		}
		MiniAppletTests.sendMessage(MiniAppletTests.serviceTestUrl, params);
	},
	...
	getOsVersion : function () {
		return jscd.osVersion;
	},
	
	getJavaVersion : function () {
		return jscd.javaVersion;
	},
	
	getJavaArch : function () {
		return jscd.javaArch;
	},

	/**
	 * Envia un mensaje por POST a un servicio.
	 */
	sendMessage : function (serviceTestUrl, params) {
	
		var httpRequest = MiniAppletTests.getHttpRequest();
		if (!httpRequest) {
			this.throwException("java.lang.Exception", "Su navegador no permite preprocesar los datos que desea tratar");
		}

		httpRequest.open("POST", serviceTestUrl, false);
		...
		//	alert("Error de conexion con el servidor de resultados");
		//}
	},
 ...
 var TestEnvironment = {
	...
	updateJavaInfoFromApplet : function (echo) {
		...
		jscd.javaVersion = echo.substring(i + header.length, j).trim();
		
		i = echo.indexOf(':', j) + 1;
		
		jscd.javaArch = echo.substring(i).trim();
	},
