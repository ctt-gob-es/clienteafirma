...
final class XAdESSigner
{
	 ...
	 */
	static byte[] sign(final byte[] data, final String algorithm, final PrivateKeyEntry keyEntry, final Properties xParams) throws AOException
	{
		...
		if (mode.equals(AOSignConstants.SIGN_MODE_IMPLICIT))
		{
			try
			{
				...
				try
				{
					...
					if (styleType != null && styleHref != null)
					{
						...
						XAdESSigner.LOGGER.info("Se ha encontrado una hoja de estilo asociada al XML a firmar: tipo=" + styleType //$NON-NLS-1$
								+ ", referencia=" //$NON-NLS-1$
								+ styleHref);

						try
						{
							final Document tmpDoc = Utils.dereferenceStyleSheet(TransformerFactory.newInstance().getAssociatedStylesheet(new StreamSource(new ByteArrayInputStream(data)), null, null, null).getSystemId(), headLess);
							...
							if (!styleHref.startsWith(AOXAdESSigner.HTTP_PROTOCOL_PREFIX) && !styleHref.startsWith(AOXAdESSigner.HTTPS_PROTOCOL_PREFIX))
							{
						...
						{
							XAdESSigner.LOGGER.info("La hoja de estilo esta referenciada internamente, por lo que no se necesita dereferenciar"); //$NON-NLS-1$
						} catch (final ReferenceIsNotXMLException ex)
						{
							XAdESSigner.LOGGER.warning("La hoja de estilo referenciada no es XML o no se ha dereferenciado apropiadamente"); //$NON-NLS-1$
						} catch (final CannotDereferenceException ex)
						{
							XAdESSigner.LOGGER.warning("La hoja de estilo no ha podido dereferenciar, probablemente sea un enlace relativo local"); //$NON-NLS-1$
						} catch (final Exception ex)
						{
							XAdESSigner.LOGGER.severe("Error intentando dereferenciar la hoja de estilo: " + ex); //$NON-NLS-1$
						}
					}
				} catch (final Exception e)
				{
					XAdESSigner.LOGGER.info("No se ha encontrado ninguna hoja de estilo asociada al XML a firmar"); //$NON-NLS-1$
				}
				...
				if (format.equals(AOSignConstants.SIGN_FORMAT_XADES_DETACHED))
				{
					...
					if (styleElement != null)
					{
						...
						{
							XAdESSigner.LOGGER.warning("No ha sido posible crear el elemento DOM para incluir la hoja de estilo del XML como Internally Detached: " + e); //$NON-NLS-1$
							styleElement = null;
						}
					}
				} else
			...
			catch (final Exception e)
			{
				...
				XAdESSigner.LOGGER.info("El documento no es un XML valido. Se convertira a Base64: " + e); //$NON-NLS-1$
				...
				try
				{
					...
					if (AOUtil.isBase64(data) && (XMLConstants.BASE64_ENCODING.equals(encoding) || "base64".equalsIgnoreCase(encoding))) { //$NON-NLS-1$
						XAdESSigner.LOGGER.info("El documento se ha indicado como Base64, se insertara como tal en el XML"); //$NON-NLS-1$
					...
					} else
					{
						...
						{
							XAdESSigner.LOGGER.info("El documento se ha indicado como Base64, pero no es un Base64 valido. Se convertira a Base64 antes de insertarlo en el XML y se declarara la transformacion"); //$NON-NLS-1$
						} else
						{
							XAdESSigner.LOGGER.info("El documento se considera binario, se convertira a Base64 antes de insertarlo en el XML y se declarara la transformacion"); //$NON-NLS-1$
						}
		...
		if (!isBase64)
		{
			...
			if (!facturaeSign)
			{
				...
				{
					XAdESSigner.LOGGER.severe("No se puede encontrar el algoritmo de canonicalizacion, la referencia no se canonicalizara: " + e); //$NON-NLS-1$
				}
			}
		}
		...
		else if (wasEncodedToBase64 && !avoidBase64Transforms)
		{
			...
			{
				XAdESSigner.LOGGER.severe("No se puede encontrar el algoritmo transformacion Base64, esta no se declarara: " + e); //$NON-NLS-1$
			}
		}
		...
		if (format.equals(AOSignConstants.SIGN_FORMAT_XADES_ENVELOPING))
		{
			try
			{
				...
				referenceList.add(fac.newReference("#" + objectId, digestMethod, transformList, AOXAdESSigner.OBJURI, referenceId)); //$NON-NLS-1$

				// Vamos con la hoja de estilo
				if (styleElement != null)
				{
					final String objectStyleId = "StyleObject-" + UUID.randomUUID().toString(); //$NON-NLS-1$
					envelopingStyleObject = fac.newXMLObject(Collections.singletonList(new DOMStructure(styleElement)), objectStyleId, styleType, styleEncoding);
					referenceList.add(fac.newReference("#" + objectStyleId, //$NON-NLS-1$
							digestMethod, Collections.singletonList(fac.newTransform(canonicalizationAlgorithm, (TransformParameterSpec) null)), AOXAdESSigner.OBJURI, referenceStyleId));
			...
			if (styleHref != null && styleElement == null && (styleHref.startsWith(AOXAdESSigner.HTTP_PROTOCOL_PREFIX) || styleHref.startsWith(AOXAdESSigner.HTTPS_PROTOCOL_PREFIX)))
			{ // Comprobamos si la referencia al estilo es externa
				try
				{
					referenceList.add(fac.newReference(styleHref, digestMethod, Collections.singletonList(fac.newTransform(canonicalizationAlgorithm, (TransformParameterSpec) null)), null, referenceStyleId));
				} catch (final Exception e)
				{
					XAdESSigner.LOGGER.severe("No ha sido posible anadir la referencia a la hoja de estilo del XML, esta no se firmara: " + e); //$NON-NLS-1$
				}
			}
		...
		else if (format.equals(AOSignConstants.SIGN_FORMAT_XADES_DETACHED))
		{
			try
			{
				...
				if (styleElement != null)
				{
					...
					// crea la referencia a los datos firmados que se
					// encontraran en el mismo documento
					referenceList.add(fac.newReference(tmpStyleUri, digestMethod, Collections.singletonList(fac.newTransform(canonicalizationAlgorithm, (TransformParameterSpec) null)), null, referenceStyleId));
				}
		...
		else if (format.equals(AOSignConstants.SIGN_FORMAT_XADES_EXTERNALLY_DETACHED))
		{
			...
			else
			{
				...
				if (uri != null && uri.getScheme().equals("file")) { //$NON-NLS-1$
					try
					{
						ref = fac.newReference("", //$NON-NLS-1$
								digestMethod, null, null, referenceId, MessageDigest.getInstance(AOSignConstants.getDigestAlgorithmName(digestMethodAlgorithm)).digest(AOUtil.getDataFromInputStream(AOUtil.loadFile(uri))));
					} catch (final Exception e)
					{
			...
			if (styleHref != null && styleElement == null)
			{
				...
				{
					XAdESSigner.LOGGER.warning("Se necesita una referencia externa HTTP o HTTPS a la hoja de estilo para referenciarla en firmas XML Externally Detached"); //$NON-NLS-1$
				}
			}
		...
		else if (format.equals(AOSignConstants.SIGN_FORMAT_XADES_ENVELOPED))
		{
			try
			{
				...
				// Salvo que sea una factura electronica, se agrega una
				// transformacion XPATH
				// para eliminar el resto de firmas del documento
				if (!facturaeSign)
				{
					transformList.add(fac.newTransform(Transform.XPATH, new XPathFilterParameterSpec("not(ancestor-or-self::" + AOXAdESSigner.XML_SIGNATURE_PREFIX + ":Signature)", //$NON-NLS-1$ //$NON-NLS-2$
					Collections.singletonMap(AOXAdESSigner.XML_SIGNATURE_PREFIX, XMLSignature.XMLNS))));
				}
		...
		final DataObjectFormat objectFormat = new DataObjectFormatImpl(null, objectIdentifier, mimeType, encoding, "#" + referenceId //$NON-NLS-1$
		);
		...
		{
			XAdESSigner.LOGGER.severe("No se ha podido establecer el algoritmo de huella digital (" + algoUri //$NON-NLS-1$
		...
		if (format.equals(AOSignConstants.SIGN_FORMAT_XADES_ENVELOPED) && styleElement != null)
		{
			...
			{
				XAdESSigner.LOGGER.severe("No se ha podido anadir una referencia a la hoja de estilo, esta se incluira dentro de la firma, pero no estara firmada: " + e); //$NON-NLS-1$
			}
		}
		...
		if (format.equals(AOSignConstants.SIGN_FORMAT_XADES_ENVELOPING))
		{
			try
			{
				if (docSignature.getElementsByTagNameNS(XMLConstants.DSIGNNS, AOXAdESSigner.SIGNATURE_TAG).getLength() == 1)
				{
					final Document newdoc = dbf.newDocumentBuilder().newDocument();
					newdoc.appendChild(newdoc.adoptNode(docSignature.getElementsByTagNameNS(XMLConstants.DSIGNNS, AOXAdESSigner.SIGNATURE_TAG).item(0)));
					docSignature = newdoc;
				}
			} catch (final Exception e)
			{
				XAdESSigner.LOGGER.info("No se ha eliminado el nodo padre '<AFIRMA>': " + e); //$NON-NLS-1$
			}
		}
