...
final class XAdESCounterSigner {
    ...
    static byte[] countersign(final byte[] sign,
        ...
        try {
            if (targetType == CounterSignTarget.TREE) {
                countersignTree(root, keyEntry, extraParams, algorithm, doc);
            }
            else if (targetType == CounterSignTarget.LEAFS) {
                countersignLeafs(root, keyEntry, extraParams, algorithm, doc);
            }
            else if (targetType == CounterSignTarget.NODES) {
                countersignNodes(root, targets, keyEntry, extraParams, algorithm, doc);
            }
            else if (targetType == CounterSignTarget.SIGNERS) {
                countersignSigners(root, targets, keyEntry, extraParams, algorithm, doc);
            }
        }
        catch (final Exception e) {
        ...
        if (esFirmaSimple) {
            try {
                ...
                newdoc.appendChild(newdoc.adoptNode(doc.getElementsByTagNameNS(XMLConstants.DSIGNNS, SIGNATURE_TAG).item(0)));
                doc = newdoc;
            }
            catch (final Exception e) {
    ...
    private static void countersignNodes(final Element root,
        ...
        try {
            for (final Element node : nodes) {
                cs(node, keyEntry, extraParams, algorithm, doc);
            }
        }
        catch (final Exception e) {
    ...
    private static void countersignTree(final Element root, 
        ...
        try {
            for (int i = 0; i < numSignatures; i++) {
                cs(nodes[i], keyEntry, extraParams, algorithm, doc);
            }
        }
        catch (final Exception e) {
    ...
    private static void countersignLeafs(final Element root, 
        ...
        try {
            for (int i = 0; i < numSignatures; i++) {
                ...
                // y crea sus contrafirmas
                if (children == 0) {
                    cs(signature, keyEntry, extraParams, algorithm, doc);
                    numSignatures++;
                    i++;
                }
            }
    ...
    private static void cs(final Element signature, 
        ...
        try {
            ...
            LOGGER.severe("No se ha podido establecer el algoritmo de huella digital (" + XMLConstants.SIGN_ALGOS_URI.get(algorithm) //$NON-NLS-1$
