...
OutFile AutoFirma64/AutoFirma_64_v1_6_3_installer.exe
...
Name "AutoFirma"
Caption "AutoFirma installer (@firma client)"
...
DirText "Choose a directory where to install the application:"
...
UninstallText "AutoFirma Uninstaller."
...
Section "Programa" sPrograma
	...
	pop $3
	IntCmp $1 1 +3 0 0
		MessageBox MB_OK "You can not install AutoFirma 64 bits in a 32 bit environment." 
		Quit
	...
	; Comprueba que no este ya instalada
	  ...
	  ${If} ${Errors}
		...
		IfFileExists '$INSTDIR\unistall.exe' 0 +4
		  MessageBox MB_YESNO "There is an earlier version of AutoFirma on the computer. Do you want to uninstall it?" /SD IDYES IDNO Exit
	  ...
	  ${If} $R2 = 0
        MessageBox MB_OK "This version of Autofirma is already installed." 
	  ${else}
		${If} $R2 = 2
		MessageBox MB_YESNO "There is an earlier version of AutoFirma on the computer. Do you want to uninstall it?" /SD IDYES IDNO Exit
		Goto UninstallOlderVersion
		${else}
		MessageBox MB_OK "The current version of AutoFirma is newer than the one you want to install."
		${EndIf}
      ${EndIf}
	Exit:
	  Quit
   ...
   SetShellVarContext all
	...
	StrCmp $R2 0 0 +2
		MessageBox MB_OK|MB_DEFBUTTON1|MB_ICONEXCLAMATION 'Close the Mozilla Firefox browser to continue with the installation of AutoFirma.' IDOK loopFirefox

	loopChrome:
	${nsProcess::FindProcess} "chrome.exe" $R3
	StrCmp $R3 0 0 +2
		MessageBox MB_OK|MB_DEFBUTTON1|MB_ICONEXCLAMATION 'Close the Google Chrome browser to continue with the installation of AutoFirma.' IDOK loopChrome
	...
	;Registro
	;CascadeAfirma.reg
	WriteRegStr HKEY_CLASSES_ROOT "*\shell\afirma.sign" "" "Sign with AutoFirma"
	...
	;Generar huella archivos
 	WriteRegStr HKEY_CLASSES_ROOT "*\shell\afirma.hashFile" "" "Generate fingerprint with AutoFirma"
	...
	WriteRegStr HKEY_CLASSES_ROOT "Directory\shell\afirma.hashDirectory" "" "Generate fingerprint with AutoFirma"
	...
	;Comprobar huella .hash
 	WriteRegStr HKEY_CLASSES_ROOT ".hash\shell\afirma.hash" "" "Check fingerprint with AutoFirma"
	...
	;Comprobar huella .hashb64
 	WriteRegStr HKEY_CLASSES_ROOT ".hashb64\shell\afirma.hasbh64" "" "Check fingerprint with AutoFirma"
	...
	;Comprobar huella .hashfiles
 	WriteRegStr HKEY_CLASSES_ROOT ".hashfiles\shell\afirma.hashfiles" "" "Check fingerprint with AutoFirma"
	...
	;Verify
	; .csig
	WriteRegStr HKEY_CLASSES_ROOT ".csig" "" "Sign CMS/CAdES"
	...
	;Verify
	; .xsig
	WriteRegStr HKEY_CLASSES_ROOT ".xsig" "" "Sign XMLDSig/XAdES"
...
Function AddCertificateToStore
  ...
  ${If} $R0 <> 0
    ...
    ${Else}
      ...
      StrCpy $0 "It was not possible to open the certificate store"
 
    ${EndIf}
 
  ${Else}
 
    StrCpy $0 "It was not possible to open the certificate file"
...
Function AddToPath
  ...
  IntCmp $4 234 0 +3 +3 ; $4 == ERROR_MORE_DATA
    DetailPrint "The PATH is too long. The AutoAfirma route will not be added."
    Goto done
  IntCmp $4 0 +5 ; $4 != NO_ERROR
    IntCmp $4 2 +3 ; $4 != ERROR_FILE_NOT_FOUND
      DetailPrint "Unexpected error adding route to PATH: $4"
      Goto done
    StrCpy $1 ""
  ...
  IntCmp $2 ${NSIS_MAX_STRLEN} +3 +3 0
    DetailPrint "The AutoFirma route makes the PATH too long. It will not be added"
    Goto done
  ; Append dir to PATH
  DetailPrint "Output Folder: $0"
  StrCpy $2 $1 1 -1
  StrCmp $2 ";" 0 +2
...
Function un.RemoveFromPath
  ...
  StrCmp $2 "" done
  DetailPrint "Remove from the PATH: $0"
  StrLen $3 "$0;"
  StrLen $4 $2
