...
public class InvalidBouncyCastleException extends InvalidLibraryException {
	...
	public InvalidBouncyCastleException(final String expected, final String found, final Throwable e) {
		super("Se necesitaba BouncyCastle version " + expected + ", pero se encontro la version " + found, e, BinaryErrorCode.ThirdParty.BOUNCYCASTLE_VERSION_NOT_SUPPORTED); //$NON-NLS-1$ //$NON-NLS-2$
		this.exp = expected;
		this.fnd = found;
	}
