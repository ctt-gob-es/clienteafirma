...
public final class PdfExtraParams {
     ...
     * de certificaci&oacute;n (propiedad compartida con XAdES y CAdES). */
     ...
     *  Por ejemplo, la cadena <code>2010:12:25:12:30:01</code> indica el venticinco de diciembre de 2010 a las 12 horas,
     ...
     * menos tama&ntilde;o. Esta propiedad s&oacute;lo se aplica si se trata de un PDF v4 o superior, en versiones
     ...
     * crea en documentos que ya contengan alguna firma.<br>
     * En los documentos cifrados <u>siempre</u> se crea una revisi&oacute;n, sea cual sea el valor de este par&aacute;metro.  */
     ...
     * Las p&aacute;ginas comienzan su numeraci&oacute;n desde el uno.
     * Este par&aacute;metro se ignora si se ha establecido valor al par&aacute;metro
    ...
    <p>
     Este texto puede incluir una serie de palabras clave que ser&aacute;n sustituidas por los textos apropiados del titular o emisor del
     ...
     * Se ignora si no se ha establecido adem&aacute;s <code>tsaUsr</code>. */
    static final String TSA_EXTENSION_VALUE_BASE64 = "tsaExtensionValueBase64";//$NON-NLS-1$

    /** Indica si la extensi&oacute;n indicada en <code>tsaExtensionOid</code> es cr&iacute;tica (valor <code>true</code>) o no (valor <code>false</code>).
     * Se ignora si no se ha establecido adem&aacute;s <code>tsaUsr</code>. */
    static final String TSA_EXTENSION_CRITICAL = "tsaExtensionCritical";//$NON-NLS-1$

    /** Almac&eacute;n de claves codificado en Base64 que contiene el certificado SSL cliente que pedir&aacute; la TSA al establecer la conexi&oacute;n HTTPS. */
    ...
    /** Almac&eacute;n de confianza en base64 que contiene los certificados emisores de confianza del certificado SSL de la TSA. */
