...
/* ********  P R I V A T E   F I E L D S  ******** */  
    ...
    /** Preferred encoding. */
    private final static String PREFERRED_ENCODING = "US-ASCII"; //$NON-NLS-1$
...
/* ********  E N C O D I N G   M E T H O D S  ******** */    
    ...
    public static byte[] encodeBytesToBytes( byte[] source, int off, int len, int options ) throws java.io.IOException {
        ...
        if( off + len > source.length  ){
            ...
            String.format( "Cannot have offset of %d and length of %d with array of length %d", Integer.valueOf(off), Integer.valueOf(len), Integer.valueOf(source.length))); //$NON-NLS-1$
        }   // end if: off < 0
        ...
        if( (options & GZIP) != 0 ) {
            ...
            try {
                ...
                try{ baos.close();  } catch( Exception e ){ /* Ignored */ }
            }   // end finally
...
/* ********  D E C O D I N G   M E T H O D S  ******** */
    ...
    byte[] destination, int destOffset, int options ) {
        ...
        if( srcOffset < 0 || srcOffset + 3 >= source.length ){
            ...
            "Source array with length %d cannot have offset of %d and still process four bytes.", Integer.valueOf(source.length), Integer.valueOf(srcOffset) ) ); //$NON-NLS-1$
        }   // end if
        if( destOffset < 0 || destOffset +2 >= destination.length ){
            throw new IllegalArgumentException( String.format(
            "Destination array with length %d cannot have offset of %d and still store three bytes.", Integer.valueOf(destination.length), Integer.valueOf(destOffset) ) ); //$NON-NLS-1$
        }   // end if
    ...
    throws java.io.IOException {
        ...
        if( off < 0 || off + len > source.length ){
            throw new IllegalArgumentException( String.format(
            "Source array with length %d cannot have offset of %d and process %d bytes.", Integer.valueOf(source.length), Integer.valueOf(off), Integer.valueOf(len) ) ); //$NON-NLS-1$
        }   // end if
        ...
        for( i = off; i < off+len; i++ ) {  // Loop through source
            ...
            if( sbiDecode >= WHITE_SPACE_ENC )  {
                ...
                "Bad Base64 input character '%c' in array position %d", Integer.valueOf(source[i]), Integer.valueOf(i) ) ); //$NON-NLS-1$
            }   // end else: 
        ...
        if( (bytes != null) && (bytes.length >= 4) && (!dontGunzip) ) {
            ...
            if( java.util.zip.GZIPInputStream.GZIP_MAGIC == head )  {
                ...
                try { 
                    ...
                    try{ baos.close(); } catch( Exception e ){ /* Ignored */}
                    if (gzis != null) { try{ gzis.close(); } catch( Exception e ){ /* Ignored */} }
                    if (bais != null) { try{ bais.close(); } catch( Exception e ){ /* Ignored */} }
                }   // end finally
            ...
            // the provided class loader.
                ...
                ois = new java.io.ObjectInputStream(bais){
                    ...
                    throws java.io.IOException, ClassNotFoundException {
                        ...
                        return c;   // Class loader knows of this class: not null
                    }   // end resolveClass
                };  // end ois
