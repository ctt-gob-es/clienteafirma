		...
		if (this.servicesRequired) {
			if (params.containsKey(STORAGE_SERVLET_PARAM)) {
				...
				try {
					...
					throw new ParameterException("Error al validar la URL del servicio de guardado del servidor intermedio: " + e, e, ErrorCode.Request.INVALID_STORAGE_URL_TO_SAVE); //$NON-NLS-1$
				}
			...
			// Si no se encuentra a pesar de tener todos los parametros, falla la operacion
				...
				throw new ParameterException("No se ha recibido la direccion del servlet para el guardado del resultado de la operacion en el servidor intermedio", ErrorCode.Request.STORAGE_URL_TO_SAVE_NOT_FOUND); //$NON-NLS-1$
			}
		}
