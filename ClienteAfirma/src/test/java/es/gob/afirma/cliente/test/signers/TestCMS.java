...
public class TestCMS {
	...
	public void testSignature() {
		...
		for(AOConstants.AOKeyStore kstore : KEYSTORES) {
			for (String algo : ALGOS) {
				for (Properties extraParams : CMS_MODES) {
					System.out.println(); System.out.println(); System.out.println();
					System.out.println(
						"Prueba con '" + algo + "', '" + kstore + "' y formato '" + extraParams.getProperty("format")
					);
	...
	public void testCounterSignature() {
		...
		try {
			p7s = AOUtil.getDataFromInputStream(
		...
		if (p7s != null && p7s.length == 0) p7s = null;
			...
			for (String algo : ALGOS) {
				for (Properties extraParams : CMS_MODES) {
					for (CounterSignTarget targetType : CS_TARGETS) {
						System.out.println(); System.out.println(); System.out.println();
						System.out.println(
							"Prueba CONTRAFIRMA con '" + algo + "', '" + kstore + "', formato '" + extraParams.getProperty("format") + "', objetivo '" + targetType + "' y modo '" + extraParams.getProperty("mode") + "'"
						);
						if (AOConstants.AOKeyStore.WINDOWS.equals(kstore) && AOConstants.SIGN_ALGORITHM_SHA512WITHRSA.equals(algo)) {
							System.out.println("Omitimos las pruebas en CAPI con SHA2 por errores conocidos de Java");
							continue;
		...
		for(AOConstants.AOKeyStore kstore : KEYSTORES) {
			for (String algo : ALGOS) {
				for (Properties extraParams : CMS_MODES) {
					...
					System.out.println(); System.out.println(); System.out.println();
					System.out.println(
						"Prueba COFIRMA con '" + algo + "', '" + kstore + "', formato '" + extraParams.getProperty("format") + "' y modo '" + extraParams.getProperty("mode") + "'"
					);
					if (AOConstants.AOKeyStore.WINDOWS.equals(kstore) && AOConstants.SIGN_ALGORITHM_SHA512WITHRSA.equals(algo)) {
						System.out.println("Omitimos las pruebas en CAPI con SHA2 por errores conocidos de Java");
						continue;
