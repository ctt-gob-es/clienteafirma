...
import es.gob.afirma.keystores.MozillaKeyStoreUtilities;
...
/**
 * Almacena una configuracui&oacute;n para el almac&eacute;bn establecido del Cliente @firma. 
 * Gestiona su inicializaci&oacute;n y la selecci&oacute;n de certificados por parte del
 * usuario.
 */
final class KeyStoreConfigurationManager {
	...
	/** Ruta del almac&eacute;n de claves seleccionado (para alamcenes P12/PFX, JKS, P11...). */
	...
	/**
	 * Indica si debe advertirse al usuario de que inserte los dispositivos criptogr&aacute;ficos
	 * externos antes de inicializar el almac&eacute;n de certificados.
	 */
	private boolean showLoadingWarning = false;
	
	/** Configuraci&oacute;n para el filtrado de certificados en el di6aacute;logo de selecci&oacute;n. */
	private CertificateFilterConfiguration filterConfig = null;
	
	/**
	 * Construye la configuraci&oacute;n por defecto para el Cliente, pudiendo variar el
	 * almac&eacute;n seg&uacute;n el sistema operativo:
	 * <ul>
	 ...
	 * </ul>
	 * @param keyStore Almac&eacute;n de claves por defecto.
	 * @param parent Componente padre sobre el que se mostrar&aacute;n los di&aacute;logos
	 * modales necesarios.
	 */
	...
	/**
	 * Recupera el almac&eacute;n de claves por defecto para el sistema operativo actual.
	...
	private AOKeyStore getDefaultKeyStore() {
		...
		if ( Platform.getOS().equals(Platform.OS.LINUX) ||
			 Platform.getOS().equals(Platform.OS.SOLARIS)) return AOKeyStore.MOZ_UNI;
	...
	/**
	 * Reestablece el almac&eacute;n de certificados por defecto y reinicia su configuraci&oacute;n.
	 */
	void initialize() {
	 ...
	 * @throws AOKeystoreAlternativeException Cuando ocurre un error durante la inicializaci&oacute;n 
	 */
	private void initKeyStore(final String path, final String password) throws AOCancelledOperationException, AOKeystoreAlternativeException {
		...
		// Comprobamos que la configuracion sea valida, y si no lo es cambiamos el almacen a
		// un PKCS#12
		if (!checkKeyStoreAccess()) {
			JOptionPane.showMessageDialog(
					this.parent,
					this.errorMessage + " " + AppletMessages.getString("KeyStoreConfigurationManager.0"), //$NON-NLS-1$ //$NON-NLS-2$
					AppletMessages.getString("SignApplet.156"),  //$NON-NLS-1$
					JOptionPane.ERROR_MESSAGE); 
			
			this.changeKeyStore(AOKeyStore.PKCS12);
			this.ksPath = null;
		...
		this.ksManager = AOKeyStoreManagerFactory.getAOKeyStoreManager(
				...
				this.ksPath,
				null,
				this.getKeystorePasswordCallback(),
				this.parent
		);
	}
	...
	/**
	 * Cambia el tipo de almac&eacute;n de claves establecido. Si existe una configuraci&oacute;n
	 * previa (contrase&ntilde;a del almacen, alias seleccionado,...) es recomendable utilizar
	 * previamente {@link #initialize()}. 
	 * @param keyStore Nuevo almac&eacute;n de claves.
	 */
	void changeKeyStore(final AOKeyStore keyStore) {
		...
		// Inicializamos la configuracion interna, la externa se respeta porque puede haberse
	...
	/**
	 * Selecciona un certificado con clave privada del almac&eacute;n, ya sea a trav&eacute;s
	 * de la configuraci&oacute;n proporcionada o solicit&aacute;ndoselo al usuario. 
	 * @throws AOCancelledOperationException Cuando el usuario cancela la operaci&oacute;n.
	 * @throws AOCertificateException Cuando no se pueda seleccionar un certificado.
	 * @throws AOCertificateKeyException Cuando no se puede extraer la clave privada de un certificado.
	 * @throws AOKeyStoreManagerException Cuando no se pueda inicializar el almac&eacute;n de certificados.
	 * @throws AOCertificatesNotFoundException Cuando no se encuentran certificados v&aacute;lido en el almac&eacute;n.
	 * @throws AOKeystoreAlternativeException Cuando no se pueda inicializar el almac&eacute;n de certificados pero existe un almac&eacute;n alternativo
	 */
	...
	/**
	 * Selecciona un certificado del almac&eacute;n, ya sea a trav&eacute;s de la configuraci&oacute;n
	 * proporcionada o solicit&aacute;ndoselo al usuario.
	 * @param checkPrivateKey Si es {@code true}, filtra los certificados que no tienen clave privada.  
	 * @throws AOCancelledOperationException Cuando el usuario cancela la operaci&oacute;n.
	 * @throws AOCertificateException Cuando no se pueda seleccionar un certificado.
	 * @throws AOCertificateKeyException Cuando no se puede extraer la clave privada de un certificado.
	 * @throws AOKeyStoreManagerException Cuando no se pueda inicializar el almac&eacute;n de certificados.
	...
	private void selectCertificate(final boolean checkPrivateKey) throws AOCancelledOperationException, 
		...
		// En caso de ser todos certificados con clave privada, obtenemos la referencia a esta
		if (checkPrivateKey) {
	...
	/**
	 * Recupera el certificado con el alias indicado del repositorio actual. Si no existe
	 * ning&uacute;n certificado con ese alias, se devolver&aacute; {@code null}. 
	 ...
	 * @throws AOKeyStoreManagerException Cuando no se ha podido inicializar el almac&eacute;n
	 * de certificados.
	 */
	...
	/**
	 * Recupera los alias del almacen seleccionado. Si ocurre alg&uacute;n error durante
	 * la operaci&oacute;n se devuelve un array vac&iacute;o.
	 * @return Certificado con el alias seleccionado.
	 * @throws AOCancelledOperationException Cuando se cancela la operaci&oacute;n.
	 * @throws AOKeyStoreManagerException Cuando no se ha podido inicializar el almac&eacute;n
	 * de certificados.
	...
	/**
	 * Recupera el gestor del almac&eacute;n de certificados actual ya inicializado.
	...
	/**
	 * Devuelñve el almac&eacute;n de certificados configurado.
	 ...
	 * @param certAlias Alias de los certificados sobre los que hay que aplicar el filtro.
	 ...
	 * @throws AOCertificatesNotFoundException Cuando no se encuentran certificados que cumplan el filtro.
	 * @throws AOCancelledOperationException Cuando el usuario cancel&oacute; la operaci&oacute;n.
	 */
	private String showCertSelectionDialog(final String[] certAlias, 
			                               final boolean checkPrivateKey) throws AOCertificatesNotFoundException, 
		...
		if(optionAlias.length == 0) {
			throw new AOCertificatesNotFoundException(
				"No se ha encontrado ning\u00FAn certificado que cumpla el patr\u00F3n requerido. Se cancelar\u00E1 la operaci\u00F3n." //$NON-NLS-1$
			);
		}
				
		return AOUIManager.showCertSelectionDialog(
				optionAlias, // Aliases
				(this.ksManager == null) ? null : this.ksManager.getKeyStores(), // KeyStores
						this.filterConfig.getKeyUsageFilter(),  // Filtro por KeyUsage
						...
						this.filterConfig.isShowExpiratedCertificates(), // Muestra certificados caducados
						this.filterConfig.getRfc2254IssuerFilter(),  // Filtro RFC2254 para el Ussuer
						this.filterConfig.getRfc2254SubjectFilter(), // Filtro RFC2254 para el Subject
						this.filterConfig.isMandatoryCert()			 // Solo se admite un certificado
		);
	}
	 ...
	 * @param aliasList Alias de los certificados a comprobar.
	 * @param certFilter Filtro con las condiciones que debe cumplir el Principal del certificado.
	 * @return Alias de los certificados que pasan el filtro.
	 * @see #setCertFilter(String) Establece el filtro de certificados.
	 * @deprecated
	 */
	@Deprecated
	private String[] filtCert(String[] aliasList, es.gob.afirma.cliente.AOCertFilter certFilter) {
	...
	/**
	 * Recupera el PasswordCallback apropiado para los certificados del almac&eacute;n concreto o,
	 * si se especific&oacute; una contrase&ntilde;a, uno con ella prefijada.
	 * @return PasswordCallback para la obtenci&oacute;n de la clave del certificado.
	 */
	...
	/**
	 * Recupera el PasswordCallback apropiado para el almacen de claves configurado o,
	 * si se especific&oacute; una contrase&ntilde;a, uno con ella prefijada.
	 ...
	 * @param selectedAlias Alias del certificado.
	 */
	 ...
	 * @param password Contrase&ntilde;a para el almac&eacute;n
	 */
	 ...
	 * @param path Ruta absoluta del almac&eacute;n.
	 */
	...
	/**
	 * Indica si se seleccionar&aacute; autom&aacute;ticamente el certificado de firma.
	...
	/**
	 * Establece el filtro seg&uacute;n usos permitidos del certificado. El par&acute;metro
	 * recibido puede ser {@code null}, para eliminar el filtro, o un array de 8 elementos.<br/>
	 * Cada certificado puede permitir simult&aacute;neamente cualquiera de estos 8 usos:<br/>
	 * <ol>
	 ...
	 * </ol>
	 * Cada uno de los elementos del array designan en orden a uno de estos 8 usos. El valor de
	 ...
	 * </ul>
	 * @param keyUsageFilter Usos permitidos del certificado.
	 */
	...
	/**
	 * NO SE RECOMIENDA EL USO DE ESTA FUNCI&Oacute;N. Establece una expresi&oacute;n regular
	 * para el filtrado de certificados a partir de su principal. 
	 * @param nameFilter Filtro para el principal del certificado.
	 * @deprecated
	 */
	@Deprecated
	...
	/**
	 * Establece el filtro seg&uacute;n la RFC2254 para el Subject del certificado.
	 * @param rfc2254SubjectFilter Filtro RFC2254.
	 */
	...
	/**
	 * Establece el filtro seg&uacute;n la RFC2254 para el emisor del certificado.
	 * @param rfc2254IssuerFilter Filtro RFC2254.
	 */
	 ...
	 * @param showExpiratedCertificates {@code true} para mostrar los certificados caducados.
	 */
	...
	/**
	 * Establece que se seleccione autom&aacute;ticamente el certificado cuando s&oacute;lo
	 * quede uno despu&eacute;s de pasar los distintos filtros. Si quedase m&aacute;s de un
	 * certificado se lanzar&iacute;a una excepci&oacute;n.
	 * @param mandatoryCert {@code true} para indicar que se seleccione autom&aacute;ticamente
	 * el certificado.
	 */
	...
	/**
	 * Establece que se muestre o no, antes de cargar un almac&eacute;n de certificados, una advertencia
	 * del tipo "Inserte su tarjeta inteligente o cualquier otro dispositivo antes de continuar".
	 * @param showWarning Si es {@code true} se mostrar&aacute; la advertencia.
	 */
	...
	/**
	 * Comprueba que la configuraci&oacute;n actual del keystore del Cliente sea v&aacute;lida.
	 * Si la configuraci&oacute;n no es v&aacute;lida establece el mensaje de error para recuperse
	 * mediante el m&eacute;todo {@link #getErrorMessage()}.  
	 * @return Devuelve {@code true} si la configuraci&oacute;n es v&aacute:lida, {@code false}
	 * en caso contrario.
	 */
	boolean checkKeyStoreAccess() {
		if (this.ks == AOKeyStore.MOZILLA || this.ks == AOKeyStore.MOZ_UNI) {
			try {
				MozillaKeyStoreUtilities.getSystemNSSLibDir();
			} catch (FileNotFoundException e) {
				Logger.getLogger("es.gob.afirma").warning("Error: " + e); //$NON-NLS-1$ //$NON-NLS-2$
				this.errorMessage = AppletMessages.getString("KeyStoreConfigurationManager.1"); //$NON-NLS-1$
				return false;
			} catch (AOInvalidFormatException e) {
				Logger.getLogger("es.gob.afirma").warning("Error: " + e); //$NON-NLS-1$ //$NON-NLS-2$
				this.errorMessage = AppletMessages.getString("KeyStoreConfigurationManager.2"); //$NON-NLS-1$
				return false;
			}
		}
	...
	/**
	 * Recupera el mensaje de error identificado en la configurac&oacute;n actual para
	 * el almac&eacute;n. Si no se ha identificado ning&uacute;n error o aun no se ha
	 * ejecutado el m,&eacute;todo {@link #checkKeyStoreAccess()} se devolver&aacute;
	 * {@code null}. 
