...
/**
 * Firmas OOXML basadas en una versi&oacute;n fuertemente modificada de las clases
 * <code>es.gob.afirma.be.fedict.eid.applet.service</code>.
 */
public final class AOOOXMLSigner implements AOSigner {

	// En Java 5 es necesario disponer del proveedor XMLDSigRI para generar firmas XMLdSig
	static {
	...
	/**
	 * Comprueba que unos datos se adecuen a la estructura b&aacute;sica de un documento OOXML.
	 * @param is Datos que deseamos analizar.
	 * @return {@code true} si el documento es un OOXML, {@code false} en caso contrario.
	 */
	private boolean isOOXMLFile(final InputStream is) {

		ZipFile zipFile = null;
		try {
			zipFile = AOFileUtils.createTempZipFile(AOUtil.getDataFromInputStream(is));
		...
		return zipFile.getEntry("[Content_Types].xml") != null &&
			(zipFile.getEntry("_rels/.rels")!= null || zipFile.getEntry("_rels\\.rels") != null) &&
	...
	public String getDataMimeType(InputStream signData) throws AOUnsupportedSignFormatException {
      ...
      throw new NullPointerException("Los datos de firma introducidos son nulos");
		...
		InputStream contentTypesXml = null;
		ZipEntry zipEntry = null;
		byte[] ooxmlDocument = null;
		try {
			ooxmlDocument = AOUtil.getDataFromInputStream(signData);
			ZipFile zipFile = AOFileUtils.createTempZipFile(ooxmlDocument);
      ...
      return null;
		...
		if(zipEntry == null || contentTypesXml == null || isOOXMLFile(new ByteArrayInputStream(ooxmlDocument))) {
  ...
  return new AOSignInfo(AOConstants.SIGN_FORMAT_OOXML);
	...
	public TreeModel getSignersStructure(InputStream sign, boolean asSimpleSignInfo) {
      ...
      throw new NullPointerException("Los datos de firma introducidos son nulos");
		...
		byte[] ooxmlDocument = null;
		try {
			ooxmlDocument = AOUtil.getDataFromInputStream(sign);
		} catch (Exception e) {
      Logger.getLogger("es.gob.afirma").severe("Ocurrio un error al analizar el fichero de firma: "+e);
      return null;
		}
		
		if(!isSign(new ByteArrayInputStream(ooxmlDocument))) {
			Logger.getLogger("es.gob.afirma").severe("La firma indicada no es de tipo OOXML");
			return null;
		}

		// Las firmas contenidas en el documento OOXML son de tipo XMLdSig asi que utilizaremos el
		// signer de este tipo para gestionar el arbol de firmas
		AOXMLDSigSigner xmldsigSigner = new AOXMLDSigSigner();
		...
		try {
			for(byte[] elementSign : OOXMLUtil.getOOXMLSignatures(ooxmlDocument)) {
				
				// Recuperamos el arbol de firmas de la firma individual. Ya que esta sera una firma simple
				// solo debe contener un nodo de firma. Ignoramos la raiz del arbol, que contiene
				// el ejemplo representativo de los datos firmados y no de la propia firma.
				TreeModel signTree = xmldsigSigner.getSignersStructure(new ByteArrayInputStream(elementSign), asSimpleSignInfo);
				tree.add((MutableTreeNode)((javax.swing.tree.TreeNode)signTree.getRoot()).getChildAt(0));
			}
		} catch (Exception e) {
	...
	public boolean isSign(InputStream signData) {
		if(signData == null) {
			Logger.getLogger("es.gob.afirma").warning(
					"Se ha introducido una firma nula para su comprobacion"
			);
			return false;
		}
		return isOOXMLFile(signData);
	}

	public boolean isValidDataFile(InputStream dataFile) {
		if(dataFile == null) {
			Logger.getLogger("es.gob.afirma").warning(
	...
	public byte[] cosign(final InputStream signFile, 
		...
		return addNewSign(signFile, algorithm, keyEntry, cert, extraParams);
	}
	 ...
	 */
	private byte[] addNewSign(InputStream ooxmlFile, 
      String algorithm, 
      ...
      Properties extraParams) throws AOException {
		...
		byte[] ooxmlDocument = null;
		try {
			ooxmlDocument = AOUtil.getDataFromInputStream(ooxmlFile);
		} catch (Exception e) {
			throw new AOException("Error al leer el fichero firma", e);
		}
		
		return signOOXML(ooxmlDocument, OOXMLUtil.countOOXMLSignatures(ooxmlDocument)+1, algorithm, keyEntry, cert, extraParams);
	}
	 ...
	 * @throws AOException Cuando ocurre alg&uacute;n error durante el proceso de firma.
	 */
	private byte[] signOOXML(byte[] ooxmlDocument,
			int signNum,
			String algorithm, 
			PrivateKeyEntry keyEntry, 
			X509Certificate cert,
		...
		if (algorithm == null) digestAlgo = "SHA1";
			...
			Logger.getLogger("es.gob.afirma").warning(
					"El algoritmo de firma '" + algorithm + "' no esta soportado en OOXML, se utilizara SHA1 con RSA"
			);
			digestAlgo = "SHA1";
		}

    /*
     * Si se solicito una firma explicita, advertimos no son compatibles con OOXML y se
    ...
    if (extraParams != null && extraParams.containsKey("mode")
    		&& extraParams.getProperty("mode").equals(AOConstants.SIGN_MODE_EXPLICIT)) {
    			Logger.getLogger("es.gob.afirma").warning(
    					"El formato de firma OOXML no soporta el modo de firma explicita, " +
    ...
    if (!OfficeXMLAnalizer.isOOXMLDocument(ooxmlDocument)) {
		...
		Vector<X509Certificate> certChain = new Vector<X509Certificate>(chain.length);
		...
		try {
			return new AbstractOOXMLSignatureServiceContainer().sign(
					new ByteArrayInputStream(ooxmlDocument), certChain, digestAlgo, keyEntry.getPrivateKey(), signNum);
		}
		catch(Throwable e) {
			e.printStackTrace();
