...
public final class CMSInformation {
	...
	public String getInformation(InputStream data) throws IOException, AOInvalidFormatException {
    	String datos="";
    	...
    	if (doi.equals(PKCSObjectIdentifiers.data)){
    		...
    		datos = getFromAuthenticatedData(doj);
    	}
    	else if(doi.equals(PKCSObjectIdentifiers.id_ct_authEnvelopedData)){
    		datos = getFromAuthenticatedEnvelopedData(doj);
    	}
   ...
   /**
    ...
    /**
     * Obtiene la informaci&oacute;n de un tipo Authenticated Data.
     * @return  Representaci&oacute;n de los datos.
     */
    private String getFromAuthenticatedData(ASN1TaggedObject doj) {
        String detalle = "";
        detalle = detalle + "Tipo: AuthenticatedData\n";
        ...
        // mac algorithm
        AlgorithmIdentifier aid = ed.getMacAlgorithm();
        detalle = detalle + "OID del Algoritmo de MAC: "+aid.getObjectId()+"\n";
        ...
        //obtenemos datos de los datos cifrados.
        ContentInfo ci = ed.getEncapsulatedContentInfo();
        detalle = detalle + "OID del tipo de contenido: "+ci.getContentType()+"\n";
        
        //obtenemos lo atributos obligatorios
        ASN1Set authAttrs=ed.getAuthAttrs();
        if (authAttrs == null){
            detalle = detalle + "Atributos Autenticados: No tiene atributos autenticados\n";
        }
        else{
            String atributos = getsignedAttributes(authAttrs);
            detalle = detalle + "Atributos Autenticados: \n";
            detalle = detalle + atributos;
        }

        //obtenemos lo atributos opcionales
        ASN1Set unprotectedAttrs=ed.getUnauthAttrs();
        if (unprotectedAttrs == null){
            detalle = detalle + "Atributos Opcionales: No tiene atributos opcionales\n";
        }
        else{
            String atributos = getUnSignedAttributes(unprotectedAttrs);
            detalle = detalle + "Atributos Opcionales: \n";
            detalle = detalle + atributos;
        }
        return detalle;
    }
    ...
    /**
     * Obtiene la informaci&oacute;n de un tipo AuthenticatedEnveloped Data.
     * @return  Representaci&oacute;n de los datos.
     */
    private String getFromAuthenticatedEnvelopedData(ASN1TaggedObject doj) {
    	 String detalle = "";
         detalle = detalle + "Tipo: AuthenticatedEnvelopedData\n";
         ...
         //obtenemos datos de los datos cifrados.
         EncryptedContentInfo ci = ed.getAuthEncryptedContentInfo();
         detalle = detalle + "Informacion de los datos cifrados:\n";
         detalle = detalle + getEncryptedContentInfo(ci);
    ...
    private String getFromCompressedData(ASN1TaggedObject doj) {
        ...
        AlgorithmIdentifier aid = ed.getCompressionAlgorithmIdentifier();
        if (aid.getObjectId().toString().equals("1.2.840.113549.1.9.16.3.8")){
        	detalle = detalle + "OID del Algoritmo de firma: ZLIB\n";
        }
        else{
        	detalle = detalle + "OID del Algoritmo de firma: "+aid.getObjectId()+"\n";
        }
