...
public final class CMSHelper {
	 ...
	 * </ul>
	 * @param data Fichero (como array de octetos) que deseamos comprobar.
	...
	public static boolean isCMSValid(final byte[] data){
		...
		if (!valid) valid = validator.isCMSEncryptedData(data); // Comprobamos ENCRYPTEDDATA
		if (!valid) valid = validator.isCMSEnvelopedData(data); // Comprobamos ENVELOPEDDATA
		if (!valid) valid = validator.isCMSSignedAndEnvelopedData(data); // Comprobamos SIGNEDANDENVELOPED
		if (!valid) valid = validator.isCMSAuthenticatedData(data); // Comprobamos AUTHENTICATED
		if (!valid) valid = validator.isCMSAuthenticatedEnvelopedData(data); // Comprobamos AUTHENTICATEDENVELOPEDDATA
		if (!valid) valid = validator.isCMSCompressedData(data); // Comprobamos COMPRESSEDDATA
	 ...
	 * @param data Datos que queremos comprobar.
	...
	public static boolean isCMSValid(byte[] data, String type){
		...
		} else if(type.equals(AOConstants.CMS_CONTENTTYPE_AUTHENTICATEDDATA)) {
			return new ValidateCMS().isCMSAuthenticatedData(data);
		} else if(type.equals(AOConstants.CMS_CONTENTTYPE_AUTHENVELOPEDDATA)) {
			return new ValidateCMS().isCMSAuthenticatedEnvelopedData(data);
		} else if(type.equals(AOConstants.CMS_CONTENTTYPE_COMPRESSEDDATA)) {
