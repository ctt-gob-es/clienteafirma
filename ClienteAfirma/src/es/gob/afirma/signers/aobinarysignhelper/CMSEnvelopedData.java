...
public final class CMSEnvelopedData  {
	 ...
	 */
	public byte[] genEnvelopedData(P7ContentSignerParameters parameters, AOCipherConfig config, 
			X509Certificate[] certDest, Oid dataType, Map<Oid, byte[]> uatrib) throws IOException, 
		...
		unprotectedAttrs = Utils.generateSignerInfo(digestAlgorithm, parameters.getContent(), dataType, uatrib);
	 ...
	 */
	public byte[] genEnvelopedData(byte[] data, String digestAlg, AOCipherConfig config, 
			X509Certificate[] certDest, Oid dataType, Map<Oid, byte[]> uatrib) throws IOException, CertificateEncodingException, 
