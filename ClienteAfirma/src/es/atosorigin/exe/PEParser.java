...
public class PEParser {
	...
	public boolean parse(final byte[] peFile, final Logger logger) throws AOException {
        ...
        if (AOBinUtil.getInt(peFile, coffOffset+8) != 0) { // +8, +9, +10, +11
        	Logger.getLogger("es.atosorigin").warning("El puntero a la informacion de depuracion del COFF no es cero (se implementa una caracteristica deprecada)"); 
        }
        
        if (AOBinUtil.getInt(peFile, coffOffset+12) != 0) { // +12, +13, +14, +15
        	Logger.getLogger("es.atosorigin").warning("El numero de entradas de la tabla de simbolos no es cero (se implementa una caracteristica deprecada)"); 
        }
