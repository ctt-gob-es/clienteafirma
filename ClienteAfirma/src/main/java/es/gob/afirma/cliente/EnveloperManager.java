...
public final class EnveloperManager {
	...
	public void setSignAlgorithm(final String signAlgorithm) {
		this.signAlgorithm = (signAlgorithm == null ? AOConstants.DEFAULT_SIGN_ALGO
				: signAlgorithm);
	}
	 ...
	 */
	public void addRecipient(final byte[] certEncoded)
			throws AOCertificateException {
	 ...
	 * @throws AOCertificateException
	 */
	public void removeRecipient(final byte[] certEncoded)
			throws AOCertificateException {
	 ...
	 *             certificado.
	 */
	private X509Certificate encodeCertificate(final byte[] cert)
			throws AOCertificateException {
		try {
			return (X509Certificate) CertificateFactory.getInstance("X.509") //$NON-NLS-1$
					.generateCertificate(new ByteArrayInputStream(cert));
		} catch (final Exception e) {
			throw new AOCertificateException(
					"No se ha podido decodificar el certificado proporcionado",
					e);
		}
	}
	 ...
	 */
	public void envelop() throws IOException, NoSuchAlgorithmException,
	...
	public void envelop(final byte[] content)
		...
		if (contentType.equals(AOConstants.CMS_CONTENTTYPE_ENVELOPEDDATA)) {
			...
			throw new IllegalArgumentException(
					"Tipo de envoltorio CMS no soportado");
		}
	}
	 ...
	 */
	public void encrypt(final byte[] content) throws AOException,
	 ...
	 */
	public void unwrap() throws CertificateEncodingException,
			AOInvalidRecipientException, AOInvalidFormatException, IOException,
			AOException {
	 ...
	 *             Cuando se produce un error durante al desenvolver los datos.
	 */
	public void unwrap(final byte[] envelop)
			throws CertificateEncodingException, AOInvalidRecipientException,
			AOInvalidFormatException, IOException, AOException {

		// Comprobamos si requiere un certificado para la extraccion de los
		// datos
		if (enveloper.isCMSValid(envelop,
				AOConstants.CMS_CONTENTTYPE_ENVELOPEDDATA)
				|| enveloper.isCMSValid(envelop,
			...
			if (!ksConfigManager.isSelectedCertificate()) {
				try {
					...
					throw new AOException(
							"Error al obtener el certificado seleccionado", e);
				}
			}
			enveloper.setOriginatorKe(ksConfigManager.getCertificateKeyEntry());
		} else if (enveloper.isCMSValid(envelop,
			...
			if (cipherManager.getCipherAlgorithm().supportsKey()) {
				...
				enveloper.setCipherKey(String.valueOf(this.cipherManager
						.getCipherPassword()));
			}
		}
	...
	private byte[] recoverDataFromFile() throws IOException {
		...
		if (fileUri == null) {
			try {
				fileUri = AOUtil.createURI(AOUIManager.getLoadFileName(null,
						null, parent));
			} catch (final Exception e) {
		...
		try {
			...
			throw new IOException(
					"No se pudieron leer los datos del fichero seleccionado: "
							+ e);
		} finally {
			try {
				if (is != null)
	 ...
	 *             v&aacute;lida.
	 */
	private byte[] createCMSEncryptedData(final byte[] content)
			throws AOException, NoSuchAlgorithmException {
		return enveloper.createCMSEncryptedData(content,
				cipherManager.getCipherConfig(),
				cipherManager.getConfiguredKey());
	}
	 ...
	 */
	private byte[] createCMSEnvelopData(final byte[] content)
			throws IOException, CertificateEncodingException,
			NoSuchAlgorithmException {
		return enveloper.createCMSEnvelopedData(content,
				ksConfigManager.getCertificateKeyEntry(), // Si hay uno
															// seleccionado, se
															// utiliza
				cipherManager.getCipherConfig(), this.getRecipients());
	}
	 ...
	 */
	private byte[] createCMSSignedEnvelopData(final byte[] content)
			throws AOException, CertificateEncodingException,
	 ...
	 *             requerido por el tipo de sobre
	 */
	private byte[] createCMSAuthenticatedEnvelopData(final byte[] content)
			throws AOException, CertificateEncodingException,
			NoSuchAlgorithmException, IOException {
		if (!ksConfigManager.isSelectedCertificate()) {
			try {
				...
				throw new AOException(
						"Error al obtener seleccionar el certificado", e);
			}
		}
	...
	public byte[] coEnvelop(final byte[] envelop) throws AOException,
			AOCancelledOperationException, AOCertificatesNotFoundException,
			AOKeyStoreManagerException, AOInvalidFormatException {
		if (!ksConfigManager.isSelectedCertificate()) {
			try {
				ksConfigManager.selectCertificate();
			} catch (final Exception e) {
		...
		return envelopedData = enveloper.addOriginator(envelop,
				ksConfigManager.getCertificateKeyEntry());
	}
	 ...
	 *            Valor asignado
	 */
	public void addSignedAttribute(final org.ietf.jgss.Oid oid,
			final byte[] value) {
	 ...
	 *            Valor asignado
	 */
	public void addUnsignedAttribute(final org.ietf.jgss.Oid oid,
			final byte[] value) {
