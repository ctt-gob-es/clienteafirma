 ...
 */
@Deprecated
public final class ComplexCondition extends AClause implements
		ConditionConstants {

	/**
	 * @param str
	 *            Texto de la expresi&oacute;n
	 * @throws AOException
	...
	public ComplexCondition(final String str) throws AOException {
		Matcher compoundConditionMatcher = compoundConditionPattern
				.matcher(str);
		...
		if (nexusConditionMatcher.find()) {
			...
			while (pos < str.length() - 1 && pos > -1) {
				if (nexusConditionMatcher.find(pos)) {
					...
					throw new AOException("Error de sintaxis, pos=" + pos
							+ ": " + str);
				}
			...
			clauses = new Clause[] { new SimpleCondition(str.substring(1,
					str.length() - 1)) };
			nexus = new Nexus[0];
		}
