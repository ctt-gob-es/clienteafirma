...
public final class CipherManager {
	...
	public CipherManager() {
		this.cipherConfig = new AOCipherConfig(AOCipherAlgorithm.getDefault(),
				null, null);
	}
	...
	public void initialize() {
		cipherConfig = new AOCipherConfig(AOCipherAlgorithm.getDefault(), null,
				null);
		cipherKeyAlias = null;
	...
	public void setCipherAlgorithm(AOCipherAlgorithm cipAlgo) {
		this.cipherConfig.setAlgorithm(cipAlgo == null ? AOCipherAlgorithm
				.getDefault() : cipAlgo);
	}
	...
	public void setKeyMode(String keyMode) {
		this.keyMode = (keyMode == null ? AOConstants.DEFAULT_KEY_MODE
				: keyMode);
	}
	...
	public String getCipheredDataB64Encoded() {
		return (cipheredData == null ? null : StringUtils.newStringUtf8(Base64
				.encodeBase64(cipheredData, false)));
	}
	...
	public void setCipheredData(String cipheredDataB64) {
		this.cipheredData = (cipheredDataB64 == null ? null : Base64
				.decodeBase64(cipheredDataB64));
	}
	 ...
	 */
	public void cipherData() throws IOException, NoSuchAlgorithmException,
			AOException {
		...
		if (plainData != null) {
			...
			// Fichero de entrada
			if (fileUri == null) {
				String fileName = AOUIManager.getLoadFileName(null, null,
						parent);
				try {
					fileUri = AOUtil.createURI(fileName);
				} catch (Exception e) {
					throw new IOException(
							"Se ha proporcionado un nombre de fichero no valido: "
									+ e);
				}
			}
	 ...
	 */
	public void cipherData(final byte[] dataToCipher)
	 ...
	 *             Ocurri&oacute; un error al obtener la clave.
	 */
	private Key getConfiguredKey(AOCipher cipher, AOCipherConfig config)
			throws NoSuchAlgorithmException, AOException {
		...
		if (keyMode.equals(AOConstants.KEY_MODE_GENERATEKEY)) {
			...
			} else if (useCipherKeyStore
					&& AOCipherKeyStoreHelper.storeExists()) {
				try {
					...
					throw new AOException(
							"Error al extraer una clave del almacen de clave de cifrado",
							e);
				}
			} else {
				throw new AOException(
						"No se ha establecido una clave de cifrado");
			}
		} else if (keyMode.equals(AOConstants.KEY_MODE_PASSWORD)) {
			if (cipherPassword == null || cipherPassword.length == 0) {
				cipherPassword = AOUIManager
						.getPassword(
								AppletMessages.getString("SignApplet.414"), AOConstants.ACCEPTED_CHARS, true, parent); //$NON-NLS-1$
			}
	 ...
	 */
	public void decipherData() throws IOException, AOException,
			AOInvalidKeyException {
	 ...
	 */
	public void decipherData(final byte[] dataToDecipher) throws AOException,
			AOInvalidKeyException {
		...
		if (keyMode.equals(AOConstants.KEY_MODE_PASSWORD)) {
			if (cipherPassword == null || cipherPassword.length == 0) {
				cipherPassword = AOUIManager.getPassword(
						AppletMessages.getString("SignApplet.414"), parent); //$NON-NLS-1$
			}
			decipherKey = decipher.decodePassphrase(cipherPassword,
					cipherConfig, null);
		} else {
			...
			if (cipherB64Key == null) {
				if (AOCipherKeyStoreHelper.storeExists()) {
					try {
						...
						throw new AOException(
								"Error al extraer una clave del almacen de clave de cifrado", e); //$NON-NLS-1$
					}
				} else {
					throw new AOException(
							"No se ha indicado la clave para el descifrado de datos y no hay almacen de claves"); //$NON-NLS-1$
				}
			} else {
				decipherKey = decipher.decodeKey(cipherB64Key, cipherConfig,
						null);
			}
		}
		...
		plainData = decipher
				.decipher(dataToDecipher, cipherConfig, decipherKey);
	}
	...
	private Key getKeyFromCipherKeyStore() throws AOException {
		...
		if (!AOCipherKeyStoreHelper.storeExists()) {
			throw new AOException(
					"No existe un almacen de claves de cifrado asociado al usuario"); //$NON-NLS-1$
		}
		...
		try {
			cKs = new AOCipherKeyStoreHelper(
					cipherKeystorePass != null ? cipherKeystorePass
							: AOUIManager.getPassword(
									AppletMessages.getString("SignApplet.52"), parent) //$NON-NLS-1$
			);
		} catch (AOCancelledOperationException e) {
			throw e;
		} catch (Exception e) {
			throw new AOException(
					"Error al abrir el repositorio de claves del usuario", e); //$NON-NLS-1$
		}
		...
		if (cipherKeyAlias == null) {
			try {
				alias = AOUIManager.showCertSelectionDialog(cKs.getAliases(),
						null, null, parent, true, true, true);
			} catch (final AOCancelledOperationException e) {
				throw e;
			} catch (Exception e) {
				throw new AOException(
						"Error seleccionar la clave de cifrado", e); //$NON-NLS-1$
			}
		} else {
	 ...
	 */
	private void saveCipherKey(AOCipherConfig config, Key cipherKey)
			throws AOMaxAttemptsExceededException, AOException {
		...
		if (selectedOption == JOptionPane.CANCEL_OPTION
			...
			throw new AOCancelledOperationException(
					"Se ha cancelado el guardado de la clave de cifrado"); //$NON-NLS-1$
			...
			if (cipherKeystorePass != null) {
				try {
					...
					throw new AOException(
							"La contrasena del almacen de claves de cifrado no es valida",
							e);
				}
			} else {
				int numTries = 0;
				do {
					...
					try {
						cKs = new AOCipherKeyStoreHelper(
								new UIPasswordCallback(AppletMessages
										.getString("SignApplet.42"), parent)
										.getPassword());
					} catch (IOException e) {
						if (numTries >= 3) {
							throw new AOMaxAttemptsExceededException(
									"Se ha sobrepasado el numero maximo de intentos en la insercion de la clave del almacen");
						}
					}
				} while (cKs == null);
			}
			...
			if (alias == null) {
				try {
					alias = JOptionPane.showInputDialog(parent,
							AppletMessages.getString("SignApplet.46"), //$NON-NLS-1$
							AppletMessages.getString("SignApplet.47"), //$NON-NLS-1$
							JOptionPane.QUESTION_MESSAGE);
				} catch (Exception e) {
					throw new AOException(
							"Error al almacenar la clave de cifrado, la clave quedara sin almacenar");
				}
