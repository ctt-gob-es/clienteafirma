...
final class KeyStoreConfigurationManager {
	 ...
	 */
	KeyStoreConfigurationManager(final AOKeyStore keyStore,
	...
	private KeyStoreConfigurationManager(AOKeyStore keyStore) {
		this.ks = this.defaultKeyStore = (keyStore != null ? keyStore
				: getDefaultKeyStore());
	 ...
	 */
	private void initKeyStore(final String path, final String password)
		...
		this.ksManager = AOKeyStoreManagerFactory.getAOKeyStoreManager(this.ks,
				this.ksPath, null, this.getKeystorePasswordCallback(),
				this.parent);
	}
	 ...
	 * seleccionado,...) es recomendable utilizar previamente
	 * {@link #initialize()}.
	 * 
	 * @param keyStore
	...
	void changeKeyStore(final AOKeyStore keyStore) {
		...
		if (keyStore == null) {
			throw new NullPointerException(
					"Es obligatorio asignar un almacen de claves"); //$NON-NLS-1$
		}
	...
	void selectCertificate() throws AOCancelledOperationException,
			AOCertificateException, AOCertificateKeyException,
			AOKeyStoreManagerException, AOCertificatesNotFoundException,
			AOKeystoreAlternativeException {
	 ...
	 *             almac&eacute;n.
	 */
	private void selectCertificate(final boolean checkPrivateKey)
			throws AOCancelledOperationException, AOCertificateException,
			AOCertificateKeyException, AOKeyStoreManagerException,
			AOCertificatesNotFoundException, AOKeystoreAlternativeException {
		...
		if (ksManager == null) {
			try {
				...
				throw new AOKeyStoreManagerException(
						"No se ha podido inicializar el almacen de certificados", e); //$NON-NLS-1$
			}
		}

		if (selectedAlias == null) {
			try {
				selectedAlias = this.showCertSelectionDialog(
						ksManager.getAliases(), checkPrivateKey);
				...
				throw new AOCertificateException(
						"Error al seleccionar un certificado del repositorio", e); //$NON-NLS-1$
			}
		}
		...
		// referencia a esta
		if (checkPrivateKey) {
			this.ke = this.ksManager.getKeyEntry(this.selectedAlias,
					this.getCertificatePasswordCallback());
		}
	}

	/**
	 * Recupera el certificado con el alias indicado del repositorio actual. Si
	 * no existe ning&uacute;n certificado con ese alias, se devolver&aacute;
	 * {@code null}.
	 * 
	 * @param alias
	 ...
	 *             certificados.
	 */
	Certificate getCertificate(final String alias)
			throws AOKeyStoreManagerException, AOKeystoreAlternativeException {
		if (ksManager == null) {
			try {
				this.initKeyStore(this.ksPath, this.ksPassword);
			} catch (final AOCancelledOperationException e) {
	 ...
	 *             certificados.
	 */
	String[] getArrayCertificateAlias() throws AOKeyStoreManagerException,
			AOKeystoreAlternativeException {
		if (ksManager == null) {
			try {
				this.initKeyStore(this.ksPath, this.ksPassword);
	 ...
	 */
	AOKeyStoreManager getKeyStoreManager() throws AOKeyStoreManagerException,
			AOKeystoreAlternativeException {
		if (ksManager == null) {
			try {
				this.initKeyStore(this.ksPath, this.ksPassword);
	...
	private String showCertSelectionDialog(final String[] certAlias,
			final boolean checkPrivateKey)
			throws AOCertificatesNotFoundException,
			AOCancelledOperationException {
		final String[] optionAlias = this.filtCert(certAlias,
				this.filterConfig.getNameFilter());
		...
		return AOUIManager
				.showCertSelectionDialog(
						optionAlias, // Aliases
						(this.ksManager == null) ? null : this.ksManager
								.getKeyStores(), // KeyStores
	 ...
	 */
	@Deprecated
	private String[] filtCert(final String[] aliasList,
			final es.gob.afirma.cliente.AOCertFilter certFilter) {
		...
		final X509Certificate[] acceptedCerts = certFilter.filter(currentCerts
				.keySet().toArray(new X509Certificate[0]));
	...
	private PasswordCallback getCertificatePasswordCallback() {
		return this.ksPassword == null ? AOCryptoUtil.getCertificatePC(this.ks,
				this.parent) : new CachePasswordCallback(
				this.ksPassword.toCharArray());
	}
	...
	private PasswordCallback getKeystorePasswordCallback() {
		return this.ksPassword == null ? AOCryptoUtil.getPreferredPCB(this.ks,
				this.parent) : new CachePasswordCallback(
				this.ksPassword.toCharArray());
	}
	...
	X509Certificate getSelectedCertificate() {
		...
		throw new UnsupportedOperationException(
				"No se puede recuperar el Certificado X509");
	}
	...
	void setSelectedAlias(String selectedAlias) {
		if (this.selectedAlias != null
				&& !this.selectedAlias.equals(selectedAlias)) {
			this.ke = null;
		}
	...
	void setFilterByName(String nameFilter) {
		this.filterConfig.setNameFilter(new es.gob.afirma.cliente.AOCertFilter(
				nameFilter));
	}
	...
	void setShowExpiratedCertificates(boolean showExpiratedCertificates) {
		this.filterConfig
				.setShowExpiratedCertificates(showExpiratedCertificates);
	}
