...
class AFirmaWebSignHTMLDocument extends HTMLDocument {
	...
	class CustomReader extends HTMLDocument.HTMLReader {
		...
		public void handleText(final char[] data, final int pos) {
			Logger.getLogger("es.gob.afirma").info(
					"Posicion del lector: " + new String(data));
			...
			if (t.toString().toLowerCase().equals("afirma")) {
				if (a.isDefined(HTML.Attribute.ENDTAG)) {
					...
					if (type != null && type.equals("file")) {
						...
						try {
							AFirmaWebSignHTMLDocument.files.add(new Attachment(
		...
		private void startFile(final String uri) {
			try {
				AFirmaWebSignHTMLDocument.files.add(new Attachment(URLDecoder
						.decode(uri, "UTF-8")));
