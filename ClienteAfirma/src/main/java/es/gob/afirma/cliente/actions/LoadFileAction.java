 ...
 */
public final class LoadFileAction extends
	...
	public Boolean run() {
		...
		try {
			...
			this.setError(
					"El fichero '" + uri.toASCIIString() + "' no existe", e); //$NON-NLS-1$ //$NON-NLS-2$
			return false;
		} catch (final Exception e) {
			this.setError(
					"No se pudo acceder al fichero '" + uri.toASCIIString() + "'", e); //$NON-NLS-1$ //$NON-NLS-2$
			return false;
		} finally {
			if (is != null) {
				try {
