...
public final class AOCMSEnveloper {
	 ...
	 *             soportado.
	 */
	byte[] createCMSDigestedData(byte[] content) throws IOException,
			NoSuchAlgorithmException {

		return new CMSDigestedData().genDigestedData(content,
				this.signatureAlgorithm, DATA_TYPE_OID);
	}
	 ...
	 *             soportado.
	 */
	byte[] createCMSEncryptedData(byte[] content, AOCipherConfig cipherConfig,
			Key key) throws NoSuchAlgorithmException {

		return new CMSEncryptedData().genEncryptedData(content,
				signatureAlgorithm, cipherConfig, key, DATA_TYPE_OID, uattrib);
	}
	...
	public byte[] createCMSEnvelopedData(final byte[] content,
			final PrivateKeyEntry ke, final AOCipherConfig cipherConfig,
		...
		if (ke != null) {
			return new CMSEnvelopedData().genEnvelopedData(this
					.createContentSignerParementers(content, ke,
							signatureAlgorithm), cipherConfig, recipientsCerts,
					DATA_TYPE_OID, uattrib);
		}

		// Si no se establecio remitente
		return new CMSEnvelopedData().genEnvelopedData(content,
				signatureAlgorithm, cipherConfig, recipientsCerts,
				DATA_TYPE_OID, uattrib);
	}
	...
	public byte[] createCMSSignedAndEnvelopedData(final byte[] content,
		...
		return new CMSSignedAndEnvelopedData().genSignedAndEnvelopedData(
				this.createContentSignerParementers(content, ke,
						signatureAlgorithm), cipherConfig, recipientsCerts,
				DATA_TYPE_OID, ke, attrib, uattrib);
	}
	 ...
	 *             Cuando el certificado del remitente no es v&aacute;lido.
	 */
	byte[] createCMSAuthenticatedData(byte[] content, PrivateKeyEntry ke,
			AOCipherConfig cipherConfig, X509Certificate[] recipientsCerts)
			throws CertificateEncodingException, NoSuchAlgorithmException,
			IOException {
		return new CMSAuthenticatedData().genAuthenticatedData(
				this.createContentSignerParementers(content, ke,
						signatureAlgorithm), // ContentSignerParameters
	...
	public byte[] createCMSAuthenticatedEnvelopedData(byte[] content,
			PrivateKeyEntry ke, AOCipherConfig cipherConfig,
			X509Certificate[] recipientsCerts)
			throws CertificateEncodingException, NoSuchAlgorithmException,
			IOException {
		return new CMSAuthenticatedEnvelopedData()
				.genAuthenticatedEnvelopedData(this
						.createContentSignerParementers(content, ke,
								signatureAlgorithm), // ContentSignerParameters
	 ...
	 */
	private P7ContentSignerParameters createContentSignerParementers(
			final byte[] content, final PrivateKeyEntry ke,
	 ...
	 */
	byte[] addOriginator(byte[] envelop, PrivateKeyEntry ke)
			throws AOException, AOInvalidFormatException {
		...
		if (validator.isCMSEnvelopedData(envelop)) {
			...
			throw new AOInvalidFormatException(
					"Los datos proporcionado no son un envoltorio que soporte multiples remitentes");
		}
	 ...
	 */
	private byte[] addOriginator(final byte[] envelop,
			final String contentInfo, final PrivateKeyEntry ke)
			throws AOException {
		...
		if (contentInfo.equals(AOConstants.CMS_CONTENTTYPE_ENVELOPEDDATA)) {
			...
			newEnvelop = enveloper.addOriginatorInfo(envelop,
					originatorCertChain);
		} else if (contentInfo
				.equals(AOConstants.CMS_CONTENTTYPE_SIGNEDANDENVELOPEDDATA)) {
			newEnvelop = new AOCMSSigner().cosign(envelop,
					AOConstants.DEFAULT_SIGN_ALGO, ke, null);
		} else if (contentInfo
				.equals(AOConstants.CMS_CONTENTTYPE_AUTHENVELOPEDDATA)) {
		...
		if (newEnvelop == null) {
			throw new AOException(
					"Error al agregar el nuevo remitente al envoltorio");
		}
	...
	public void setSignatureAlgorithm(final String algorithm) {
		this.signatureAlgorithm = (algorithm == null ? AOConstants.DEFAULT_SIGN_ALGO
				: algorithm);
	}
	...
	byte[] recoverData(byte[] cmsEnvelop) throws AOInvalidRecipientException,
			InvalidKeyException, CertificateEncodingException, IOException,
			AOInvalidFormatException, AOException {

		final org.bouncycastle.asn1.ASN1InputStream is = new org.bouncycastle.asn1.ASN1InputStream(
				cmsEnvelop);
		...
		final org.bouncycastle.asn1.DERObjectIdentifier doi = (org.bouncycastle.asn1.DERObjectIdentifier) objects
				.nextElement();
		...
		if (doi.equals(org.bouncycastle.asn1.pkcs.PKCSObjectIdentifiers.data)) {
			Logger.getLogger("es.gob.afirma")
					.warning(
							"La extraccion de datos de los envoltorios CMS Data no esta implementada");
			datos = null;
			// datos = this.recoverCMSEncryptedData(cmsEnvelop, cipherKey);
		} else if (doi
				.equals(org.bouncycastle.asn1.pkcs.PKCSObjectIdentifiers.digestedData)) {
			Logger.getLogger("es.gob.afirma")
					.warning(
							"La extraccion de datos de los envoltorios CMS DigestedData no esta implementada");
			datos = null;
			// datos = this.recoverCMSEncryptedData(cmsEnvelop, cipherKey);
		} else if (doi
				.equals(org.bouncycastle.asn1.cms.CMSObjectIdentifiers.compressedData)) {
			datos = this.recoverCMSCompressedData(cmsEnvelop);
		} else if (doi
				.equals(org.bouncycastle.asn1.pkcs.PKCSObjectIdentifiers.encryptedData)) {
			datos = this.recoverCMSEncryptedData(cmsEnvelop, cipherKey);
		} else if (doi
				.equals(org.bouncycastle.asn1.pkcs.PKCSObjectIdentifiers.envelopedData)) {
			datos = this.recoverCMSEnvelopedData(cmsEnvelop, configuredKe);
		} else if (doi
				.equals(org.bouncycastle.asn1.cms.CMSObjectIdentifiers.authEnvelopedData)) {
			datos = this.recoverCMSAuthenticatedEnvelopedData(cmsEnvelop,
					configuredKe);
		} else if (doi
				.equals(org.bouncycastle.asn1.cms.CMSObjectIdentifiers.authenticatedData)) {
			datos = this.recoverCMSAuthenticatedData(cmsEnvelop, configuredKe);
		} else if (doi
				.equals(org.bouncycastle.asn1.cms.CMSObjectIdentifiers.signedAndEnvelopedData)) {
			datos = this
					.recoverCMSSignedEnvelopedData(cmsEnvelop, configuredKe);
		} else {
			throw new AOInvalidFormatException(
	 ...
	 */
	byte[] recoverCMSEncryptedData(byte[] encryptedData, String passkey)
			throws InvalidKeyException, AOException {
		return new CMSDecipherEncryptedData().dechiperEncryptedData(
				encryptedData, passkey);
	}
	 ...
	 */
	byte[] recoverCMSEnvelopedData(final byte[] envelopedData,
		...
		return new CMSDecipherEnvelopData().dechiperEnvelopData(envelopedData,
				ke);
	}
	...
	byte[] recoverCMSSignedEnvelopedData(final byte[] signedEnvelopedData,
			...
			CertificateEncodingException, AOException,
			AOInvalidRecipientException, InvalidKeyException {
		return new CMSDecipherSignedAndEnvelopedData()
				.dechiperSignedAndEnvelopData(signedEnvelopedData, ke);
	}
	 ...
	 *             Cuando la clave almacenada en el sobre no es v&aacute;lida.
	 */
	byte[] recoverCMSAuthenticatedData(final byte[] authenticatedData,
			final PrivateKeyEntry ke) throws IOException,
			CertificateEncodingException, AOException, InvalidKeyException {
		return new CMSDecipherAuthenticatedData().decipherAuthenticatedData(
				authenticatedData, ke);
	}
	 ...
	 *             Cuando la clave almacenada en el sobre no es v&aacute;lida.
	 */
	byte[] recoverCMSAuthenticatedEnvelopedData(
			final byte[] authenticatedEnvelopedData, final PrivateKeyEntry ke)
			throws IOException, CertificateEncodingException, AOException,
			AOInvalidRecipientException, InvalidKeyException {
		return new CMSDecipherAuthenticatedEnvelopedData()
				.dechiperAuthenticatedEnvelopedData(authenticatedEnvelopedData,
						ke);
	}
