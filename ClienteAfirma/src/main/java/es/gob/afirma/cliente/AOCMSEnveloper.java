...
/**
 * Manejador para la generaci&oacute;n de envoltorios CMS/PKCS#7. Los tipos de envoltorios
 * aceptados son:
 * <ul>
 * <li>Data</li>
 * <li>Digested Data</li>
...
public final class AOCMSEnveloper {
	...
	/** Tipo de los datos contenidos en la envoltura. Siempre data por est&aacute;ndar. */
	...
	/** Atributos que no requieren firma y se desean agregar a todos los envoltorios que los soporten. */
	...
	/** Clave para el descifrado de los datos de un envoltorio EncryptedData. Si se
	 * utiliza un algoritmo PBE de cifrado, ser&aacute; una contrase&ntilda;a en texto
	 * plano. Si es otro algoritmo ser&aacute; su clave en base 64. */
	...
	byte[] createCMSDigestedData(byte[] content) throws IOException, NoSuchAlgorithmException {
		...
		return new CMSDigestedData().genDigestedData(
				content,
	...
	byte[] createCMSEncryptedData(byte[] content, AOCipherConfig cipherConfig, Key key) throws NoSuchAlgorithmException {
		...
		return new CMSEncryptedData().genEncryptedData(
				content, signatureAlgorithm, cipherConfig, key, DATA_TYPE_OID, uattrib);
	}
	
	/**
	 * Crea un envoltorio CMS de tipo EnvelopedData.
	 * @param content Contenido que se desea ensobrar.
	 * @param ke Clave privada del remitente (s&oacute;lo si se quiere indicar remitente).
	...
	public byte[] createCMSEnvelopedData(final byte[] content, 
			                             ...
			                             final X509Certificate[] recipientsCerts) throws NoSuchAlgorithmException, CertificateEncodingException, IOException {
		
		// Si se establecion un remitente
		if (ke != null) {
			return new CMSEnvelopedData().genEnvelopedData(
					this.createContentSignerParementers(content, ke, signatureAlgorithm),
					cipherConfig,
					recipientsCerts,
					DATA_TYPE_OID,
					uattrib);
	...
	public byte[] createCMSSignedAndEnvelopedData (final byte[] content, 
			                                       final PrivateKeyEntry ke, 
			                                       final AOCipherConfig cipherConfig, 
			                                       final X509Certificate[] recipientsCerts) throws CertificateEncodingException, NoSuchAlgorithmException, IOException {
	 ...
	 * @throws CertificateEncodingException Cuando el certificado del remitente no es v&aacute;lido.
	 */
	byte[] createCMSAuthenticatedData (byte[] content, PrivateKeyEntry ke, AOCipherConfig cipherConfig, X509Certificate[] recipientsCerts) throws CertificateEncodingException, NoSuchAlgorithmException, IOException {
		return new CMSAuthenticatedData().genAuthenticatedData(
				this.createContentSignerParementers(content, ke, signatureAlgorithm),	 // ContentSignerParameters
	 ...
	 * @throws CertificateEncodingException Cuando el certificado del remitente no es v&aacute;lido.
	 */
	public byte[] createCMSAuthenticatedEnvelopedData (byte[] content, PrivateKeyEntry ke, AOCipherConfig cipherConfig, X509Certificate[] recipientsCerts) throws CertificateEncodingException, NoSuchAlgorithmException, IOException {
	 ...
	 */
	private P7ContentSignerParameters createContentSignerParementers(final byte[] content, 
			                                                         final PrivateKeyEntry ke, 
		...
		return new P7ContentSignerParameters(
			content,
			digestAlgorithm,
			xCerts
		);
	...
	/**
	 * Algoritmo de firma que se utilizar&aacute; internamente en el sobre. El algoritmo de huella
	 ...
	 * @param originatorKe Clave del remitente.
	 */
	...
	/**
	 * Establece la contrase&ntilde;a o clave para la encriptaci&oacute;n de los datos.
	 * @param keyPass Clave en base 64 o contrase&ntilda;a de cifrado.
	 */
	...
	/**
	 * Recupera el algoritmo de firma configurado. El algoritmo de huella digital se tomar&aacute;
	 * de este.
	...
	/**
	 * Recupera la clave o contrascontrase&ntilde;a para la encriptaci&oacute;n de los datos.
	 ...
	 */
	byte[] recoverData(byte[] cmsEnvelop) throws AOInvalidRecipientException, InvalidKeyException, CertificateEncodingException, IOException, AOInvalidFormatException, AOException {
		
		final org.bouncycastle.asn1.ASN1InputStream is
			= new org.bouncycastle.asn1.ASN1InputStream(cmsEnvelop);
		...
		final org.bouncycastle.asn1.DERObjectIdentifier doi =
			(org.bouncycastle.asn1.DERObjectIdentifier) objects.nextElement();
...
//			datos = this.recoverCMSEncryptedData(cmsEnvelop, cipherKey);
	 ...
	 * @param encryptedData Envoltorio CMS de tipo EncryptedData.
	 * @param passkey Contrase&ntilde;a o clave (base64) necesaria para desencriptar los datos.
	 * @return Contenido del envoltorio.
	 ...
	 * @param envelopedData Envoltorio CMS de tipo EnvelopedData.
	 ...
	 * @param signedEnvelopedData Envoltorio CMS de tipo SignedEnvelopedData.
	 * @param ke Clave de un destinatario del sobre.
	 * @return Contenido del envoltorio.
	 * @throws IOException Si ocurre alg&uacute;n problema leyendo o escribiendo los datos
     * @throws CertificateEncodingException Si se produce alguna excepci&oacute;n con los certificados de firma.
     ...
     * @throws InvalidKeyException Cuando la clave almacenada en el sobre no es v&aacute;lida.
	 */
	byte[] recoverCMSSignedEnvelopedData(final byte[] signedEnvelopedData, 
	...
	/**
	 * Comprueba la integridad de un envoltorio AuthenticatedData y, si es correcto, extrae su contenido.
	 * @param authenticatedData Envoltorio CMS de tipo AuthenticatedData.
	 * @param ke Clave de un destinatario del sobre.
	 * @return Contenido del envoltorio.
	 * @throws IOException Si ocurre alg&uacute;n problema leyendo o escribiendo los datos
	 * @throws CertificateEncodingException Si el certificado proporcionado no se adec&uacute;a a la norma X.509v3
     * @throws AOException Cuando ocurre un error durante el proceso de extracci&oacute;n.
     * @throws InvalidKeyException Cuando la clave almacenada en el sobre no es v&aacute;lida.
     */
	 ...
	 * @param authenticatedEnvelopedData Envoltorio CMS de tipo AuthenticatedEnvelopedData.
	 * @param ke Clave de un destinatario del sobre.
	 * @return Contenido del envoltorio.
	 * @throws IOException Si ocurre algun error gen&eacute;rico de entrada/salida
	 * @throws IOException Si ocurre alg&uacute;n problema leyendo o escribiendo los datos
     * @throws CertificateEncodingException Si se produce alguna excepci&oacute;n con los certificados de firma.
     * @throws AOException Cuando ocurre un error durante el proceso de descifrado (formato o clave incorrecto,...)
     * @throws AOInvalidRecipientException Cuando se indica un certificado que no est&aacute; entre los destinatarios del sobre.
	...
	/**
	 * M&eacute;todo que comprueba que unos datos se corresponden con una estructura CMS/PKCS#7.
	 * Se realiza la verificaci&ocute;n sobre los los siguientes tipos reconocidos:
	 * <ul>
	 * <li>Data</li>
	 * <li>Signed Data</li>
	 * <li>Digested Data</li>
	 ...
	 * <li>Authenticated and Enveloped Data</li>
	 * </ul>
	 * @param cmsData  Datos que deseamos comprobar.
	...
	/**
	 * M&eacute;todo que comprueba que unos datos se corresponden con una estructura
	 * CMS/PKCS#7 concreta.
	 * @param data Datos que deseamos comprobar.
	 * @param type Tipo de contenido del envoltorio que queremos comprobar.
	 * @return Indica los datos son una envoltura CMS con el tipo de contenido indicado.
	 */
