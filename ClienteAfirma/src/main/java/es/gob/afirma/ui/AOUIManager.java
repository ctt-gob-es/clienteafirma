...
public final class AOUIManager {
	...
	public final static String showCertSelectionDialog(final String[] alias,
			final Vector<KeyStore> kss, final Boolean[] keyUsageFilter,
			final Component parentComponent, final boolean checkPrivateKeys,
			final boolean checkValidity, final boolean showExpiredCertificates)
			throws AOCancelledOperationException,
			AOCertificatesNotFoundException {
		return showCertSelectionDialog(alias, kss, keyUsageFilter,
				parentComponent, checkPrivateKeys, checkValidity,
				showExpiredCertificates, null, null, false);
	}
	 ...
	 *            aun no v&aacute;lidos
	 * @param issuerFilter
	 *            Filtro seg&uacute; la RFC2254 para el emisor del certificado
	 * @param subjectFilter
	 *            Filtro seg&uacute; la RFC2254 para el titular del certificado
	...
	public final static String showCertSelectionDialog(final String[] alias,
		...
		if (alias == null || alias.length == 0) {
			throw new AOCertificatesNotFoundException(
					"El almac\u00E9n no conten\u00EDa entradas"); //$NON-NLS-1$
		}

		final Hashtable<String, String> aliassesByFriendlyName = KeyStoreUtilities
				.getAlisasesByFriendlyName(alias, kss, keyUsageFilter,
						checkPrivateKeys, checkValidity,
						showExpiredCertificates, issuerFilter, subjectFilter);

		// Miramos si despues de filtrar las entradas queda alguna o se ha
		// quedado la lista vacia
		if (aliassesByFriendlyName.size() == 0) {
			throw new AOCertificatesNotFoundException(
					"El almacen no contenia entradas validas"); //$NON-NLS-1$
		}
		...
		final String[] finalOrderedAliases = aliassesByFriendlyName.values()
		...
		Object o = JOptionPane.showInputDialog(parentComponent,
				Messages.getString("AOUIManager.13"), //$NON-NLS-1$
				Messages.getString("AOUIManager.14"), //$NON-NLS-1$
				JOptionPane.PLAIN_MESSAGE, null, finalOrderedAliases, null);
		...
		if (o != null)
			...
			throw new AOCancelledOperationException(
					"Operacion de seleccion de certificado cancelada"); //$NON-NLS-1$
		}
					...
					for (KeyStore ks : kss) {
						try {
							if (ks.containsAlias(al)) {
								try {
									cv.checkCertificate(
											new java.security.cert.Certificate[] { ks
													.getCertificate(al) },
											false);
									...
									if (JOptionPane
											.showConfirmDialog(
													...
													cv.getErrorMessage()
															+ Messages
																	.getString("AOUIManager.8"), //$NON-NLS-1$
						...
						if (rejected)
							throw new AOCancelledOperationException(
									"Se ha reusado un certificado probablemente no valido"); //$NON-NLS-1$
					}
				}
				return al;
			}
	...
	public static final String getLoadFileName(final String[] extensions,
			final String description, final Component parentComponent) {
	 ...
	 */
	public final static String selectDirectory(final Component parentComponent,
			final String title) {
		final JFileChooser jfc = new JFileChooser();
	 ...
	 * @version 0.3
	 */
	public final static class ExtFilter extends FileFilter implements
			java.io.FileFilter {
		...
		/**
		 * Construye un filtro para la selecci&oacute;n de ficheros en un
		...
		if (icon == null) {
			try {
				icon = Toolkit.getDefaultToolkit().getImage(
						AOUIManager.class
								.getResource("/resources/afirma_ico.png")); //$NON-NLS-1$
			} catch (Exception e) {
	 ...
	 */
	public final static char[] getPassword(String text, final String charSet,
			final boolean beep, final Component c)
			throws AOCancelledOperationException {
		if (text == null)
		...
		lbText.setMinimumSize(new Dimension(lbText.getFontMetrics(
				lbText.getFont()).stringWidth(text), lbText.getSize().height));
		...
		pane.createDialog(null, Messages.getString("AOUIManager.24"))
				.setVisible(true);
	 ...
	 *             contrase&ntilde;a
	 */
	public final static char[] getPassword(final String text, final Component c)
			throws AOCancelledOperationException {
	...
	public static final FileFilter getFileFilter(final String signFormat) {
		...
		if (signFormat.equals(AOConstants.SIGN_FORMAT_XADES_ENVELOPED)
			...
			return new ExtFilter(
					new String[] { "xml" }, Messages.getString("AOUIManager.28")); //$NON-NLS-1$ //$NON-NLS-2$
		} else if (signFormat.equals(AOConstants.SIGN_FORMAT_PDF)) {
			return new ExtFilter(
					new String[] { "pdf" }, Messages.getString("AOUIManager.30")); //$NON-NLS-1$ //$NON-NLS-2$
		} else if (signFormat.equals(AOConstants.SIGN_FORMAT_OOXML)) {
			return new ExtFilter(
					new String[] { "docx", "pptx", "ppsx" }, Messages.getString("AOUIManager.50")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		} else if (signFormat.equals(AOConstants.SIGN_FORMAT_ODF)) {
			return new ExtFilter(
					new String[] { "odt", "ods", "odp" }, Messages.getString("AOUIManager.16")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
		}
	...
	public static final FileFilter getOutFileFilter(final String signFormat) {
		if (signFormat.equals(AOConstants.SIGN_FORMAT_CMS)) {
			return new ExtFilter(
					new String[] { "csig" }, Messages.getString("AOUIManager.43")); //$NON-NLS-1$ //$NON-NLS-2$
		} else if (signFormat.equals(AOConstants.SIGN_FORMAT_CADES)) {
			return new ExtFilter(
					new String[] { "csig" }, Messages.getString("AOUIManager.1")); //$NON-NLS-1$ //$NON-NLS-2$
		} else if (signFormat.equals(AOConstants.SIGN_FORMAT_PDF)) {
			return new ExtFilter(
					new String[] { "pdf" }, Messages.getString("AOUIManager.30")); //$NON-NLS-1$ //$NON-NLS-2$
		} else if (signFormat.equals(AOConstants.SIGN_FORMAT_ODF)) {
			return new ExtFilter(
					new String[] { "odt", "ods", "odp" }, Messages.getString("AOUIManager.16")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		} else if (signFormat.equals(AOConstants.SIGN_FORMAT_OOXML)) {
			return new ExtFilter(
					new String[] { "docx", "xlsx", "pptx", "ppsx" }, Messages.getString("AOUIManager.50")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				...
				|| signFormat
						.equals(AOConstants.SIGN_FORMAT_XADES_EXTERNALLY_DETACHED)) {
			return new ExtFilter(
					new String[] { "xsig" }, Messages.getString("AOUIManager.17")); //$NON-NLS-1$ //$NON-NLS-2$
		}
		return new ExtFilter(
				new String[] { "sig" }, Messages.getString("AOUIManager.52")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	 ...
	 */
	public static final String getOutFileName(final String inName,
		...
		if (signFormat.equals(AOConstants.SIGN_FORMAT_CMS)
				|| signFormat.equals(AOConstants.SIGN_FORMAT_CADES))
		...
		if (signFormat.equals(AOConstants.SIGN_FORMAT_PDF)
			...
			if (i > 0 && i < inName.length() - 1)
				return inName.substring(0, i)
						+ ".signed" + inName.substring(i).toLowerCase(); //$NON-NLS-1$
		}
	 ...
	 */
	public static final int[] showNodeSignSelectionPane(
			final es.gob.afirma.misc.tree.TreeModel treeModel,
			final Component parentComponent)
			throws AOCancelledOperationException {
		...
		arbolNodos.getSelectionModel().setSelectionMode(
		...
		panel.add(new JLabel(
				"Seleccione los nodos de firma que desee contrafirmar:"), c); //$NON-NLS-1$
		c.weighty = 1.0;
		c.gridy = 1;
		panel.add(spArbolNodos, c);

		do {
			final int action = JOptionPane.showConfirmDialog(parentComponent,
					panel, Messages.getString("AOUIManager.61"), //$NON-NLS-1$
					JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
			if (action != JOptionPane.OK_OPTION) {
				throw new AOCancelledOperationException(
						"La operacion de firma ha sido cancelada por el usuario"); //$NON-NLS-1$
			}

			if (arbolNodos.getSelectionRows() == null
					|| arbolNodos.getSelectionRows().length < 1) {
				JOptionPane
						.showMessageDialog(
								parentComponent,
		...
		} while (arbolNodos.getSelectionRows() == null
				|| arbolNodos.getSelectionRows().length < 1);
	 ...
	 *             Cuando el usuario cancel&oacute; la operaci&oacute;n.
	 */
	public static final String[] showSignersSelectionPane(
			final es.gob.afirma.misc.tree.TreeModel treeModel,
			final Component parentComponent) throws AOException,
		...
		if (tree == null || tree.getRoot() == null
				|| !(tree.getRoot() instanceof DefaultMutableTreeNode)) {
		...
		try {
			for (int i = 0; i < root.getChildCount(); i++)
				getSigners((DefaultMutableTreeNode) root.getChildAt(i),
						signersSet);
		} catch (final Exception e) {
			throw new AOException(
					"El arbol introducido contiene elementos no validos", e); //$NON-NLS-1$
		}
		...
		do {
			final int action = JOptionPane.showConfirmDialog(parentComponent,
					panel, Messages.getString("AOUIManager.66"), //$NON-NLS-1$
			...
			if (jList.getSelectedIndices() == null
					|| jList.getSelectedIndices().length < 1) {
				JOptionPane
						.showMessageDialog(
								parentComponent,
		...
		} while (jList.getSelectedIndices() == null
				|| jList.getSelectedIndices().length < 1);
	 ...
	 */
	private static final void getSigners(final DefaultMutableTreeNode node,
			final Set<String> signersSet) {
	...
	public final static String saveDataToFile(final Component parentComponent,
			final byte[] data, final File selectedFile,
		...
		if (data == null) {
			...
			throw new NullPointerException(
					"No se introdujeron datos que almacenar"); //$NON-NLS-1$
		}
		...
		while (tryAgain) {
			...
			fileChooser.getAccessibleContext().setAccessibleName(
					Messages.getString("AOUIManager.81")); //$NON-NLS-1$
			fileChooser.getAccessibleContext().setAccessibleDescription(
					Messages.getString("AOUIManager.82")); //$NON-NLS-1$
	 ...
	 */
	public static DefaultTreeModel convertToSwingModel(
			final es.gob.afirma.misc.tree.TreeModel treeModel) {
		// Primero hay que obtener el objeto raiz
		es.gob.afirma.misc.tree.TreeNode root = (es.gob.afirma.misc.tree.TreeNode) treeModel
				.getRoot();
		Object rootObject = root.getUserObject();

		// Iniciamos el DefaultTreeModel
		DefaultMutableTreeNode rootSwing = new DefaultMutableTreeNode(
				rootObject);
		DefaultTreeModel swingDefaultTreeModel = new DefaultTreeModel(
				rootSwing, treeModel.asksAllowsChildren());
		...
		while (nParent < parents.size()) {
			...
			for (int i = 0; i < nNodes; i++) {
				...
				// Swing
				swingNode = new DefaultMutableTreeNode(
