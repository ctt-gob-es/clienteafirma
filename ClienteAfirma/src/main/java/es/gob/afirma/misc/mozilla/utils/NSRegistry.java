...
/**
 * Provides simple read-only access to the (binary) Netscape registry file. The
 * registry file, typically C:\WINNT\nsreg.dat on windows or
 * ~/.netscape/registry on Unix, contains configuration information for Netscape
 * applications. The registry is organized like the windows registry: it's tree
 * structured and nodes (called "keys") have a list of name/value pairs called
 * entries.
 * <p>
 * The format of the file doesn't seem to be officially documented however it's
 * not complicated and can be easily derived from the Mozilla sources that
 * provide access to it. See the references below for more information.
 * <p>
 * To use this class you must create an NSRegistry object and open a valid
 * registry file. To look up a value (the value of an "entry") you provide it's
 * path; here's an example that prints the current version of Communicator.
 * 
 * <pre>
 ...
 * 
 * 
 * Inspirada en la clase com.sun.deploy.net.proxy.NSRegistry de Sun
 * Microsystems.
 */
final class NSRegistry {
	 ...
	 * formatted like the REGDESC struct below, however note that the last field
	 * is not part of this record. Each record is exactly 32 bytes long. This
	 ...
	 * source/modules/libreg/src/reg.h </a>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * The REGTYPE_ENTRY constant (0x0010) comes from NSReg.h, see <a href=
	 * "http://lxr.mozilla.org/mozilla/source/modules/libreg/include/NSReg.h">
	 ...
	 * "location" field in the record doesn't match its offset in the file.
	 */
	 ...
	 * Windows) and read its header. Return <code>this</code> if the file was
	 * opened and the header was read successfully, null otherwise. This method
	 * must be called, before calling anything else.
	 * <p>
	 * The layout of the file header is only documented in the Mozilla sources,
	 * see
	 * <p>
	 * <a href="http://lxr.mozilla.org/mozilla/source/modules/libreg/src/reg.h">
	 * source/modules/libreg/src/reg.h </a>
	 * <p>
	 * Here's the header struct itself: typedef struct _hdr { uint32 magic; //
	 * must equal MAGIC_NUMBER uint16 verMajor; // major version number uint16
	 * verMinor; // minor version number REGOFF avail; // next available offset
	 * REGOFF root; // root object } REGHDR;
	 */
	NSRegistry open(File regFile) {
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").severe(
					"Error abriendo el registro de Mozilla, se devolvera null: "
							+ e);
			return null;
		}
	}
	 ...
	 * in parent.
	 */
	private String get(Record parent, StringTokenizer pathElts)
			throws IOException {
