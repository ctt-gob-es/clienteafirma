...
public final class MimeHelper {
	...
	public MimeHelper(final byte[] data) {
		...
		if (data == null)
			throw new NullPointerException(
					"No se han indicado los datos que se desean analizar");
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").warning(
					"Error durante el analisis de la cabecera de los datos: "
							+ e);
		}
	}
	...
	private static void loadOidMimetypeProperties() {
		...
		final InputStream isProp = MimeHelper.class
				.getResourceAsStream("/resources/oids_mimetypes.properties");
		try {
			oidMimetypeProp.load(isProp);
		} catch (final Exception e) {
			Logger.getLogger("es.gob.afirma").warning(
					"No se ha podido cargar la tabla de relaciones Oid-Mimetype: "
							+ e);
		}
		try {
			isProp.close();
	 ...
	 * @param contentType
	 *            Tipo de los datos, en formato MIMEType. Si se proporciona
	 *            <code>null</code> se intenta determinar
	 * @param fileName
	...
	public static String getMimeEncodedAsAttachment(final byte[] data,
			final String contentID, String contentType, final String fileName,
		...
		if (fileName != null) {
			if (!isPureAscii(fileName)) {
				Logger.getLogger("es.gob.afirma")
						.warning(
								"MIME solo soporta nombres de ficheros ASCII, se ignorara el campo 'filename': "
										+ fileName);
			} else if (fileName.length() > 78) {
				Logger.getLogger("es.gob.afirma")
						.warning(
								"No se soporta la codificacion RFC 2184 para los nombres de fichero de mas de 78 caracteres, se ignorara el campo 'filename': "
										+ fileName);
			} else {
				sb.append("; filename=");
	...
	private static boolean isPureAscii(String v) {
		...
		final java.nio.charset.CharsetDecoder d = java.nio.charset.Charset
				.forName("US-ASCII").newDecoder();
		try {
			java.nio.CharBuffer r = d.decode(java.nio.ByteBuffer
					.wrap(bytearray));
			r.toString();
