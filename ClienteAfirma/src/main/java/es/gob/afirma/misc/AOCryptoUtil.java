...
public final class AOCryptoUtil {
	 ...
	 */
	public static byte[] getMessageDigest(final byte[] data,
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").severe(
					"No se encuentra el proveedor '" + PROVIDER + "': " + e);
			return null;
		}
		md.update(data);
		return md.digest();
	 ...
	 * especificado. Si el formato no est&aacute; soportado se devuelve
	 * <code>null</code>.
	 * 
	 * @param format
	 *            Formato de firma.
	...
	public static AOSigner getSigner(final File signFile) {
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").warning(
					"No se pudo leer el fichero '" + signFile.getAbsolutePath()
							+ "': " + e);
			return null;
		}
		try {
	 ...
	 */
	public static PasswordCallback getCertificatePC(
			final AOConstants.AOKeyStore store, final Component parent) {
		...
		if (store == AOConstants.AOKeyStore.WINDOWS
			...
			pssCallback = new UIPasswordCallback(
					"Contrase\u00F1a del certificado", parent);
	 ...
	 * @return Manejador para la solicitud de la clave.
	 */
	public static PasswordCallback getPreferredPCB(
			AOConstants.AOKeyStore kStore, Component parent) {

		if (kStore == null)
			throw new NullPointerException(
					"No se ha indicado el KeyStore del que desea " + //$NON-NLS-1$
		...
		if (kStore == AOConstants.AOKeyStore.WINDOWS
			...
			pssCallback = new UIPasswordCallback(Messages.getString(
					"AOCryptoUtil.0", kStore.getDescription()), parent); //$NON-NLS-1$ 
		}
		return pssCallback;
	}
	...
	public static X509Certificate createCert(final String b64Cert) {
		if (b64Cert == null || "".equals(b64Cert)) {
			Logger.getLogger("es.gob.afirma")
					.severe("Se ha proporcionado una cadena nula o vacia, se devolvera null");
			return null;
		}
		final X509Certificate cert;
		try {
			final InputStream isCert = new ByteArrayInputStream(
					decodeBase64(b64Cert));
			cert = (X509Certificate) CertificateFactory.getInstance("X.509")
					.generateCertificate(isCert);
			try {
				isCert.close();
			} catch (Exception e) {
			}
			...
			Logger.getLogger("es.gob.afirma").severe(
					"No se pudo decodificar el certificado en Base64, se devolvera null: "
							+ e);
			return null;
		}
		return cert;
	...
	public static String getDigestAlgorithmName(final String pseudoName) {
		...
		throw new IllegalArgumentException(
				"Algoritmo de huella digital no soportado: " + pseudoName);
	}
	...
	public static String encodeBase64(byte[] data, boolean chunked) {
		...
		try {
			return StringUtils
					.newStringUtf8(Base64.encodeBase64(data, chunked));
		} catch (final Exception e) {
			Logger.getLogger("es.gob.afirma").severe(
					"No se pudo convertir un binario a base 64, se devolvera null: "
							+ e);
			return null;
		}
	}
