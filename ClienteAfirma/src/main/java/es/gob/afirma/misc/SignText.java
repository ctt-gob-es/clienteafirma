 ...
 * funcionamiento del m&eacute;todo
...
public final class SignText {
	 ...
	 *         <ul>
	 *         <li>
	 *         <code>error:noMatchingCert</code> si el usuario no dispone de
	 *         ning&uacute;n certificado emitido por las CA indicadas.</li>
	 *         <li>
	 *         <code>error:userCancel</code> si el usuario cancela la
	 *         operaci&oacute;n.</li>
	 *         <li>
	 *         <code>error:internalError</code> si ocurre cualquier error
	 ...
	 *            <ul>
	 *            <li>
	 *            <code>"auto"</code> indica que el programa seleccionar&aacute;
	 *            autom&aacute;ticamente un certificado. Si se ha indicado uno o
	 *            m&aacute;s nombre de CA mediante los par&aacute;metros
	 *            <code>caName<i>N</i></code> la selecci&oacute;n
	 *            autom&aacute;tica se limita a los certificado emitidos por las
	 *            CA indicadas.</li>
	 *            <li>
	 *            <code>"ask"</code> indica que se debe solicitar al usuario que
	 *            seleccione un certificado. Si se ha indicado uno o m&aacute;s
	 *            nombre de CA mediante los par&aacute;metros
	 *            <code>caName<i>N</i></code> la selecci&oacute;n
	 *            autom&aacute;tica se limita a los certificado emitidos por las
	 *            CA indicadas.</li>
	 *            </ul>
	 ...
	 */
	public void signText(final String stringToSign, final String caOption,
		...
		if (!("ask".equals(caOption)) && !("auto".equals(caOption))) {
			Logger.getLogger("es.gob.afirma").severe(
					"Valor incorrecto para caOption, debe ser 'ask' u 'auto': "
							+ caOption);
			result = INTERNAL_ERROR;
			return;
		}
		...
		if (caNameN != null && caNameN.length > 0) {
			...
			for (int j = 0; j < caNameN.length; j++) {
				...
				try {
					...
					Logger.getLogger("es.gob.afirma").warning(
							"DN de CA para filtro mal formado ('" + ca
									+ "'), se ignorara: " + e);
					continue;
				}
		...
		Logger.getLogger("es.gob.afirma").info(
				"Se utilizara el siguiente filtro de emisor: " + issuerFilter);

		// Obtenemos la lista de nombres descriptivos de los alias de los
		// certificados
		final Hashtable<String, String> aliasesByFriendlyName;
		try {
			aliasesByFriendlyName = KeyStoreUtilities
					.getAlisasesByFriendlyName(alias, kss.getKeyStores(), null, // keyUsageFilter
							true, // checkPrivateKeys
							true, // checkValidity
							true, // showExpiredCertificates
							issuerFilter, null // subjectFilter
					);
		} catch (final Exception e) {
			Logger.getLogger("es.gob.afirma").severe(
					"Error obteniendo los nombres descriptivos de los alias: "
							+ e);
			result = INTERNAL_ERROR;
			return;
		}
		...
		if ("auto".equals(caOption)) {
			final Hashtable<String, String> tmpHash = new Hashtable<String, String>(
					1);
	...
	private String firmar(final String stringToSign,
			final PrivateKeyEntry keyEntry, final X509Certificate cert) {
		if (stringToSign == null) {
			Logger.getLogger("es.gob.afirma").severe(
					"El texto a firmar no puede ser nulo");
			return INTERNAL_ERROR;
		}
		if (keyEntry == null) {
			Logger.getLogger("es.gob.afirma").severe(
					"La clave privada de firma no puede ser nula");
			return NO_MATCHING_CERT;
		}
		if (cert == null) {
			Logger.getLogger("es.gob.afirma").severe(
					"El certificado de firma no puede ser nulo");
			return NO_MATCHING_CERT;
		}
		try {
			if (!useCAdES)
				return AOCryptoUtil.encodeBase64(
						new AOCMSSigner().sign(stringToSign.getBytes(),
								AOConstants.SIGN_ALGORITHM_SHA1WITHRSA,
								keyEntry, null), false);
			return AOCryptoUtil.encodeBase64(new AOCAdESSigner().sign(
					stringToSign.getBytes(),
					AOConstants.SIGN_ALGORITHM_SHA1WITHRSA, keyEntry, null),
					false);
		} catch (final Exception e) {
			Logger.getLogger("es.gob.afirma").severe(
	...
	/**
	 * Crea un objeto para la firma de Texto simulando el funcionamiento de
	 * <code>crypto.signText()</code>
	 * 
	 * @param aliases
	 ...
	 *            Si de especifica <code>null</code> se utilizara un
	 *            <code>NullPasswordCallback</code>
	 */
	public SignText(final String[] aliases, final AOKeyStoreManager ksm,
			final Component parentComponent, final PasswordCallback pc) {
		alias = aliases;
		kss = ksm;
	...
	private void createUI(final String stringToSign,
			final Hashtable<String, String> aliasesByFriendlyName) {
		...
		final JLabel label1 = new JLabel(
				"Se va a firmar la siguiente informaci\u00F3n:");
		...
		final JLabel label2 = new JLabel(
				"Por favor, seleccione el certificado de firma:");
		...
		if (aliasesByFriendlyName.size() > 1) {
			...
			offset = -25;
			label2.setText("Se utilizar\u00E1 el siguiente certificado: "
					+ aliasesByFriendlyName.keys().nextElement());
		}
		pane.add(label2);

		final JLabel label3 = new JLabel(
				"Si est\u00E1 conforme en firmar este mensaje pulse el bot\u00F3n Aceptar");
		...
		if (JOptionPane.showConfirmDialog(parent, pane,
				"Firma Electr\u00F3nica", JOptionPane.OK_CANCEL_OPTION,
			...
			try {
				...
				for (String al : aliasesByFriendlyName.keySet().toArray(
						new String[aliasesByFriendlyName.size()])) {
				...
				JOptionPane.showMessageDialog(parent,
						"Error durante el proceso de firma", "Error",
