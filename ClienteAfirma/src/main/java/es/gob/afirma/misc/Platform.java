...
public final class Platform {
		...
		try {
			...
			if (osName.contains("indows")) {
				...
				Logger.getLogger("es.gob.afirma").warning(
						"No se ha podido determinar el sistema operativo");
			}
	 ...
	 * 
	 * @return Navegador web desde el que se realiza la ejecuci&oacute;n o
	 *         {@code null} si no se realiza desde un navegador.
	 */
	...
	private final static String recoverJavaHome() {
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").warning(
					"No se ha podido identificar el directorio de java");
		}
	...
	public static String getEndorsedDir() {
		...
		if (endorsedDirsString != null
				&& (endorsedDirs = endorsedDirsString.split(System
						.getProperty("path.separator"))).length > 0) { //$NON-NLS-1$
		...
		if (endorsedDir == null) {
			endorsedDir = getJavaHome() + File.separator
					+ "lib" + File.separator + "endorsed"; //$NON-NLS-1$ //$NON-NLS-2$
		}
