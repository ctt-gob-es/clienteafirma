...
public final class AsynchronousSaveData implements Runnable {
	 ...
	 *            contrario
	 */
	public AsynchronousSaveData(final byte[] data, final String fileName,
			final String desc, final String[] exts, final Frame p,
			final boolean errorDialog) {
		if (data == null || data.length == 0)
			throw new NullPointerException(
					"Los datos a guardar no pueden ser nulos");
		dataToSave = data;
				...
				.doPrivileged(new java.security.PrivilegedAction<Void>() {
					public Void run() {
						if (savingTarget == null || "".equals(savingTarget)) {
							try {
								savingTarget = AOUIManager.getSaveFileName(
										extensions, description, parent);
							} catch (final Exception e) {
								Logger.getLogger("es.gob.afirma").severe(
										"El nombre de fichero para guardar los datos no es valido: "
												+ e);
								return null;
							}
							if (savingTarget == null) {
								Logger.getLogger("es.gob.afirma")
										.severe("No se establecio un nombre de fichero de salida");
								return null;
							}
						}
						...
						try {
							...
							Logger.getLogger("es.gob.afirma").severe(
									"No se pudieron almacenar los datos en disco: "
											+ e);
