...
public final class CMSAuthenticatedEnvelopedData {
	...
	public byte[] genAuthenticatedEnvelopedData(
			...
			String autenticationAlgorithm, AOCipherConfig config,
			X509Certificate[] certDest, Oid dataType, boolean applySigningTime,
			Map<Oid, byte[]> atrib, Map<Oid, byte[]> uatrib)
		...
		// 5. MAC
		byte[] mac = Utils.genMac(autenticationAlgorithm,
				genPack(authAttr.getDEREncoded(), parameters.getContent()),
				cipherKey);
		...
		return new ContentInfo(PKCSObjectIdentifiers.id_ct_authEnvelopedData,
	 ...
	 */
	private ASN1Set generateSignedAtt(final Oid datatype,
			final boolean signingTime, final Map<Oid, byte[]> atrib) {
	...
	public byte[] addOriginatorInfo(byte[] data,
		...
		try {
			...
			if (doi.equals(PKCSObjectIdentifiers.id_ct_authEnvelopedData)) {
				...
				AuthEnvelopedData authEnv = new AuthEnvelopedData(
				...
				retorno = new ContentInfo(
						...
						new AuthEnvelopedData(
								...
								authEnv.getAuthAttrs(), authEnv.getMac(),
