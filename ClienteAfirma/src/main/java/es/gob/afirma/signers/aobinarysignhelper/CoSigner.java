...
public final class CoSigner {
	...
	public byte[] coSigner(final P7ContentSignerParameters parameters,
			final byte[] sign, final boolean omitContent, final Oid dataType,
			final PrivateKeyEntry keyEntry, final Map<Oid, byte[]> atrib,
			final Map<Oid, byte[]> uatrib, final byte[] messageDigest)
		...
		if (messageDigest == null)
			signedAttr = generateSignerInfo(signerCertificateChain[0],
					digestAlgorithm, parameters.getContent(), dataType, atrib);
		else
			signedAttr = generateSignerInfoFromHash(signerCertificateChain[0],
					digestAlgorithm, messageDigest, dataType, atrib);
		...
		signerInfos.add(new SignerInfo(identifier, digAlgId, signedAttr,
				encAlgId, sign2, unSignedAttr// null //unsignedAttr
				));
	...
	public byte[] coSigner(String signatureAlgorithm,
			X509Certificate[] signerCertificateChain, byte[] sign,
			Oid dataType, PrivateKeyEntry keyEntry, Map<Oid, byte[]> atrib,
			Map<Oid, byte[]> uatrib, byte[] messageDigest) throws IOException,
		...
		if (contenidoDatos != null) {
			signedAttr = generateSignerInfo(signerCertificateChain[0],
					digestAlgorithm, contenidoDatos, dataType, atrib);
	...
	private ASN1Set generateSignerInfoFromHash(X509Certificate cert,
		...
		// MessageDigest
		ContexExpecific.add(new Attribute(CMSAttributes.messageDigest,
				new DERSet(new DEROctetString(datos))));
