...
public final class CadesCoSigner {
	 ...
	 */
	public byte[] coSigner(P7ContentSignerParameters parameters, byte[] sign,
			boolean omitContent, String policy, Oid qualifier,
			boolean signingCertificateV2, Oid dataType,
			PrivateKeyEntry keyEntry, byte[] messageDigest) throws IOException,
		...
		if (omitContent == false) {
			...
			try {
				...
				throw new IOException(
						"Error en la escritura del procesable CMS: " + ex);
			}
		...
		if (messageDigest == null) {
			ASN1EncodableVector contextExpecific = Utils.generateSignerInfo(
					signerCertificateChain[0], digestAlgorithmId,
					digestAlgorithm, digAlgId, parameters.getContent(), policy,
					qualifier, signingCertificateV2, dataType, null);
			...
			ASN1EncodableVector contextExpecific = Utils.generateSignerInfo(
					signerCertificateChain[0], digestAlgorithmId,
					digestAlgorithm, digAlgId, null, policy, qualifier,
		...
		return new ContentInfo(PKCSObjectIdentifiers.signedData,
				new SignedData(sd.getDigestAlgorithms(), encInfo, certificates,
						certrevlist, new DERSet(signerInfos)// unsignedAttr
				)).getDEREncoded();
	 ...
	 * @throws javax.security.cert.CertificateException
	 */
	public byte[] coSigner(String signatureAlgorithm,
			X509Certificate[] signerCertificateChain, InputStream data,
			String policy, Oid qualifier, boolean signingCertificateV2,
			Oid dataType, PrivateKeyEntry keyEntry, byte[] messageDigest)
		...
		if (contenido != null) {
			contenidoDatos = AOUtil.getDataFromInputStream(contenido
					.getOctetStream());
		}
		...
		for (int i = 0; i < signerInfosSd.size(); i++) {
			...
			if (algHash.getAlgorithm().toString()
					.equals(digestAlgorithmId.getOID().toString())) {
				ASN1Set signedAttrib = si.getAuthenticatedAttributes();
				for (int s = 0; s < signedAttrib.size(); s++) {
					ASN1Sequence elemento = (ASN1Sequence) signedAttrib
							.getObjectAt(s);
					DERObjectIdentifier oids = (DERObjectIdentifier) elemento
							.getObjectAt(0);
					if (CMSAttributes.messageDigest.getId().toString()
							.equals(oids.toString())) {
						DERSet derSetHash = (DERSet) elemento.getObjectAt(1);
						DEROctetString derHash = (DEROctetString) derSetHash
								.getObjectAt(0);
		...
		if (contenidoDatos != null) {
			ASN1EncodableVector contextExpecific = Utils.generateSignerInfo(
					signerCertificateChain[0], digestAlgorithmId,
					digestAlgorithm, digAlgId, contenidoDatos, policy,
					qualifier, signingCertificateV2, dataType, null);
			signedAttr2 = getAttributeSet(new AttributeTable(contextExpecific));
			signedAttr = getAttributeSet(new AttributeTable(contextExpecific));
			...
			ASN1EncodableVector contextExpecific = Utils.generateSignerInfo(
					signerCertificateChain[0], digestAlgorithmId,
					digestAlgorithm, digAlgId, null, policy, qualifier,
					signingCertificateV2, dataType, messageDigest);
			signedAttr2 = getAttributeSet(new AttributeTable(contextExpecific));
			signedAttr = getAttributeSet(new AttributeTable(contextExpecific));
		} else {
