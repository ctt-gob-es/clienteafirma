...
public final class ObtainContentSignedData {
	 ...
	 * tanto en CADES como en CMS. Si la firma no contiene los datos, devuelve
	 * <code>null</code>.
	 * 
	 * @param data
			...
			if (doi.equals(PKCSObjectIdentifiers.signedData)) {
				...
				if (ci.getContent() != null) {
					...
					Logger.getLogger("es.gob.afirma").warning(
							"No existe contenido en esta firma.");
				}
			} else {
				Logger.getLogger("es.gob.afirma").warning(
						"No se puede obtener el contenido de esta firma.");
			}

		} catch (Exception e) {
			Logger.getLogger("es.gob.afirma").severe(
					"No se pudieron recuperar los datos contenidos en la firma: "
							+ e);
			contenido = null;
		}
