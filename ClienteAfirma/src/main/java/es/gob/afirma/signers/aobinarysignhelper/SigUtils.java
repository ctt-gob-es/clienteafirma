...
final class SigUtils {
	 ...
	 */
	static AlgorithmIdentifier makeAlgId(String oid, byte[] params)
			throws IOException {
		if (params != null)
	...
	static DERObject makeObj(byte[] encoding) throws IOException {
		if (encoding == null) {
			Logger.getLogger("es.gob.afirma").warning(
					"La codificacion era nula, se devolvera null");
			return null;
		}
	...
	static ASN1Set getAttributeSet(AttributeTable attr) {
		...
		Logger.getLogger("es.gob.afirma").warning(
				"Los atributos eran nulos, se devolvera null");
		return null;
	}
	 ...
	 * @return Un SET de ASN1 con los elementos de la lista introducida.
	 */
	static ASN1Set fillRestCerts(final List<DEREncodable> derObjects,
	 ...
	 */
	static byte[] signData(final InputStream file, final String algorithm,
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").warning(
					"Error al cerrar el fichero de datos, el proceso de firma continuara: "
							+ e);
		}
		byte[] realSig;
		try {
