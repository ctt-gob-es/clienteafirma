...
public final class AOCMSSigner implements AOSigner {
	...
	public byte[] sign(final byte[] data, String algorithm,
		...
		if (this.dataType == null) {
			try {
				...
				Logger.getLogger("es.gob.afirma").severe(
						"Error al asignar el OID por defecto: " + ex);
			}
		}
		...
		try {
			boolean omitContent = mode.equals(AOConstants.SIGN_MODE_EXPLICIT)
			...
			return new GenSignedData().generateSignedData(csp, omitContent,
					Boolean.parseBoolean(extraParams.getProperty(
							"applySystemDate", "true")), dataType, keyEntry,
					atrib, uatrib, messageDigest);
	...
	public byte[] cosign(final byte[] data, final byte[] sign,
		...
		final boolean omitContent = mode.equals(AOConstants.SIGN_MODE_EXPLICIT)
				|| precalculatedDigest != null;

		// Si la firma que nos introducen es SignedData
		final boolean signedData = CMSHelper.isCMSValid(sign,
				AOConstants.CMS_CONTENTTYPE_SIGNEDDATA);
		if (signedData) {
			try {
				return new CoSigner().coSigner(csp, sign, omitContent,
						dataType, keyEntry, atrib, uatrib, messageDigest);
			} catch (final Exception e) {
		...
		try {
			...
			// envelopedData.
			return new CoSignerEnveloped().coSigner(csp, sign, dataType,
					keyEntry, atrib, uatrib, messageDigest);
		} catch (final Exception e) {
			throw new AOException("Error generando la Cofirma PKCS#7", e);
		}
	}
	...
	public byte[] cosign(final byte[] sign, String algorithm,
		...
		if (CMSHelper.isCMSValid(sign, AOConstants.CMS_CONTENTTYPE_SIGNEDDATA)) {
			...
			try {
				return new CoSigner().coSigner(typeAlgorithm, aCertificados,
						sign, dataType, keyEntry, atrib, uatrib, null // null
																		// porque
																		// no
																		// nos
																		// pueden
		...
		// Cofirma de la firma usando unicamente el fichero de firmas.
		try {
			return new CoSignerEnveloped().coSigner(typeAlgorithm,
					aCertificados, sign, dataType, keyEntry, atrib, uatrib,
	...
	public byte[] countersign(final byte[] sign, String algorithm,
		...
		if (CMSHelper.isCMSValid(sign, AOConstants.CMS_CONTENTTYPE_SIGNEDDATA)) {
			try {
				...
				if (targetType == CounterSignTarget.Tree) {
					int[] nodes = { 0 };
					dataSigned = new CounterSigner().counterSigner(csp, sign,
							CounterSignTarget.Tree, nodes, keyEntry, dataType,
							atrib, uatrib);
				}
				// CASO DE FIRMA DE HOJAS
					...
					int[] nodes = { 0 };
					dataSigned = new CounterSigner().counterSigner(csp, sign,
							CounterSignTarget.Leafs, nodes, keyEntry, dataType,
							atrib, uatrib);
				}
				// CASO DE FIRMA DE NODOS
					...
					dataSigned = new CounterSigner().counterSigner(csp, sign,
							CounterSignTarget.Nodes, nodesID, keyEntry,
							dataType, atrib, uatrib);
				}
				// CASO DE FIRMA DE NODOS DE UNO O VARIOS FIRMANTES
					...
					dataSigned = new CounterSigner().counterSigner(csp, sign,
							CounterSignTarget.Signers, nodes2, keyEntry,
							dataType, atrib, uatrib);

				}
			} catch (final Exception e) {
				throw new AOException("Error generando la Contrafirma PKCS#7",
						e);
			}
		}
		// Si la firma es SignedAndEnveloped
			...
			try {
				...
				if (targetType == CounterSignTarget.Tree) {
					int[] nodes = { 0 };
					dataSigned = new CounterSignerEnveloped()
							.counterSignerEnveloped(csp, sign,
									CounterSignTarget.Tree, nodes, keyEntry,
									dataType, atrib, uatrib);
				}
				// CASO DE FIRMA DE HOJAS
				else if (targetType == CounterSignTarget.Leafs) {
					...
					dataSigned = new CounterSignerEnveloped()
							.counterSignerEnveloped(csp, sign,
									CounterSignTarget.Leafs, nodes, keyEntry,
									dataType, atrib, uatrib);
				}
				// CASO DE FIRMA DE NODOS
				else if (targetType == CounterSignTarget.Nodes) {
				...
				// CASO DE FIRMA DE NODOS DE UNO O VARIOS FIRMANTES
					...
					dataSigned = new CounterSignerEnveloped()
							.counterSignerEnveloped(csp, sign,
									CounterSignTarget.Signers, nodes2,
									keyEntry, dataType, atrib, uatrib);
	...
	public boolean isSign(byte[] signData) {
		...
		boolean signed = CMSHelper.isCMSValid(signData,
				AOConstants.CMS_CONTENTTYPE_SIGNEDDATA);
		if (!signed) {
			signed = CMSHelper.isCMSValid(signData,
