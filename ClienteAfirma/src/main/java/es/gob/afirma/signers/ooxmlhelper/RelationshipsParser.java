...
public final class RelationshipsParser {
	 ...
	 * del tipo indicado. Si no se encuentra ninguna, se devuelve
	 * <code>null</code>.
	 * 
	 * @param type
	 ...
	 */
	private RelationShip[] getRelationships(InputStream xmlRelationships)
			throws AOException {

		Document doc = null;
		try {
			doc = DocumentBuilderFactory.newInstance().newDocumentBuilder()
					.parse(xmlRelationships);
		} catch (Exception e) {
			throw new AOException(
					"El flujo de datos proporcionado no era un XML valido");
		}
		...
		if (!root.getNodeName().equals("Relationships")
			...
			throw new AOException(
					"El nodo principal no es una etiqueta Relationships");
	...
	private RelationShip getRelationship(final Node node) throws AOException {
		...
		if (!node.getNodeName().equals("Relationship"))
			throw new AOException(
					"Se ha encontrado un nodo que es de relacion: "
							+ node.getNodeName());
		...
		if (attributes.getNamedItem("Id") == null
			...
			throw new AOException(
					"Se ha encontrado un nodo de relacion que no disponia de todos sus atributos");

		// Creamos la relacion
		return new RelationShip(attributes.getNamedItem("Id").getNodeValue(),
				attributes.getNamedItem("Type").getNodeValue(), attributes
