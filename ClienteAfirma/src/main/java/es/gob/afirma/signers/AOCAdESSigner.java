 ...
 * <dl>
 * <dt>mode</dt>
 * <dd>Modo de firma a usar (Expl&iacute;cita = <code>explicit</code> o
 * Impl&iacute;cita = <code>implicit</code>)</dd>
 ...
 * <dd>Debe establecerse a <code>true</code> si se desea usar la versi&oacute;n
 * 2 del atributo <i>Signing Certificate</i> de CAdES. Si no se establece o se
 * hace a <code>false</code> se utilizara la versi&oacute;n 1</dd>
 * <dt>useMIMEencoding</dt>
 * <dd>Debe establecerse a <code>true</code> si se desea usar
 * codificaci&oacute;n MIME para empotrar el binario en la firma (seg&uacute;n
 * recomendaciones de la CE). Este par&aacute;metro solo tiene efecto si el
 * par&aacute;metro <code>mode</code> se estableci&oacute; a
 * <code>implicit</code></dd>
 * <dt>MIMEContentID</dt>
 * <dd>Identificador MIME del contenido a firmar. S&oacute;lo se tiene en cuenta
 * si el par&aacute;metro <code>MIMEContentID</code> est&aacute; establecido a
 * <code>true</code> y el par&aacute;metro <code>mode</code> a
 * <code>implicit</code></dd>
 * <dt>MIMEContentType</dt>
 * <dd>Tipo MIME (MIME-Type) del contenido a firmar. S&oacute;lo se tiene en
 * cuenta si el par&aacute;metro <code>MIMEContentID</code> est&aacute;
 * establecido a <code>true</code> y el par&aacute;metro <code>mode</code> a
 * <code>implicit</code></dd>
 * <dt>MIMEFileName</dt>
 * <dd>Nombre original del fichero a firmar. S&oacute;lo se tiene en cuenta si
 * el par&aacute;metro <code>MIMEContentID</code> est&aacute; establecido a
 * <code>true</code> y el par&aacute;metro <code>mode</code> a
 * <code>implicit</code></dd>
 * <dt>MIMEModificationDate</dt>
 * <dd>Fecha de la &uacute;ltima modificaci&oacute;n (por defecto en formato
 * "<i>yyyy/MM/dd HH:mm:ss</i>") del contenido a firmar. S&oacute;lo se tiene en
 * cuenta si el par&aacute;metro <code>MIMEContentID</code> est&aacute;
 * establecido a <code>true</code> y el par&aacute;metro <code>mode</code> a
 * <code>implicit</code></dd>
 * <dt>MIMEModificationDateFormat</dt>
 * <dd>Formato en el que se ha proporcionado el valor del par&aacute;metro
 * <code></code> (fecha de la &uacute;ltima modificaci&oacute;n del contenido a
 * firmar) si este difiere de "<i>yyyy/MM/dd HH:mm:ss</i>". Misma sintaxis de
 * formato que <code>java.text.SimpleDateFormat</code>. S&oacute;lo se tiene en
 * cuenta si se ha establecido un valor para el par&aacute;metro
 * <code>MIMEModificationDate</code>, <code>MIMEContentID</code> est&aacute;
 * establecido a <code>true</code> y el par&aacute;metro <code>mode</code> a
 * <code>implicit</code></dd>
 * </dl>
 * 
...
public final class AOCAdESSigner implements AOSigner {
	...
	public byte[] sign(byte[] data, String algorithm,
		...
		final String precalculatedDigest = extraParams
				.getProperty("precalculatedHashAlgorithm");
		final boolean signingCertificateV2 = Boolean.parseBoolean(extraParams
				.getProperty("signingCertificateV2", "false"));
		...
		final String mode = extraParams.getProperty("mode",
				AOConstants.DEFAULT_SIGN_MODE);

		if (mode.equals(AOConstants.SIGN_MODE_IMPLICIT)
				&& "true".equalsIgnoreCase(extraParams
						.getProperty("useMIMEencoding"))) {

			Date modDateObject = null;
			final String modDate = extraParams
					.getProperty("MIMEModificationDate");
			if (modDate != null) {
				try {
					modDateObject = new SimpleDateFormat(
							extraParams.getProperty("useMIMEencoding",
									DEFAULT_MIME_MODIFICATION_DATE_FORMAT))
							.parse(modDate);
			...
			data = MimeHelper.getMimeEncodedAsAttachment(data,
					extraParams.getProperty("MIMEContentID"),
					extraParams.getProperty("MIMEContentType"),
					extraParams.getProperty("MIMEFileName"), modDateObject)
					.getBytes();
		}
		...
		if (certs != null && (certs instanceof X509Certificate[]))
			...
			if (cert instanceof X509Certificate)
				xCerts = new X509Certificate[] { (X509Certificate) cert };
		}

		final P7ContentSignerParameters csp = new P7ContentSignerParameters(
				data, algorithm, xCerts);
		...
		try {
			...
			if (mode.equals(AOConstants.SIGN_MODE_EXPLICIT)
					|| precalculatedDigest != null)
				omitContent = true;
			Oid policyQualifier = null;
			// Nos puede venir como URN o como OID
			try {
				policyQualifier = new Oid(extraParams
						.getProperty("policyQualifier").replace("urn:oid:", "")
						.replace("URN:oid:", "").replace("Urn:oid:", ""));
			} catch (final Exception e) {
			}

			return new GenCadesEPESSignedData().generateSignedData(csp,
					omitContent, extraParams.getProperty("policyIdentifier"),
					policyQualifier, signingCertificateV2, dataType, keyEntry,
					messageDigest);
	...
	public byte[] cosign(final byte[] data, final byte[] sign,
		...
		try {
			...
			if (signedData) {
				...
				final String mode = extraParams.getProperty("mode",
						AOConstants.DEFAULT_SIGN_MODE);
				final boolean omitContent = mode
						.equals(AOConstants.SIGN_MODE_EXPLICIT)
						|| precalculatedDigest != null;

				return new CadesCoSigner().coSigner(csp, sign, omitContent,
						extraParams.getProperty("policyIdentifier"),
						policyQualifier, signingCertificateV2, dataType,
						keyEntry, messageDigest);
			}

			return new CadesCoSignerEnveloped().coSigner(csp, sign,
					extraParams.getProperty("policyIdentifier"),
					policyQualifier, signingCertificateV2, dataType, keyEntry,
					messageDigest);
	...
	public byte[] cosign(final byte[] sign, String algorithm,
		...
		if (certs != null && (certs instanceof X509Certificate[]))
			...
			final Certificate cert = keyEntry.getCertificate();
			if (cert instanceof X509Certificate)
				aCertificados = new X509Certificate[] { (X509Certificate) cert };
		}
		...
		if (signedData) {
			try {
				return new CadesCoSigner().coSigner(typeAlgorithm,
						aCertificados, new ByteArrayInputStream(sign),
						extraParams.getProperty("policyIdentifier"),
						policyQualifier, signingCertificateV2, dataType,
						keyEntry, null // null porque no nos pueden dar un hash
		...
		if (signedData) {
			try {
				...
				if (targetType == CounterSignTarget.Tree) {
					...
					dataSigned = new CadesCounterSigner().counterSigner(csp,
							sign, CounterSignTarget.Tree, nodes, keyEntry,
							policyIdentifier, policyQualifier,
							signingCertificateV2, dataType);
				}
				// CASO DE FIRMA DE HOJAS
				else if (targetType == CounterSignTarget.Leafs) {
					int[] nodes = { 0 };
					dataSigned = new CadesCounterSigner().counterSigner(csp,
							sign, CounterSignTarget.Leafs, nodes, keyEntry,
							policyIdentifier, policyQualifier,
							signingCertificateV2, dataType);
				}
				// CASO DE FIRMA DE NODOS
					...
					dataSigned = new CadesCounterSigner().counterSigner(csp,
							sign, CounterSignTarget.Nodes, nodesID, keyEntry,
							policyIdentifier, policyQualifier,
							signingCertificateV2, dataType);
				}
				// CASO DE FIRMA DE NODOS DE UNO O VARIOS FIRMANTES
					...
					final int[] nodes2 = new ReadNodesTree()
							.readNodesFromSigners(signers, sign);
					dataSigned = new CadesCounterSigner().counterSigner(csp,
							sign, CounterSignTarget.Signers, nodes2, keyEntry,
							policyIdentifier, policyQualifier,
							signingCertificateV2, dataType);
		...
		try {
			...
			if (targetType == CounterSignTarget.Tree) {
				...
				dataSigned = new CadesCounterSignerEnveloped().counterSigner(
						csp, sign, CounterSignTarget.Tree, nodes, keyEntry,
						policyIdentifier, policyQualifier,
						signingCertificateV2, dataType);
			}
			// CASO DE FIRMA DE HOJAS
				...
				int[] nodes = { 0 };
				dataSigned = new CadesCounterSignerEnveloped().counterSigner(
						csp, sign, CounterSignTarget.Leafs, nodes, keyEntry,
						policyIdentifier, policyQualifier,
						signingCertificateV2, dataType);
			}
			// CASO DE FIRMA DE NODOS
				...
				nodesID = new ReadNodesTree().simplyArray(nodesID);
				dataSigned = new CadesCounterSignerEnveloped().counterSigner(
						csp, sign, CounterSignTarget.Nodes, nodesID, keyEntry,
						policyIdentifier, policyQualifier,
						signingCertificateV2, dataType);
			}
			// CASO DE FIRMA DE NODOS DE UNO O VARIOS FIRMANTES
				...
				for (int i = 0; i < targets.length; i++)
					signers[i] = (String) targets[i];
				final int[] nodes2 = new ReadNodesTree().readNodesFromSigners(
						signers, sign);
				dataSigned = new CadesCounterSignerEnveloped().counterSigner(
						csp, sign, CounterSignTarget.Signers, nodes2, keyEntry,
						policyIdentifier, policyQualifier,
						signingCertificateV2, dataType);
	 ...
	 */
	public byte[] envelop(final InputStream file, final String digestAlgorithm,
			String type, final PrivateKeyEntry keyEntry,
			final X509Certificate[] certDest, Properties extraParams)
			throws AOException {
		...
		if (file == null) {
			throw new NullPointerException(
					"El archivo a tratar no puede ser nulo.");
		}
		...
		if (keyEntry != null) {
			...
			csp = new P7ContentSignerParameters(plainData, digestAlgorithm,
					xCerts);
		...
		if (this.cipherAlgorithm == null) {
			...
			config = new AOCipherConfig(AOCipherAlgorithm.AES,
					AOCipherBlockMode.CBC, AOCipherPadding.PKCS5PADDING);
		}
		/*
		 ...
		 */
			...
			config = new AOCipherConfig(this.cipherAlgorithm,
					AOCipherBlockMode.CBC, AOCipherPadding.PKCS5PADDING);
		}
		...
		try {
			...
			// Es Digested Data.
				...
				dataSigned = new CADESDigestedData().genDigestedData(csp,
						dataType);
			}
			// Es Enveloped Data.
				...
				if (keyEntry != null) {
					dataSigned = new CADESEnvelopedData().genEnvelopedData(csp,
							config, certDest, dataType);
				} else {
					dataSigned = new CADESEnvelopedData().genEnvelopedData(
							plainData, digestAlgorithm, config, certDest,
							dataType);
				}
			}
			// Es Signed and Enveloped Data.
				...
				try {
					this.dataType = new Oid(
							PKCSObjectIdentifiers.signedData.getId());
				} catch (final Exception ex) {
					throw new AOException(
							"Error al asignar el OID por defecto del SignedData",
							ex);
				}
				Oid policyQualifier = null;
				try {
				...
				dataSigned = new CADESEPESSignedAndEnvelopedData()
						.genCADESEPESSignedAndEnvelopedData(csp, config,
								extraParams.getProperty("policyIdentifier"),
								policyQualifier, signingCertificateV2,
								certDest, dataType, keyEntry);
			}
		} catch (final Exception e) {
	 ...
	 * Los algoritmos y modos de firma disponibles se declaran en
	 * {@link AOConstants}.<br/>
	 ...
	 */
	public byte[] encrypt(final InputStream file, final String digestAlgorithm,
			final String key) throws AOException {

		// Comprobamos que el archivo a cifrar no sea nulo.
		if (file == null) {
			throw new NullPointerException(
					"El archivo a cifrar no puede ser nulo.");
		}
		...
		try {
			return new CADESEncryptedData().genEncryptedData(file,
					digestAlgorithm, config, key, dataType);
		} catch (final Exception e) {
			throw new AOException("Error generando el enveloped de CADES", e);
		}
	...
	public TreeModel getSignersStructure(final byte[] sign,
		...
		try {
			...
			Logger.getLogger("es.gob.afirma")
					.severe("No se ha podido obtener el albol de firmantes de la firma, se devolvera null: "
							+ ex);
		}
		return null;
	}
	...
	public static boolean isCADESValid(final byte[] signData, final String type) {
		if (type.equals(AOConstants.CMS_CONTENTTYPE_DATA)) {
			...
			return new ValidateCADES().isCADESEnvelopedData(signData);
				...
				.equals(AOConstants.CMS_CONTENTTYPE_SIGNEDANDENVELOPEDDATA)) {
			return new ValidateCADES().isCADESSignedAndEnvelopedData(signData);
		}
		Logger.getLogger("es.gob.afirma").warning(
				"Tipo de contenido CADES no reconocido");
		return false;
	}
	 ...
	 *             proporcionado.
	 */
	public String getDataMimeType(final byte[] signData)
			throws AOUnsupportedSignFormatException {
		...
		try {
			...
			throw new AOUnsupportedSignFormatException(
					"No es un tipo de firma valido", e1);
		}

		// Extraemos el mimetype y transformamos el OID a mimeType
		return MimeHelper.transformOidToMimeType(new ExtractMimeType()
				.extractMimeType(signData));
	 ...
	 *             Cuando ocurre cualquier problema durante el proceso
	 */
	public byte[] addOriginatorInfo(InputStream signFile, InputStream file,
			String signatureAlgorithm, PrivateKeyEntry keyEntry,
			Properties extraParams) throws AOException {
		...
		if (keyEntry != null) {
			...
			csp = new P7ContentSignerParameters(plainData, signatureAlgorithm,
					xCerts);
		...
		if (this.dataType == null) {
			try {
				...
				// + ex);
				throw new AOException(
						"Error al asignar el OID por defecto en el envoltorio CMS",
						ex);
			}
		}
		...
		try {
			dataSigned = new CADESEPESSignedAndEnvelopedData()
					.addOriginatorInfo(signFile, csp, keyEntry, dataType,
							extraParams.getProperty("policyIdentifier"),
							policyQualifier, signingCertificateV2);
	...
	public void setDataObjectFormat(final String description,
			...
			final javax.activation.MimeType mimeType, String encoding) {
	...
	public AOSignInfo getSignInfo(final byte[] signData)
			throws AOInvalidFormatException, AOException {
		if (signData == null)
			throw new NullPointerException(
