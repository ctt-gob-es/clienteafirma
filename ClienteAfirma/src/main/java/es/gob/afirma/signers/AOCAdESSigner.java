...
public final class AOCAdESSigner implements AOSigner {
    ...
    public byte[] sign(byte[] data, String algorithm, final PrivateKeyEntry keyEntry, Properties extraParams) throws AOException {
        ...
        if (mode.equals(AOConstants.SIGN_MODE_IMPLICIT) && "true".equalsIgnoreCase(extraParams.getProperty("useMIMEencoding"))) {
            ...
            if (modDate != null) {
                try {
                    ...
                    Logger.getLogger("es.gob.afirma").warning("La fecha MIME de ultima modificacion del contenido a firmar proporcionada (" + modDate
        ...
        try {
            ...
            try {
                policyQualifier =
                        new Oid(extraParams.getProperty("policyQualifier").replace("urn:oid:", "").replace("URN:oid:", "").replace("Urn:oid:", ""));
            }
            catch (final Exception e) {}
    ...
    public TreeModel getSignersStructure(final byte[] sign, final boolean asSimpleSignInfo) {
        ...
        try {
            ...
            Logger.getLogger("es.gob.afirma").severe("No se ha podido obtener el albol de firmantes de la firma, se devolvera null: " + ex);
        }
        return null;
    }
