...
/**
 * Generador de c&oacute;digos de verificaci&oacute;n para ser insertados en
 * ficheros PDF.
 */
public final class AOPDFVericode {
	...
	public static enum VericodeOrientation {
		/**
		 * Indicador de que el codigo de verificaci&oacute;n debe generarse con
		 * una orientacion de izquierda a derecha.
		 */
		DIRECTION_LEFT_TO_RIGHT,
		/**
		 * Indicador de que el codigo de verificaci&oacute;n debe generarse con
		 * una orientacion de arriba a abajo.
		 */
	...
	/**
	 * Genera una imagen con el c&oacute;digo de verificaci&oacute;n. El
	 * c&oacute;digo esta formado por una imagen (opcional), un c&oacute;digo de
	 * barras, un texto que se muestra encima del c&oacute;digo de barras
	 * (opcional) y un segundo texto que se muestra debajo. En caso de indicarse
	 * una imagen, &eacute;sta se mostrar&aacute; con un tama&ntilde;o
	 * m&aacute;ximo de 64x64 p&iacute;xeles. Si es necesario reducir la imagen
	 * para mostrarla, no se mantendr&aacute; la proporcion anchura-altura.
	 * 
	 * @param logo
	 *            Imagen que se mostrar&aacute; junto al c&oacute;digo.
	 * @param upperText
	 *            Texto superior.
	 * @param bottomText
	 *            Texto inferior.
	 ...
	 * @param direction
	 *            Orientaci&oacute;n del c&oacute;digo de verificaci&oacute;n.
	 ...
	 */
	public static BufferedImage getVericode(Image logo, String upperText,
			String bottomText, String code, VericodeOrientation direction)
			throws AOException {

		if (code == null) {
			throw new NullPointerException(
					"No se ha indicado el codigo que se desea generar");
		}
		...
		BufferedImage bImg = new BufferedImage(imageWidth, LOGO_WIDTH,
		...
		if (logo != null) {
			...
			gbImage.drawImage(
					new ImageIcon(logo.getScaledInstance(width, height,
							Image.SCALE_SMOOTH)).getImage(), 0, 0, null);
	...
	/**
	 * Gira una imagen seg&uacute;n la orientaci&oacute;n indicada. Las
	 * posiciones disponibles son: de izquierda a derecha (por defecto), de
	...
	private static BufferedImage rotateImage(BufferedImage image,
			VericodeOrientation direction) {

		if (image == null) {
			throw new NullPointerException(
					"No se ha indicado la imagen que se desea rotar");
		}

		BufferedImage rotatedImage = image;

		// Si hay que rotar, operamos como corresponde
		if (direction == VericodeOrientation.DIRECTION_UP_TO_DOWN
				|| direction == VericodeOrientation.DIRECTION_DOWN_TO_UP) {

			int initWidth = image.getWidth();
			int initHeight = image.getHeight();

			int[] pixels = image.getRGB(0, 0, initWidth, initHeight, null, 0,
					image.getWidth());
			rotatedImage = new BufferedImage(initHeight, initWidth,
					BufferedImage.TYPE_INT_RGB);
			...
			if (direction == VericodeOrientation.DIRECTION_UP_TO_DOWN) { // UP-TO-DOWN
				for (int x = 0; x < initHeight; x++)
					for (int y = 0; y < initWidth; y++)
						rotatedImage.setRGB((initHeight - 1) - x, y,
								pixels[p++]);
			} else { // DOWN-TO-UP
				for (int x = 0; x < initHeight; x++)
					for (int y = 0; y < initWidth; y++)
						rotatedImage
								.setRGB(x, (initWidth - 1) - y, pixels[p++]);
			}
		}
	...
	/**
	 * Calculamos la longitud de los distintos textos con las fuentes asignadas
	 * y tomamos la mayor como referencia para la longitud del codigo de barras.
	 * 
	 * @param canvas
	 *            Lienzo sobre el que se pinta el c&oacute;digo de barras.
	 * @param upperText
	 *            Texto superior.
	 * @param bottomText
	 *            Texto inferior.
	 ...
	 * @return Longitud
	 */
	private static int calculateWidth(String upperText, String bottomText,
			String code) {

		final int DEFAULT_MAX_WIDTH = 100;
		int maxWidth = DEFAULT_MAX_WIDTH;
		FontMetrics fm = null;
		BufferedImage canvas = new BufferedImage(100, 60,
				BufferedImage.TYPE_INT_RGB);
		...
		try {
			...
			Logger.getLogger("es.gob.afirma")
					.warning(
							"Error al calcular el tama\u00F1o m\u00E1ximo de los texto, se devolvera la longitud por defecto: "
									+ e.getMessage());
	...
	/**
	 * Comprueba que el c&oacute;digo textual cumpla con el est&aacute;ndar y lo
	 * modifica si no lo hace. Esto es, que la cadena empiece y termine con
	 * asterisco ('*'). Por ejemplo: *Mi Texto*.
	 * 
	 * @param code
	...
	private static Font getBarcodeFont() throws AOException {
		if (barcodeFont == null) {
			try {
				InputStream is = AOPDFVericode.class
						.getResourceAsStream("/FREE3OF9.TTF");
				barcodeFont = Font.createFont(Font.TRUETYPE_FONT, is)
						.deriveFont(Font.PLAIN, 36);
			} catch (Exception e) {
				Logger.getLogger("es.gob.afirma")
						.severe("No se ha podido cargar la fuente del codigo de barras");
				throw new AOException(
						"No se ha podido cargar la fuente del codigo de barras",
						e);
			}
		}
		return barcodeFont;
	...
	// JLabel label = new JLabel(new ImageIcon(getVericode(logo,
	// "Referencia del documento", "Consulte su validez en: www.mpr.es",
	// "1234567890", VericodeOrientation.DIRECTION_LEFT_TO_RIGHT)));
