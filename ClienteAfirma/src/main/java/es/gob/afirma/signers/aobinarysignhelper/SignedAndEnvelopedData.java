...
public final class SignedAndEnvelopedData extends ASN1Encodable {
	...
	SignedAndEnvelopedData(ASN1Set recipientInfos, ASN1Set digestAlgorithms,
	...
	SignedAndEnvelopedData(ASN1Sequence seq) {
		...
		this.encryptedContentInfo = EncryptedContentInfo.getInstance(seq
				.getObjectAt(index++));

		if (seq.size() > 5) {
			if (seq.size() == 6) {
				this.certificates = ASN1Set.getInstance(
						(ASN1TaggedObject) seq.getObjectAt(index++), false);
			} else {
				this.certificates = ASN1Set.getInstance(
						(ASN1TaggedObject) seq.getObjectAt(index++), false);
				this.crls = ASN1Set.getInstance(
						(ASN1TaggedObject) seq.getObjectAt(index++), false);
			}
	 ...
	 *                converted.
	 */
	static SignedAndEnvelopedData getInstance(ASN1TaggedObject obj,
			boolean explicit) {
