...
public final class CMSDecipherAuthenticatedEnvelopedData {
	 ...
	 */
	public byte[] dechiperAuthenticatedEnvelopedData(byte[] cmsData,
		...
		try {
			ASN1Sequence contentAuthEnvelopedData = Utils
					.fetchWrappedData(cmsData);

			authEnvelopedData = AuthEnvelopedData
					.getInstance(contentAuthEnvelopedData);
			elementRecipient = authEnvelopedData.getRecipientInfos()
					.getObjects();
		} catch (final Exception ex) {
			throw new AOException(
					"El fichero no contiene un tipo AuthenticatedEnvelopedData",
					ex);
		}

		X509Certificate userCert = (X509Certificate) keyEntry.getCertificate();
		final EncryptedKeyDatas encryptedKeyDatas = Utils
				.fetchEncryptedKeyDatas(userCert, elementRecipient);

		// Obtenemos el contenido cifrado
		final EncryptedContentInfo contenidoCifrado = authEnvelopedData
				.getAuthEncryptedContentInfo();

		// Obtenemos el algoritmo usado para cifrar la clave generada.
		final AlgorithmIdentifier algClave = contenidoCifrado
				.getContentEncryptionAlgorithm();

		// Asignamos la clave de descifrado del contenido.
		final KeyAsigned keyAsigned = Utils.assignKey(
				encryptedKeyDatas.getEncryptedKey(), keyEntry, algClave);

		// Desciframos el contenido.
		final byte[] deciphered;
		final byte[] contCifrado = contenidoCifrado.getEncryptedContent()
				.getOctets();
		try {
