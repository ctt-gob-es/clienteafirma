...
public final class ReadNodesTree {
	 ...
	 *             datos
	 */
	public TreeModel readNodesTree(byte[] data, boolean asSimpleSignInfo)
			throws IOException {
		...
		try {
			...
			try {
				Logger.getLogger("es.gob.afirma")
						.info("No es signedData, probamos con SignedAndEnvelopedData.");
		...
		if (asSimpleSignInfo && signerInfosSd != null) {
			for (int i = 0; i < signerInfosSd.size(); i++) {
				ASN1Sequence atribute = (ASN1Sequence) signerInfosSd
						.getObjectAt(i);
				IssuerAndSerialNumber issuerSerial = new IssuerAndSerialNumber(
						(ASN1Sequence) atribute.getObjectAt(1));
				X509Certificate[] nameSigner = searchCert(certificates,
						issuerSerial.getSerialNumber());
				SignerInfo si = new SignerInfo(atribute);
				Date signingTime = getSigningTime(si);
				AOSimpleSignInfo aossi = new AOSimpleSignInfo(nameSigner,
						signingTime);
				aossi.setPkcs1(si.getEncryptedDigest().getOctets());
				rama = new TreeNode(aossi);
				listaCert.add(nameSigner);
				getUnsignedAtributesWithCertificates(
						si.getUnauthenticatedAttributes(), rama, certificates);
			...
			for (int i = 0; i < signerInfosSd.size(); i++) {
				...
				IssuerAndSerialNumber issuerSerial = new IssuerAndSerialNumber(
						(ASN1Sequence) atribute.getObjectAt(1));
				String nameSigner = searchName(certificates,
						issuerSerial.getSerialNumber());
				SignerInfo si = new SignerInfo(atribute);
				rama = new TreeNode(nameSigner);
				lista.add(nameSigner);
				getUnsignedAtributes(si.getUnauthenticatedAttributes(), rama,
						certificates);
	 ...
	 *            Certificados.
	 */
	private void getUnsignedAtributesWithCertificates(ASN1Set signerInfouAtrib,
			TreeNode ramahija, ASN1Set certificates) {
				...
				if (!data.getAttrType().equals(
					...
					while (eAtributesData.hasMoreElements()) {
						...
						if (obj instanceof ASN1Sequence) {
							...
							IssuerAndSerialNumber issuerSerial = new IssuerAndSerialNumber(
									(ASN1Sequence) atrib.getObjectAt(1));
							SignerInfo si = new SignerInfo(atrib);
							X509Certificate[] nameSigner = searchCert(
									certificates,
									issuerSerial.getSerialNumber());
							Date signingTime = getSigningTime(si);
							AOSimpleSignInfo aossi = new AOSimpleSignInfo(
									nameSigner, signingTime);
							aossi.setPkcs1(si.getEncryptedDigest().getOctets());
							rama2 = new TreeNode(aossi);
							listaCert.add(nameSigner);
							ramahija.add(rama2);
							getUnsignedAtributesWithCertificates(
									si.getUnauthenticatedAttributes(), rama2,
									certificates);
						}
					}
				}
	 ...
	 *            Certificados.
	 */
	private void getUnsignedAtributes(ASN1Set signerInfouAtrib,
			TreeNode ramahija, ASN1Set certificates) {
	 ...
	 */
	public int[] readNodesFromSigners(final String[] signers, final byte[] data)
			throws IOException {
		int[] solucion;
	...
	private String searchName(ASN1Set certificates, DERInteger serialNumber) {
		...
		while (certSet.hasMoreElements()) {
			...
			TBSCertificateStructure atrib = TBSCertificateStructure
					.getInstance(atrib2.getObjectAt(0));
			if (serialNumber.toString().equals(
					atrib.getSerialNumber().toString())) {
				X509Name name = atrib.getSubject();
				nombre = name.getValues(X509ObjectIdentifiers.commonName)
						.toString();
				if (nombre != null && !nombre.equals("")
						&& !nombre.equals("[]")) {
					nombre = nombre.substring(1, nombre.length() - 1);
				} else {
					nombre = name.getValues(
	 ...
	 *         en orden).
	 */
	private X509Certificate[] searchCert(ASN1Set certificates,
		...
		while (certSet.hasMoreElements()) {
			...
			if (serialNumber.toString().equals(
				...
				try {
					ret[0] = new X509CertificateObject(
							new X509CertificateStructure(atrib2));
				} catch (Exception e) {
					Logger.getLogger("es.gob.afirma")
							.severe("No se pudieron recuperar los certificados de la lista, se devolvera un array vacio: "
									+ e);
		...
		Logger.getLogger("es.gob.afirma")
				.severe("El certificados pedido no estaba en la lista, se devolvera un array vacio");
		return new X509Certificate[0];
	}
	...
	private Date getSigningTime(SignerInfo si) {
		...
		if (si.getAuthenticatedAttributes() != null) {
			Enumeration<?> eAtributes = si.getAuthenticatedAttributes()
					.getObjects();
			while (eAtributes.hasMoreElements()) {
				Attribute data = new Attribute(
						(ASN1Sequence) eAtributes.nextElement());
