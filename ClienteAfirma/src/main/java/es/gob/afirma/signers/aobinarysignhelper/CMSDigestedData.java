...
public final class CMSDigestedData {
	 ...
	 *             datos
	 */
	public byte[] genDigestedData(final byte[] content,
			final String digestAlgorithm, final Oid dataType)
			throws NoSuchAlgorithmException, IOException {

		// Obtenemos el algoritmo para hacer el digest
		AlgorithmId digestAlgorithmId = AlgorithmId.get(AOCryptoUtil
				.getDigestAlgorithmName(digestAlgorithm));
		...
		// digest
		DEROctetString digest = new DEROctetString(MessageDigest.getInstance(
				digestAlgorithm).digest(content));

		// construimos el digestedData.
		return new ContentInfo(PKCSObjectIdentifiers.digestedData,
				new DigestedData(digAlgId, encInfo, digest)).getDEREncoded();
	}
}