...
final class Utils {
	...
	private static final byte[] SALT = { (byte) 0xA2, (byte) 0x35, (byte) 0xDC,
			(byte) 0xA4, (byte) 0x11, (byte) 0x7C, (byte) 0x99, (byte) 0x4B };
	 ...
	 */
	private static final byte[] IV_8 = { (byte) 0xC6, (byte) 0xBA, (byte) 0xDE,
			(byte) 0xA4, (byte) 0x76, (byte) 0x43, (byte) 0x32, (byte) 0x6B };
	 ...
	 */
	private static final byte[] IV_16 = { (byte) 0xB2, (byte) 0xBA,
			(byte) 0xDE, (byte) 0xA4, (byte) 0x41, (byte) 0x7F, (byte) 0x97,
			(byte) 0x4B, (byte) 0xAC, (byte) 0x63, (byte) 0xAC, (byte) 0xAA,
	 ...
	 */
	public static SecretKey initEnvelopedData(final AOCipherConfig config,
			final X509Certificate[] certDest) throws NullPointerException {
		// Comprobamos que el archivo a tratar no sea nulo.
		if (certDest == null || certDest.length == 0) {
			throw new NullPointerException(
					"No se pueden envolver datos sin certificados destino.");
		}
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").severe(
					"Error durante el proceso de asignado de clave: " + ex);
		}
	 ...
	 * @return
	 */
	private static SecretKey assignKey(AOCipherConfig config)
			throws NoSuchAlgorithmException {
		SecureRandom rand = new SecureRandom();

		KeyGenerator kg = KeyGenerator.getInstance(config.getAlgorithm()
				.getName());
		kg.init(rand);
	 ...
	 * @return Clave secreta
	 */
	public static SecretKey assignKey(final AOCipherConfig config,
			final String key) {
		...
		if ((config.getAlgorithm().equals(AOCipherAlgorithm.PBEWITHMD5ANDDES))
			...
			try {
				return SecretKeyFactory.getInstance(
						...
						.generateSecret(
								new PBEKeySpec(key.toCharArray(), SALT,
										ITERATION_COUNT));
			} catch (Exception ex) {
				Logger.getLogger("es.gob.afirma").severe(
			...
			return new SecretKeySpec(Base64.decodeBase64(key), config
	 ...
	 */
	public static ASN1Set fetchCertificatesList(
		...
		if (signerCertificateChain.length != 0) {
			...
			for (int i = 0; i < signerCertificateChain.length; i++)
				ce.add(X509CertificateStructure.getInstance(ASN1Object
						.fromByteArray(signerCertificateChain[i].getEncoded())));
	...
	public static Info initVariables(byte[] data, AOCipherConfig config,
			X509Certificate[] certDest, SecretKey cipherKey)
		...
		for (int contCert = 0; contCert < certDest.length; contCert++) {
			...
			tbs = TBSCertificateStructure.getInstance(ASN1Object
					.fromByteArray(cert.getTBSCertificate()));
			// Obtenemos el Isuer & serial number
			isse = new IssuerAndSerialNumber(X500Name.getInstance(tbs
			...
			try {
				...
				Logger.getLogger("es.gob.afirma").severe(
						"Error durante el proceso cifrado de la clave: " + e);
			}
			...
			final KeyTransRecipientInfo keyTransRecipientInfo = new KeyTransRecipientInfo(
					rid, keyEncAlg, new DEROctetString(encryptedKey));
	 ...
	 */
	public static EncryptedContentInfo getEncryptedContentInfo(byte[] file,
			AOCipherConfig config, SecretKey cipherKey)
	 ...
	 */
	public static EncryptedContentInfo getEncryptedContentInfo(
			final byte[] file, final Key cipherKey, final AOCipherConfig config)
			throws NoSuchAlgorithmException, NoSuchPaddingException,
			InvalidAlgorithmParameterException, InvalidKeyException,
			IOException {
	...
	private static EncryptedContentInfo getEncryptedContentInfo(byte[] file,
		...
		try {
			...
			Logger.getLogger(CMSEnvelopedData.class.getName()).log(
		...
		if (params != null) {
			ASN1InputStream aIn = new ASN1InputStream(cipher.getParameters()
					.getEncoded("ASN.1"));
		...
		return new EncryptedContentInfo(contentType, encAlgId,
	 ...
	 * @throws javax.crypto.NoSuchPaddingException
	 */
	private static Cipher createCipher(String algName)
			throws NoSuchAlgorithmException, NoSuchPaddingException {
	 ...
	 */
	public static AlgorithmParameterSpec getParams(
		...
		if (algorithmConfig.getAlgorithm().supportsPassword())
			...
			if (!algorithmConfig.getBlockMode().equals(AOCipherBlockMode.ECB)) {
				params = new IvParameterSpec(algorithmConfig.getAlgorithm()
						.equals(AOCipherAlgorithm.AES) ? IV_16 : IV_8);
			}
		}
	 ...
	 * @throws javax.crypto.BadPaddingException
	 */
	private static byte[] cipherKey(PublicKey pKey, SecretKey cipherKey)
			throws NoSuchAlgorithmException, NoSuchPaddingException,
	 ...
	 * @throws NoSuchAlgorithmException
	 */
	public static ASN1EncodableVector initContexExpecific(
		...
		if (messageDigest == null) {
			messageDigest = MessageDigest.getInstance(digestAlgorithm).digest(
					datos);
		}
	...
	public static byte[] genMac(String encryptionAlg, byte[] content,
			SecretKey ciphKey) throws NoSuchAlgorithmException, IOException {
		Mac mac;
		...
		try {
			...
			throw new IOException(
					"Error al generar el codigo de autenticacion Mac: " + e);
		}
	}

	public static OriginatorInfo checkCertificates(
			X509Certificate[] signerCertificateChain, OriginatorInfo origInfo,
			ASN1Set certs) throws IOException, CertificateEncodingException {
			...
			if (certs == null) {
				...
				for (int i = 0; i < signerCertificateChain.length; i++)
					if (signerCertificateChain[i] != null)
						ce.add(X509CertificateStructure.getInstance(ASN1Object
								.fromByteArray(signerCertificateChain[i]
										.getEncoded())));
	 ...
	 * @throws CertificateEncodingException
	 */
	public static EncryptedKeyDatas fetchEncryptedKeyDatas(
			X509Certificate userCert, Enumeration<?> elementRecipient)
			throws AOInvalidRecipientException, IOException,
			CertificateEncodingException {
		...
		tbs = TBSCertificateStructure.getInstance(ASN1Object
				.fromByteArray(userCert.getTBSCertificate()));
		// Obtenemos el Isuer & serial number
		...
		while (elementRecipient.hasMoreElements()) {
			...
			ASN1Sequence intermedio = (ASN1Sequence) elementRecipient
					.nextElement();
			reci = RecipientInfo.getInstance(intermedio);
			KeyTransRecipientInfo kri = KeyTransRecipientInfo.getInstance(reci
					.getDERObject());
			IssuerAndSerialNumber actual = IssuerAndSerialNumber
					.getInstance(kri.getRecipientIdentifier().getDERObject());
	 ...
	 * @throws IOException
	 */
	public static ASN1Sequence fetchWrappedData(byte[] cmsData)
			throws IOException {
	 ...
	 * @throws javax.crypto.BadPaddingException
	 */
	public static byte[] deCipherContent(byte[] file, AOCipherConfig config,
			SecretKey cipherKey) throws NoSuchAlgorithmException,
	 ...
	 * @throws NoSuchAlgorithmException
	 */
	public static SecretKey loadCipherKey(AOCipherConfig config, String key)
			throws InvalidKeySpecException, NoSuchAlgorithmException {
		SecretKey cipherKey = SecretKeyFactory.getInstance(
				config.getAlgorithm().getName()).generateSecret(
	 ...
	 *             usuario.
	 */
	public static KeyAsigned assignKey(final byte[] passCiphered,
			final PrivateKeyEntry keyEntry, final AlgorithmIdentifier algClave)
			throws AOException {
		...
		if (algorithm == null)
			throw new AOException(
					"No se ha podido determinal el algoritmo de cifrado de la clave");
		...
		try {
			...
			keyAsigned.setCipherKey((SecretKey) cipher2.unwrap(encrypted,
					algorithm.getName(), Cipher.SECRET_KEY));
			return keyAsigned;
		} catch (Exception e) {
			Logger.getLogger("es.gob.afirma").severe(
					"Ocurri\u00F3 un error al recuperar la clave de cifrado del sobre digital: "
							+ e);
			throw new AOException(
					"Ocurri\u00F3 un error al recuperar la clave de cifrado del sobre digital",
					e);
	 ...
	 * @throws java.io.IOException
	 */
	public static ASN1Set generateSignerInfo(String digestAlgorithm,
			byte[] datos, Oid dataType, Map<Oid, byte[]> uatrib)
			throws NoSuchAlgorithmException {

		// // ATRIBUTOS

		// authenticatedAttributes
		ASN1EncodableVector ContexExpecific = Utils.initContexExpecific(
				digestAlgorithm, datos, dataType, null);
	...
	public static String getEncryptedContentInfo(EncryptedContentInfo datos) {
		...
		if (algorithm != null) {
			...
			info = info + "\tOID del Algoritmo de cifrado: "
					+ ai.getAlgorithm().toString() + "\n";
		}
	 ...
	 * @throws CertificateEncodingException
	 */
	public static ASN1EncodableVector loadCertificatesList(
			SignedAndEnvelopedData signEnv,
			X509Certificate[] signerCertificateChain) throws IOException,
			CertificateEncodingException {
		...
		if (signerCertificateChain.length != 0) {
			for (int i = 0; i < signerCertificateChain.length; i++)
				signCerts
						.add(X509CertificateStructure.getInstance(ASN1Object
								.fromByteArray(signerCertificateChain[i]
										.getEncoded())));
		}
	...
	public static SignerInfo signAndEnvelope(PrivateKeyEntry keyEntry,
			String signatureAlgorithm, AlgorithmIdentifier digAlgId,
			SignerIdentifier identifier, ASN1Set signedAttr,
			ASN1Set unSignedAttr, AlgorithmId digestAlgorithmIdEnc,
		...
		SignerInfo nuevoSigner = new SignerInfo(identifier, digAlgId,
				signedAttr, encAlgId, sign2, unSignedAttr// null //unsignedAttr
		);
	 ...
	 */
	public static ASN1OctetString firma(final String signatureAlgorithm,
			final PrivateKeyEntry keyEntry, final ASN1Set signedAttr2)
			throws AOException {
		...
		try {
			...
			Logger.getLogger("No se han podido codificar en ASN.1 los atributos firmados: "
					+ ex);
		}
	 ...
	 * @param doj
	 * @param envelopeType
	 *            Tipo de formato: <li>0: EnvelopedData</li> <li>1:
	 *            AuthenticatedData</li> <li>2: AuthEnvelopedData</li> <li>3:
	 *            SignedAndEnvelopedData</li> <li>4: SignedData</li> <li>5:
	 *            Encrypted</li>
	 * @param tipoDetalle
	...
	public static String extractData(ASN1TaggedObject doj, String envelopeType,
		...
		if (envelopeType.equals("0")) {
			...
			AuthenticatedData ed = new AuthenticatedData(
			...
			AuthEnvelopedData ed = new AuthEnvelopedData(
					(ASN1Sequence) doj.getObject());
			version = ed.getVersion();
			rins = ed.getRecipientInfos();
			...
			SignedAndEnvelopedData ed = new SignedAndEnvelopedData(
					(ASN1Sequence) doj.getObject());
			version = ed.getVersion();
			rins = ed.getRecipientInfos();
			encryptedContentInfo = ed.getEncryptedContentInfo();
			...
			encryptedContentInfo = EncryptedContentInfo.getInstance(ed
					.getObjectAt(1));
			if (ed.size() == 3)
		...
		// recipientInfo
		if (rins != null) {
			if (!envelopeType.equals("4") && !envelopeType.equals("5")
					&& rins.size() > 0) {
				detalle = detalle + "Destinatarios: \n";
			}
			for (int i = 0; i < rins.size(); i++) {
				KeyTransRecipientInfo kti = KeyTransRecipientInfo
						.getInstance(RecipientInfo.getInstance(
								rins.getObjectAt(i)).getInfo());
				detalle = detalle + " - Informacion de destino de firma "
						+ (i + 1) + ":\n";
				final AlgorithmIdentifier diAlg = kti
						.getKeyEncryptionAlgorithm();

				// issuer y serial
				final IssuerAndSerialNumber iss = (IssuerAndSerialNumber) SignerIdentifier
						.getInstance(kti.getRecipientIdentifier().getId())
						.getId();
				detalle = detalle + "\tIssuer: " + iss.getName().toString()
						+ "\n";
				detalle = detalle + "\tNumero de serie: "
						+ iss.getSerialNumber() + "\n";
				...
				for (int j = 0; j < algos.length; j++) {
					if (algos[j].getOid().equals(
							diAlg.getAlgorithm().toString())) {
						algorithm = algos[j];
					}
				}
				if (algorithm != null) {
					detalle = detalle + "\tAlgoritmo de cifrado: "
							+ algorithm.getName() + "\n";
				} else {
					detalle = detalle + "\tOID del algoritmo de cifrado: "
							+ diAlg.getAlgorithm() + "\n";
		...
		if (envelopeType.equals("0") || envelopeType.equals("5")) {
			...
			detalle = detalle
					+ Utils.getEncryptedContentInfo(encryptedContentInfo);
		} else if (envelopeType.equals("1") && aid != null && ci != null) {
			// mac algorithm
			detalle = detalle + "OID del Algoritmo de MAC: "
					+ aid.getAlgorithm() + "\n";

			// digestAlgorithm
			ASN1Sequence seq = (ASN1Sequence) doj.getObject();
			ASN1TaggedObject da = (ASN1TaggedObject) seq.getObjectAt(4);
			AlgorithmIdentifier dai = AlgorithmIdentifier.getInstance(da
					.getObject());
			detalle = detalle + "OID del Algoritmo de firma: "
					+ dai.getAlgorithm() + "\n";

			// obtenemos datos de los datos cifrados.
			detalle = detalle + "OID del tipo de contenido: "
					+ ci.getContentType() + "\n";
			...
			// algoritmo de firma
			AlgorithmIdentifier dai = AlgorithmIdentifier.getInstance(ds
					.getObjectAt(0));
			detalle = detalle + "OID del Algoritmo de firma: "
					+ dai.getAlgorithm() + "\n";
			detalle = detalle + "OID del tipo de contenido: "
		...
		if (!envelopeType.equals("3"))
			if (unprotectedAttrs == null) {
				detalle = detalle
						+ "Atributos : No tiene atributos opcionales\n";
			} else {
			...
			if (signerInfosSd != null) {
				...
				for (int i = 0; i < signerInfosSd.size(); i++) {
					...
					IssuerAndSerialNumber iss = IssuerAndSerialNumber
							.getInstance(sident.getId());
					detalle = detalle + "\tIssuer: " + iss.getName().toString()
							+ "\n";
					detalle = detalle + "\tNumero de serie: "
							+ iss.getSerialNumber() + "\n";
					...
					detalle = detalle
							+ "\tOID del algoritmo de firma de este firmante: "
							+ algId.getAlgorithm() + "\n";
	...
	private static String getObligatorieAtrib(String signBinaryType,
		...
		if (authAttrs == null) {
			detalle = detalle
					+ "Atributos Autenticados: No tiene atributos autenticados\n";
		} else {
	 ...
	 */
	public static String getsignedAttributes(ASN1Set attributes,
			String binarySignType) {
		String attributos = "";
		...
		while (e.hasMoreElements()) {
			...
			if (derIden.equals(CMSAttributes.signingTime)) {
				...
				try {
					...
					Logger.getLogger("es.gob.afirma").warning(
							"No es posible convertir la fecha");
				}
			...
			if (binarySignType.equals("CADES")) {
				...
				if (derIden
					...
					attributos = attributos
							+ "\t\tContiene el atributo \"Signing Certificate V2\" \n";
				}
				// Politica de firma.
				if (derIden.equals(PKCSObjectIdentifiers.id_aa_ets_sigPolicyId)) {
					attributos = attributos
							+ "\t\tContiene la politica de la firma \n";
				}
			}
		}
		return attributos;
	...
	private static String getUnSignedAttributes(Enumeration<?> e) {
		...
		while (e.hasMoreElements()) {
			...
			if (derIden.equals(CMSAttributes.counterSignature)) {
				attributos = attributos
						+ "\tContiene la contrafirma de la firma \n";
			}
	...
	public static String getKeyAlgorithm(String signatureAlgorithm,
			AlgorithmId digestAlgorithmId) throws IOException {
		try {
	...
	public static ASN1EncodableVector generateSignerInfo(X509Certificate cert,
			AlgorithmId digestAlgorithmId, String digestAlgorithm,
			AlgorithmIdentifier digAlgId, byte[] datos, String politica,
			Oid qualifier, boolean signingCertificateV2, Oid dataType,
			byte[] messageDigest) throws NoSuchAlgorithmException, IOException,
			CertificateEncodingException {
		// // ATRIBUTOS

		// authenticatedAttributes
		ASN1EncodableVector ContexExpecific = initContexExpecific(
				digestAlgorithm, datos, dataType, messageDigest);
		...
		if (signingCertificateV2) {
			...
			MessageDigest md = MessageDigest.getInstance(AOCryptoUtil
					.getDigestAlgorithmName(digestAlgorithmId.getName()));
			byte[] certHash = md.digest(cert.getEncoded());
			ESSCertIDv2[] essCertIDv2 = { new ESSCertIDv2(digAlgId, certHash,
					isuerSerial) };
			...
			if (qualifier != null) {
				...
				DERObjectIdentifier oidQualifier = new DERObjectIdentifier(
						qualifier.toString());
				if (politica.equals(""))
					pI = new PolicyInformation[] { new PolicyInformation(
							oidQualifier) };
				else {
					PolicyQualifierInfo pqInfo = new PolicyQualifierInfo(
							politica);
					pI = new PolicyInformation[] { new PolicyInformation(
							oidQualifier, new DERSequence(pqInfo)) };
				}
			...
			String digestAlgorithmName = AOCryptoUtil
					.getDigestAlgorithmName(digestAlgorithmId.getName());
			...
			// Secuencia con singningCertificate
			ContexExpecific.add(new Attribute(
					PKCSObjectIdentifiers.id_aa_signingCertificate, new DERSet(
							scv)));
		}
		...
		if (qualifier != null) {
			 ...
			 */
			DERObjectIdentifier DOISigPolicyId = new DERObjectIdentifier(
					qualifier.toString());
			...
			SigPolicyQualifierInfo spqInfo = new SigPolicyQualifierInfo(
					politica);
			...
			// Secuencia con singningCertificate
			ContexExpecific.add(new Attribute(
					PKCSObjectIdentifiers.id_aa_ets_sigPolicyId, new DERSet(ds
							.toASN1Object())));
