...
public final class CounterSignerEnveloped {
	...
	public byte[] counterSignerEnveloped(P7ContentSignerParameters parameters,
			...
			PrivateKeyEntry keyEntry, Oid dataType, Map<Oid, byte[]> atri,
		...
		if (signerCertificateChain.length != 0) {
			...
			vCertsSig.add(X509CertificateStructure.getInstance(ASN1Object
					.fromByteArray(signerCertificateChain[0].getEncoded())));
				...
				if (!data.getAttrType().equals(
					...
					while (eAtributesData.hasMoreElements()) {
						...
						if (obj instanceof ASN1Sequence) {
							...
							SignerInfo obtained = getCounterUnsignedAtributes(
	...
	private ASN1Set generateSignerInfo(X509Certificate cert,
			String digestAlgorithm, byte[] datos)
		...
		// DERSet(new DERObjectIdentifier(actualOid.toString()))));
		...
		ContexExpecific.add(new Attribute(CMSAttributes.messageDigest,
				new DERSet(new DEROctetString(MessageDigest.getInstance(
						digestAlgorithm.toString()).digest(datos)))));
	...
	private SignerInfo UnsignedAtributte(P7ContentSignerParameters parameters,
		...
		signedAttr = generateSignerInfo(cert, digestAlgorithm, si
		...
		try {
			...
			Logger.getLogger(CounterSignerEnveloped.class.getName()).log(
					Level.SEVERE, null, ex);
		}

		SignerInfo uAtrib = new SignerInfo(identifier, digAlgId, signedAttr,
				encAlgId, sign2, unsignedAttr// null
		);
