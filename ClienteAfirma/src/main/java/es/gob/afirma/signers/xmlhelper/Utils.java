...
public final class Utils {
	 ...
	 *             Documento DOM
	 */
	public static Document dereferenceStyleSheet(final String id,
			final boolean headLess) throws CannotDereferenceException,
			IsInnerlException, ReferenceIsNotXMLException {
		...
		// http://, https:// y file://
		try {
			xml = AOUtil.getDataFromInputStream(AOUtil.loadFile(
					AOUtil.createURI(id), null, false));
			...
			if (fileName.startsWith("#"))throw new IsInnerlException(); //$NON-NLS-1$
				...
				if (JOptionPane.showConfirmDialog(
					...
					try {
						...
						throw new CannotDereferenceException(
								"No se ha podido dereferenciar la hoja de estilo", ex); //$NON-NLS-1$
					}
				}
			} else
				throw new CannotDereferenceException(
		...
		try {
			...
			throw new CannotDereferenceException(
					"No se ha dereferenciado la hoja de estilo"); //$NON-NLS-1$
		} catch (final Exception e) {
	 ...
	 *         a&ntilde;adida
	 */
	public static String addStyleSheetHeader(final String xml, String type,
			final String href) {
		if (href == null)
			return xml;
		if (type == null)
	 ...
	 *            Prefijo XMLDSig
	 */
	public static void addCustomTransforms(List<Transform> transformList,
			Properties extraParams, String xmlSignaturePrefix) {
		...
		int numTransforms = Integer.parseInt(extraParams.getProperty(
				"xmlTransforms", "0")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		for (int i = 0; i < numTransforms; i++) {
			transformType = extraParams
					.getProperty("xmlTransform" + Integer.toString(i) + "Type"); //$NON-NLS-1$ //$NON-NLS-2$
			transformBody = extraParams
					.getProperty("xmlTransform" + Integer.toString(i) + "Body"); //$NON-NLS-1$ //$NON-NLS-2$

			if (Transform.XPATH.equals(transformType) && transformBody != null) {
				try {
					transformParam = new XPathFilterParameterSpec(
							transformBody, Collections.singletonMap(
									xmlSignaturePrefix, XMLSignature.XMLNS));
				} catch (Exception e) {
			...
			} else if (Transform.XPATH2.equals(transformType)
					&& transformBody != null) {
				transformSubtype = extraParams
						.getProperty("xmlTransform" + Integer.toString(i) + "Subtype"); //$NON-NLS-1$ //$NON-NLS-2$
				...
				try {
					transformParam = new XPathFilter2ParameterSpec(
							Collections.singletonList(new XPathType(
									transformBody, xPath2TransformFilter)));
				} catch (Exception e) {
					Logger.getLogger("es.gob.afirma").warning( //$NON-NLS-1$
			...
			} else if (Transform.XSLT.equals(transformType)
					&& transformBody != null) {
				try {
					transformParam = new XSLTTransformParameterSpec(
							new DOMStructure(DocumentBuilderFactory
									.newInstance()
									.newDocumentBuilder()
			...
			// anadimos
			try {
				transformList.add(fac.newTransform(transformType,
						transformParam));
			} catch (Exception e) {
	 ...
	 *             soportado.
	 */
	public static Vector<Transform> getObjectReferenceTransforms(
			Node referenceNode, String namespacePrefix)
		...
		for (int i = 0; i < tmpNodeList.getLength(); i++) {
			...
			if ("Transforms".equals(tmpNodeList.item(i).getNodeName())
					|| (namespacePrefix + ":Transforms").equals(tmpNodeList
							.item(i).getNodeName())) {

				NodeList transformsNodeList = tmpNodeList.item(i)
						.getChildNodes();
				...
				for (int j = 0; j < transformsNodeList.getLength(); j++) {
					...
					transformList
							.add(fac.newTransform(
									getTransformAlgorithm(transformsNodeList
											.item(j)),
									getTransformParameterSpec(
											transformsNodeList.item(j),
											namespacePrefix)));
				}
	...
	private static String getTransformAlgorithm(Node transformNode) {
		...
		Node algorithmNode = transformNode.getAttributes().getNamedItem(
				"Algorithm");
	 ...
	 */
	private static TransformParameterSpec getTransformParameterSpec(
			Node transformNode, String namespacePrefix)
			throws InvalidAlgorithmParameterException {
		...
		if (algorithm != null
				&& (Transform.XPATH.equals(algorithm) || Transform.XPATH2
						.equals(algorithm))) {
			...
			if (Transform.XPATH.equals(algorithm)) {
				...
				for (int i = 0; i < xpathTransforms.getLength(); i++) {
					...
					if ("XPath".equals(xpathTransformNode.getNodeName())
							|| (namespacePrefix + ":XPath")
									.equals(xpathTransformNode.getNodeName())) {
						...
						if (namespacePrefix == null
							...
							params = new XPathFilterParameterSpec(
									xpathTransformNode.getTextContent(),
									Collections.singletonMap(namespacePrefix,
											XMLSignature.XMLNS));
						}
			...
			// subtipo
				...
				for (int i = 0; i < xpathTransforms.getLength(); i++) {
					...
					if ("XPath".equals(xpathTransformNode.getNodeName())
						...
						Filter filter = null;
						Node filterNode = xpathTransformNode.getAttributes()
								.getNamedItem("Filter");
						if (filterNode != null) {
	...
	public static void checkIllegalParams(final String format,
			final String mode, final URI uri,
			final String externallyDetachedHashAlgorithm, final boolean xades) {
		if (!mode.equals(SIGN_MODE_IMPLICIT)
				&& !mode.equals(SIGN_MODE_EXPLICIT)) {
			throw new UnsupportedOperationException(
					"El modo de firma '" + mode + "' no esta soportado"); //$NON-NLS-1$ //$NON-NLS-2$
		}
		...
		if (xades) { // XAdES
			...
			if (format.equals(SIGN_FORMAT_XADES_EXTERNALLY_DETACHED)
					&& mode.equals(SIGN_MODE_IMPLICIT)) {
				throw new UnsupportedOperationException(
						"No se puede realizar una firma XML Externally Detached con contenido Implicito" //$NON-NLS-1$
				);
			}
			if (format.equals(SIGN_FORMAT_XADES_EXTERNALLY_DETACHED)
					&& uri == null && externallyDetachedHashAlgorithm == null) {
				throw new UnsupportedOperationException(
			...
			if (!format.equals(SIGN_FORMAT_XADES_DETACHED)
				...
				throw new UnsupportedOperationException(
						"El formato de firma '" + format + "' no esta soportado"); //$NON-NLS-1$ //$NON-NLS-2$
			}
		} else { // XMLDSig
					// Combinaciones prohibidas
			if (format.equals(AOConstants.SIGN_FORMAT_XMLDSIG_ENVELOPED)
					&& mode.equals(AOConstants.SIGN_MODE_EXPLICIT)) {
				throw new UnsupportedOperationException(
						"No se puede realizar una firma enveloped sobre un contenido explícito"); //$NON-NLS-1$
			}
			if (format
					.equals(AOConstants.SIGN_FORMAT_XMLDSIG_EXTERNALLY_DETACHED)
	...
	public static String guessXMLDSigNamespaceURL(final Element el) {
		...
		int numXmlDsig = countSubstring(signatureText,
				"http://www.w3.org/2000/09/xmldsig#");
		int numXmlDsig11 = countSubstring(signatureText,
				"http://www.w3.org/2009/xmldsig11#");

		return numXmlDsig >= numXmlDsig11 ? "http://www.w3.org/2000/09/xmldsig#"
				: "http://www.w3.org/2009/xmldsig11#";
	}
	 ...
	 */
	public static List<Reference> cleanReferencesList(
			...
			if (r.getId() == null) {
				...
				for (Object t : r.getTransforms()) {
					if (t instanceof Transform) {
						if (!"http://www.w3.org/2000/09/xmldsig#base64"
								.equals(((Transform) t).getAlgorithm())) {
				...
				if (needsReconReference) {
					newList.add(fac.newReference(r.getURI(),
							r.getDigestMethod(), trans, r.getType(), r.getId()));
				}
	 ...
	 */
	public final static byte[] writeXML(final Node node,
			Hashtable<String, String> xmlProps, final String styleHref,
		...
		final String xmlEncoding = xmlProps.containsKey(OutputKeys.ENCODING) ? xmlProps
				.get(OutputKeys.ENCODING) : "UTF-8";
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").warning(
					"La codificacion '" + xmlEncoding
							+ "' no es valida, se usara la por defecto: " + e);
		...
		try {
			...
			Logger.getLogger("es.gob.afirma")
					.severe("No se ha podido recargar el XML para insertar los atributos de la cabecera, quizas la codificacion se vea afectada: "
							+ e);
		...
		try {
			return Utils.addStyleSheetHeader(
					new String(baos.toByteArray(), xmlEncoding), styleType,
					styleHref).getBytes(xmlEncoding);
		} catch (final Exception e) {
			Logger.getLogger("es.gob.afirma")
					.warning(
							"La codificacion '"
									+ xmlEncoding
									+ "' no es valida, se usara la por defecto del sistema: "
									+ e);
			return Utils.addStyleSheetHeader(new String(baos.toByteArray()),
					styleType, styleHref).getBytes();
		}
	}

	private final static void writeXMLwithXALAN(final Writer writer,
			final Node node, final String xmlEncoding) {
	...
	private final static void writeXMLwithJRE(final Writer writer,
			final Node node, final boolean indent,
		...
		try {
			...
			// UTF-8
			if (!properties.containsKey(OutputKeys.ENCODING)
					|| "".equals(properties.get(OutputKeys.ENCODING)))
	 ...
	 */
	public static AOSimpleSignInfo getSimpleSignInfoNode(String namespace,
			Element signature) {
		...
		if (namespace != null) {
			try {
				signingTime = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss")
						.parse(((Element) signature.getElementsByTagNameNS(
								namespace, "SigningTime").item(0))
								.getTextContent());
			} catch (Exception e) {
				Logger.getLogger("es.gob.afirma").warning(
						"No se ha podido recuperar la fecha de firma: " + e);
			}
		}

		final AOSimpleSignInfo ssi = new AOSimpleSignInfo(
				new X509Certificate[] { Utils.getCertificate(signature
						.getElementsByTagNameNS(DSIGNNS, "X509Certificate")
						.item(0)) }, signingTime);
		ssi.setSignAlgorithm(((Element) signature.getElementsByTagNameNS(
				DSIGNNS, "SignatureMethod").item(0)).getAttribute("Algorithm"));

		byte[] pkcs1;
		try {
			pkcs1 = AOCryptoUtil.decodeBase64(((Element) signature
					.getElementsByTagNameNS(DSIGNNS, "SignatureValue").item(0))
					.getTextContent());
		} catch (Exception e) {
			Logger.getLogger("es.gob.afirma").warning(
					"No se pudo extraer el PKCS#1 de una firma");
			pkcs1 = null;
		}
		ssi.setPkcs1(pkcs1);
	...
	public static String getStringInfoNode(Element signature) {
		return AOUtil.getCN(Utils.getCertificate(signature
				.getElementsByTagNameNS(DSIGNNS, "X509Certificate").item(0)));
	}
	...
	public static X509Certificate getCertificate(Node certificateNode) {
		return AOCryptoUtil.createCert(certificateNode.getTextContent().trim()
				.replace("\r", "").replace("\n", "").replace(" ", "")
				.replace("\t", ""));
	}
	 ...
	 */
	public static String getCounterSignerReferenceId(Element signature,
			NodeList signatureValues) {
		...
		String uri = ((Element) signature.getElementsByTagNameNS(DSIGNNS,
				"Reference").item(0)).getAttribute("URI").substring(1);
		String signatureId = "";
		for (int j = 0; j < signatureValues.getLength(); j++) {
			...
			if (signatureValue.getAttribute("Id").equals(uri)) {
				signatureId = ((Element) signatureValue.getParentNode())
						.getAttribute("Id");
				break;
			}
		}
