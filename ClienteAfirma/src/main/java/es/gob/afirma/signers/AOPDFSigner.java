 ...
 * <dd><code>true</code> si se desea usar la hora y fecha del sistema como hora
 * y fecha de firma, <code>false</code> en caso contrario
 * <dt>signReason</dt>
 ...
 * <dt>ownerPassword</dt>
 * <dd>Contrase&ntilde;a de apertura del PDF (contrase&ntilde;a del propietario)
 * si este estaba cifrado</dd>
 * <dt>headLess</dt>
 ...
 * <dd>Si se establece a <code>true</code> no cifra los PDF firmados aunque el
 * original estuviese firmado, si no se establece o se establece a
 * <code>false</code> los PDF se cifran tras firmarse si el original lo estaba,
 * usando la misma contrase&ntilde;a y opciones que este</dd>
 * <dt>allowSigningCertifiedPdfs</dt>
 * <dd>Si se establece a <code>true</code> permite la firma o cofirma de PDF
 * certificados, si no se establece o se establece a <code>false</code> se lanza
 * una excepci&oacute;n en caso de intentar firmar o cofirmar un PDF
 * certificado. <b>Solo tiene efecto cuando <code>headLess</code> est&aacute;
 * establecido a <code>true</code>, si <code>headLess</code> est&aacute; a
 * <code>false</code> se ignora este par&aacute;metro.</b>
 * </dl>
 * </p>
 */
public final class AOPDFSigner implements AOSigner {
	...
	public byte[] sign(final byte[] data, final String algorithm,
		...
		try {
			return signPDF(this.getLowagieDigestOid(signAlgorithm), keyEntry,
					data, extraParams, signAlgorithm);
	...
	public byte[] countersign(final byte[] sign, final String algorithm,
		...
		throw new UnsupportedOperationException(
				"No es posible realizar contrafirmas de ficheros PDF"); //$NON-NLS-1$
	}
	...
	public TreeModel getSignersStructure(final byte[] sign,
		...
		try {
			...
			try {
				pdfReader = new PdfReader(
						sign,
						new String(
								new UIPasswordCallback(Messages
										.getString("AOPDFSigner.0"), null).getPassword()).getBytes() //$NON-NLS-1$
				);
		...
		for (int i = 0; i < names.size(); ++i) {
			...
			if (asSimpleSignInfo) {
				AOSimpleSignInfo ssi = new AOSimpleSignInfo(
						new X509Certificate[] { pcks7.getSigningCertificate() },
						pcks7.getSignDate().getTime());

				// Extraemos el PKCS1 de la firma
				try {
					// iText antiguo
					final Field digestField = Class
							.forName("com.lowagie.text.pdf.PdfPKCS7").getDeclaredField("digest"); //$NON-NLS-1$ //$NON-NLS-2$
					// iText nuevo
				...
				root.add(new TreeNode((AOUtil.getCN(pcks7
	...
	private Rectangle getSignaturePositionOnPage(final Properties extraParams) {
		...
		try {
			return new Rectangle(Integer.parseInt(extraParams
					.getProperty("signaturePositionOnPageLowerLeftX")), //$NON-NLS-1$
					Integer.parseInt(extraParams
							.getProperty("signaturePositionOnPageLowerLeftY")), //$NON-NLS-1$
					Integer.parseInt(extraParams
							.getProperty("signaturePositionOnPageUpperRightX")), //$NON-NLS-1$
					Integer.parseInt(extraParams
							.getProperty("signaturePositionOnPageUpperRightY")) //$NON-NLS-1$
			);
		} catch (final Exception e) {
			return null;
		}
	...
	private byte[] signPDF(final String lowagieDigestOid,
			final PrivateKeyEntry ke, final byte[] inPDF,
			Properties extraParams, final String algorithm) throws Exception {

		if (extraParams == null)
			extraParams = new Properties();
		final boolean useSystemDateTime = Boolean.parseBoolean(extraParams
				.getProperty("applySystemDate", "true")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		final String signatureProductionCity = extraParams
				.getProperty("signatureProductionCity"); //$NON-NLS-1$
		final String signerContact = extraParams.getProperty("signerContact"); //$NON-NLS-1$
		final String policyIdentifier = extraParams
				.getProperty("policyIdentifier"); //$NON-NLS-1$
		final String policyQualifier = extraParams
				.getProperty("policyQualifier"); //$NON-NLS-1$
		int page = 1;
		try {
			page = Integer.parseInt(extraParams.getProperty("signaturePage")); //$NON-NLS-1$
		} catch (final Exception e) { /* Se deja la pagina tal y como esta */
		}
		...
		if (extraParams.containsKey("mode") //$NON-NLS-1$
				&& extraParams
						.getProperty("mode").equals(AOConstants.SIGN_MODE_EXPLICIT)) { //$NON-NLS-1$
			Logger.getLogger("es.gob.afirma").warning( //$NON-NLS-1$
		...
		try {
			...
			ownerPassword = new String(new UIPasswordCallback(
					Messages.getString("AOPDFSigner.0"), null).getPassword()); //$NON-NLS-1$
			try {
				pdfReader = new PdfReader(inPDF, ownerPassword.getBytes());
			} catch (final BadPasswordException e2) {
				throw new AOException(
						"La contrasena proporcionada no es valida para el PDF actual"); //$NON-NLS-1$
			}
		}
		...
		if (pdfReader.getCertificationLevel() != PdfSignatureAppearance.NOT_CERTIFIED) {
			if ("true".equalsIgnoreCase(extraParams.getProperty("headLess"))) { //$NON-NLS-1$ //$NON-NLS-2$
				...
				if (JOptionPane.NO_OPTION == JOptionPane.showConfirmDialog(
						null, Messages.getString("AOPDFSigner.8"), //$NON-NLS-1$
		...
		for (int i = 1; i <= pdfReader.getNumberOfPages(); i++) {
			...
			for (int j = 0; j < array.size(); j++) {
				...
				if (annot != null
						&& PdfName.FILEATTACHMENT.equals(annot
		...
		if (catalog != null) {
			...
			if (namesCatalog != null) {
				final PdfDictionary filesCatalog = namesCatalog
						.getAsDict(PdfName.EMBEDDEDFILES);
				if (filesCatalog != null) {
					final PdfArray filespecs = filesCatalog
		...
		// documento
		// y evita que las firmas previas queden invalidadas. Sin embargo, este
		// exige que
		// el PDF no incorpore ningun error, asi que lo mantendremos desactivado
		// para la
		// primera firma y activado para las subsiguientes. Un error incorporado
		// en un PDF
		// erroneo puede quedar subsanado en su version firmada, haciendo
		// posible incorporar
		// nuevas firmas agregando revisiones del documento.
		final PdfStamper stp = PdfStamper.createSignature(pdfReader, // PDF de
																		// entrada
				baos, // Salida
				'\0', // Mantener version
				null, // No crear temporal
				pdfReader.getAcroFields().getSignatureNames().size() > 0 // Si
																			// hay
																			// mas
																			// firmas,
																			// creo
		...
		if (pdfReader.isEncrypted() && ownerPassword != null) {
			if ("true".equalsIgnoreCase(extraParams.getProperty("avoidEncryptingSignedPdfs"))) { //$NON-NLS-1$ //$NON-NLS-2$
				...
				try {
					stp.setEncryption(ownerPassword.getBytes(),
		...
		if (chain == null)
			chain = new Certificate[] { ke.getCertificate() };
		...
		dic.setName(PdfPKCS7.getSubjectFields((X509Certificate) chain[0])
		...
		try {
			...
			try {
				...
				// Si falla la conversion a OID, se queda como null y no se
				// insertara en la firma
			}
			...
			pk = new GenCadesEPESSignedData().generateSignedData(
					...
					policyIdentifier,
					policyQualifier2,
					true, // true -> isSigningCertificateV2, false ->
							// isSigningCertificateV1
					dataType,
					ke,
					MessageDigest.getInstance(
							AOCryptoUtil.getDigestAlgorithmName(algorithm))
							.digest(AOUtil.getDataFromInputStream(sap
									.getRangeStream())));
	 ...
	 * 
	 * @param rubric
	 *            Imagen de la r&uacute;brica.
	 */
	public void setRubric(final BufferedImage rubric) {
		this.rubric = rubric;
	}

	private static com.lowagie.text.Image javaImage2IText(
			final RenderedImage inImage) throws AOException {

		final ByteArrayOutputStream imgbaos = new ByteArrayOutputStream();
		final MemoryCacheImageOutputStream img = new MemoryCacheImageOutputStream(
				imgbaos);
		...
		try {
			...
			iwp.setCompressionQuality(iwp.getCompressionQualityValues()[iwp
					.getCompressionQualityValues().length - 1]);
	...
	public byte[] getData(final byte[] sign) throws AOInvalidFormatException {
		...
		if (!isSign(sign)) {
			throw new AOInvalidFormatException(
					"El documento introducido no contiene una firma valida"); //$NON-NLS-1$
		}
	...
	public AOSignInfo getSignInfo(final byte[] data)
	...
	public String getDataMimeType(final byte[] sign)
		...
		if (sign == null)
			throw new NullPointerException(
					"Los datos de firma introducidos son nulos"); //$NON-NLS-1$

		if (!this.isSign(sign))
			throw new AOUnsupportedSignFormatException(
					"La firma introducida no es un fichero de firma PDF"); //$NON-NLS-1$
