...
public final class AOPDFSigner implements AOSigner {
    ...
    public byte[] sign(final byte[] data, final String algorithm, final PrivateKeyEntry keyEntry, Properties extraParams) throws AOException {
        ...
        if (!algorithm.equals(AOConstants.SIGN_ALGORITHM_MD5WITHRSA) && !algorithm.equals(AOConstants.SIGN_ALGORITHM_SHA1WITHRSA)
    ...
    public TreeModel getSignersStructure(final byte[] sign, final boolean asSimpleSignInfo) {
        ...
        try {
            ...
            try {
                pdfReader =
                        new PdfReader(sign, new String(new UIPasswordCallback(Messages.getString("AOPDFSigner.0"), null).getPassword()).getBytes() //$NON-NLS-1$
                        );
            }
    ...
    private byte[] signPDF(final String lowagieDigestOid, final PrivateKeyEntry ke, final byte[] inPDF, Properties extraParams, final String algorithm) throws Exception {
        ...
        if (pdfReader.getCertificationLevel() != PdfSignatureAppearance.NOT_CERTIFIED) {
            if ("true".equalsIgnoreCase(extraParams.getProperty("headLess"))) { //$NON-NLS-1$ //$NON-NLS-2$
                ...
                if (JOptionPane.NO_OPTION == JOptionPane.showConfirmDialog(null, Messages.getString("AOPDFSigner.8"), //$NON-NLS-1$
                    ...
                    throw new OperationNotSupportedException("No se ha permitido la firma de un PDF certificado" //$NON-NLS-1$
                    );
                }
            }
        }
        ...
        try {
            ...
            try {
                policyQualifier2 = new Oid(policyQualifier.replace("urn:oid:", "").replace("URN:oid:", "").replace("Urn:oid:", ""));
            }
            catch (final Exception e) {
            ...
            pk =
                    new GenCadesEPESSignedData().generateSignedData(new P7ContentSignerParameters(inPDF, algorithm, xCerts), true, // omitContent
                                                                    policyIdentifier,
                                                                    policyQualifier2,
                                                                    ...
                                                                    dataType,
                                                                    ke,
                                                                    MessageDigest.getInstance(AOCryptoUtil.getDigestAlgorithmName(algorithm))
    ...
    private static com.lowagie.text.Image javaImage2IText(final RenderedImage inImage) throws AOException {
        ...
        try {
            ...
            throw new AOException("No se pudo crear la imagen iText a partir de la imagen Java", e //$NON-NLS-1$
            );
        }
    }
