 ...
 * fragmentos de las clases <code>com.sun.deploy.config.UnixConfig</code> y
 * <code>com.sun.deploy.config.WinConfig</code>
 * 
 * @version 0.3
 */
public final class AOKeyStoreManagerFactory {
	 ...
	 *            Debe seguir el formato definido en el m&eacute;todo
	 *            <code>toString()</code> de la clase
	 *            <code>sun.security.pkcs11.Secmod.Module</code>
	 * @param pssCallback
	 ...
	 */
	public static AOKeyStoreManager getAOKeyStoreManager(
			final AOConstants.AOKeyStore store, final String lib,
			final String description, final PasswordCallback pssCallback,
		...
		if (store == AOConstants.AOKeyStore.PKCS12
			...
			if (storeFilename == null) {
				...
				storeFilename = AOUIManager.getLoadFileName(
						"Abrir repositorio " + store.getDescription(), exts,
						desc, parentComponent);
				if (storeFilename == null)
					throw new AOCancelledOperationException(
							"No se ha seleccionado el almac\u00E9n de certificados");
			}
			...
			try {
				...
				throw new AOKeystoreAlternativeException(
						...
						"No se ha podido abrir el almacen de tipo "
								+ store.getDescription(), e);
			}
			return ksm;
		}
		// Token PKCS#11, en cualquier sistema operativo
			...
			if (lib != null && !"".equals(lib) && new File(lib).exists())
				p11Lib = lib;
			if (p11Lib == null)
				p11Lib = AOUIManager.getLoadFileName(
						"Seleccionar biblioteca PKCS#11", new String[] { "dll",
								"so" }, "Bibliotecas PKCS#11 (*.dll, *.so)",
						parentComponent);
			if (p11Lib == null)
				throw new AOCancelledOperationException(
						"No se ha seleccionado el controlador PKCS#11");
			if (Platform.OS.WINDOWS.equals(Platform.getOS())) {
				try {
					...
					Logger.getLogger("es.gob.afirma").warning(
							"No se ha podido obtener informacion sobre el modulo PKCS11 '"
									+ p11Lib + "':" + e);
				}
			}
			try {
				ksm.init(store, null, pssCallback, new String[] { p11Lib,
						description });
			} catch (final Exception e) {
				throw new AOKeystoreAlternativeException(
						getAlternateKeyStoreType(store),
						"Error al inicializar el modulo PKCS#11", e);
			}
			return ksm;
		}
		...
		// o Google Chrome en Windows, que tambien usa el almacen de CAPI
				...
				&& (store == AOConstants.AOKeyStore.WINDOWS
						...
						|| store == AOConstants.AOKeyStore.WINADDRESSBOOK || store == AOConstants.AOKeyStore.WINCA /*
																													 * ||
																													 * store
																													 * ==
																													 * AOConstants
			...
			try {
				...
				throw new AOKeystoreAlternativeException(
						getAlternateKeyStoreType(store),
						"Error al inicializar el almacen "
								+ store.getDescription(), e);
			}
			return ksm;
		}
		...
		// Mozilla / Firefox en cualquier plataforma, via PKCS#11 y NSS
			...
			Logger.getLogger("es.gob.afirma").info(
					"Solicitado almacen unificado de Mozilla Firefox");
		...
		// http://developer.apple.com/technotes/tn2002/tn2110.html
				...
				&& store == AOConstants.AOKeyStore.APPLE) {
			try {
				ksm.init(store, null, pssCallback, null);
			} catch (final Exception e) {
				throw new AOKeystoreAlternativeException(
						getAlternateKeyStoreType(store),
						"Error al inicializar el Llavero de Mac OS X", e);
			}
			return ksm;
		}

		throw new AOKeystoreAlternativeException(
				getAlternateKeyStoreType(store), "La plataforma de navegador '"
						+ store.getDescription() + "' mas sistema operativo '"
						+ Platform.getOS() + " (" + Platform.getOsVersion()
						+ ")' mas Java '" + Platform.getJavaVersion()
						+ "' no esta soportada");
	}

	/**
	 * @return <code>AOConstants.AOKeyStore</code> alternativo o
	 *         <code>null</code> si no hay alternativo
	 */
	private static AOConstants.AOKeyStore getAlternateKeyStoreType(
			final AOConstants.AOKeyStore currentStore) {
		if (AOConstants.AOKeyStore.PKCS12.equals(currentStore))
			return null;
		if (Platform.OS.WINDOWS.equals(Platform.getOS())
				&& (!AOConstants.AOKeyStore.WINDOWS.equals(currentStore)))
			return AOConstants.AOKeyStore.WINDOWS;
		if (Platform.OS.MACOSX.equals(Platform.getOS())
				&& (!AOConstants.AOKeyStore.APPLE.equals(currentStore)))
