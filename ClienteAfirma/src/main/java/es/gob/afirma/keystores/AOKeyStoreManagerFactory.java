...
public final class AOKeyStoreManagerFactory {
    ...
    public static AOKeyStoreManager getAOKeyStoreManager(final AOConstants.AOKeyStore store,
        ...
        if (store == AOConstants.AOKeyStore.PKCS12 || store == AOConstants.AOKeyStore.JAVA
        ...
        // Token PKCS#11, en cualquier sistema operativo
            ...
            if (Platform.OS.WINDOWS.equals(Platform.getOS())) {
                try {
                    ...
                    Logger.getLogger("es.gob.afirma").info("Informacion sobre el modulo PKCS11 '" + p11Lib + "':");
                    ...
                    Logger.getLogger("es.gob.afirma").warning("No se ha podido obtener informacion sobre el modulo PKCS11 '" + p11Lib + "':" + e);
                }
            }
            try {
                ...
                throw new AOKeystoreAlternativeException(getAlternateKeyStoreType(store), "Error al inicializar el modulo PKCS#11", e);
            }
            return ksm;
        }
        ...
        else if (Platform.getOS().equals(Platform.OS.WINDOWS) && (store == AOConstants.AOKeyStore.WINDOWS || store == AOConstants.AOKeyStore.WINROOT
        ...
        // Mozilla / Firefox en cualquier plataforma, via PKCS#11 y NSS
            ...
            try {
                ...
                ksm.init(store, null, pssCallback, new String[] {
                        MozillaKeyStoreUtilities.getMozillaUserProfileDirectory(), nssLibDir
                });
            }
            catch (final Exception e) {
                throw new AOKeystoreAlternativeException(getAlternateKeyStoreType(store), "Error al inicializar el almacen NSS de Mozilla Firefox", e);
            }
            return ksm;
        }

        else if (store == AOConstants.AOKeyStore.MOZ_UNI) {
            Logger.getLogger("es.gob.afirma").info("Solicitado almacen unificado de Mozilla Firefox");
        ...
        // http://developer.apple.com/technotes/tn2002/tn2110.html
            ...
            try {
                ...
                throw new AOKeystoreAlternativeException(getAlternateKeyStoreType(store), "Error al inicializar el Llavero de Mac OS X", e);
            }
            return ksm;
        }

        throw new AOKeystoreAlternativeException(getAlternateKeyStoreType(store), "La plataforma de navegador '" + store.getDescription()
                                                                                  + "' mas sistema operativo '"
                                                                                  + Platform.getOS()
                                                                                  + " ("
