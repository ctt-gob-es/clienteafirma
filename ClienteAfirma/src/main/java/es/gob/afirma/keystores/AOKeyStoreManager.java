...
public class AOKeyStoreManager {
	...
	public Vector<KeyStore> init(AOConstants.AOKeyStore type,
			final InputStream store, PasswordCallback pssCallBack,
			final Object[] params) throws AOException {

		Logger.getLogger("es.gob.afirma").info(
				"Inicializamos el almacen de tipo: " + type);

		final Vector<KeyStore> ret = new Vector<KeyStore>(1);

		if (type == null) {
			Logger.getLogger("es.gob.afirma")
					.severe("Se ha solicitado inicializar un AOKeyStore nulo, se intentara inicializar un PKCS#12");
		...
		if (type == AOConstants.AOKeyStore.SINGLE) {
			...
			if (store == null)
				throw new AOException(
						"Es necesario proporcionar el fichero X.509 o PKCS#7");
			...
			try {
				...
				throw new AOKeyStoreManagerException(
						"No se ha podido obtener el almacen " + type.getName()
								+ " solicitado", e);
			}
			...
			try {
				...
				throw new AOKeyStoreManagerException(
						"No se ha podido abrir el almacen " + type.getName()
								+ " solicitado", e);
			}
			ret.add(ks);
			try {
			...
			if (store == null)
				throw new AOException(
						"Es necesario proporcionar el fichero KeyStore");

			try {
				ks = KeyStore.getInstance(type.getName());
			} catch (final Exception e) {
				throw new AOKeyStoreManagerException(
						"No se ha podido obtener el almacen de nombre '"
								+ type.getName() + "'", e);
			}
			...
			if (store == null)
				throw new AOException(
						"Es necesario proporcionar el fichero PKCS12 / PFX");
		...
		// }
		...
		else if (type == AOConstants.AOKeyStore.WINDOWS
				|| type == AOConstants.AOKeyStore.WINROOT) {
			...
			if (sunMSCAPIProvider == null) {
				try {
					sunMSCAPIProvider = (Provider) Class.forName(
							"sun.security.mscapi.SunMSCAPI").newInstance();
					Security.insertProviderAt(sunMSCAPIProvider, 1);
				} catch (final Exception e) {
					Logger.getLogger("es.gob.afirma").warning(
							"No se ha podido instanciar el proveedor 'sun.security.mscapi.SunMSCAPI': "
									+ e);
				}
			}
			...
			try {
				...
				throw new AOKeyStoreManagerException(
						"No se ha podido obtener el KeyStore de nombre '"
								+ type.getName()
								+ "' del Provider 'sun.security.mscapi.SunMSCAPI'",
						e);
			}

			Logger.getLogger("es.gob.afirma").info(
					"Cargando KeyStore de Windows.");
			try {
				ks.load(store, pssCallBack.getPassword());
			} catch (final AOCancelledOperationException e) {
				throw e;
				...
				throw new AOKeyStoreManagerException(
						"No se ha podido inicializar el KeyStore de nombre '"
								+ type.getName()
								+ "' del Provider 'sun.security.mscapi.SunMSCAPI'",
						e);
			}
		...
		else if (type == AOConstants.AOKeyStore.WINCA
				|| type == AOConstants.AOKeyStore.WINADDRESSBOOK) {

			if (!Platform.getOS().equals(Platform.OS.WINDOWS)) {
				throw new AOKeyStoreManagerException(
						"No se puede obtener el KeyStore ADDRESSBOOK de Windows en un sistema no Windows:"
								+ Platform.getOS());
			}
			...
			if (Security.getProvider("SunMSCAPI") == null) {
				try {
					...
					throw new AOKeyStoreManagerException(
							"No se ha podido obtener el KeyStore de nombre '"
									+ type.getName()
									+ "' por no poderse cargar el Provider 'sun.security.mscapi.SunMSCAPI'",
							e);
				}
			}
			...
			if (p == null) {
				try {
					p = (Provider) Class.forName(
							"es.gob.afirma.keystores.SunMSCAPIAddressBook")
							.newInstance();
				} catch (final Exception e) {
					throw new AOException(
							"Error obteniendo el almacen '"
									+ type.getName()
									+ "' por no poder instanciarse el proveedor SunMSCAPIAddressBook",
							e);
				}
			...
			try {
				...
				throw new AOException("Error obteniendo el almacen '"
						+ type.getName()
						+ "' del proveedor SunMSCAPIAddressBook", e);
			}
			...
			try {
				...
				throw new AOException(
						"Error inicializando el almacen de claves y certificados del proveedor SunMSCAPIAddressBook",
						e);
			}
			...
			final String p11ProviderName = new File(p11lib).getName()
					.replace('.', '_').replace(' ', '_');
			...
			if (p11Provider == null) {
				...
				try {
					sunPKCS11Contructor = Class.forName(
							"sun.security.pkcs11.SunPKCS11").getConstructor(
							InputStream.class);
				} catch (Exception e) {
					throw new AOKeyStoreManagerException(
							"No se ha podido construir un proveedor de tipo 'sun.security.pkcs11.SunPKCS11'",
							e);
				}

				byte[] config = KeyStoreUtilities.createPKCS11ConfigFile(
						p11lib, p11ProviderName, slot).getBytes();
				try {
					...
					try {
						p11Provider = (Provider) sunPKCS11Contructor
								.newInstance(new ByteArrayInputStream(config));
					} catch (final Exception ex) {
						throw new AOException(
								"Error instanciando el proveedor SunPKCS11 para la la biblioteca "
										+ p11lib, ex);
					}
				}
				try {
					...
					throw new AOException(
							"Error instalando el proveedor SunPKCS11 para la la biblioteca "
									+ p11lib, e);
				}
			} else {
				Logger.getLogger("es.gob.afirma")
						.info("El proveedor PKCS#11 solicitado ya estaba instanciado, se reutilizara esa instancia: "
								+ p11Provider.getName());
			}
			...
			try {
				...
				throw new AOKeyStoreManagerException(
						"No se ha podido obtener el KeyStore de nombre '"
								+ type.getName()
								+ "' del Provider 'sun.security.pkcs11.SunPKCS11'",
						e);
			}
			...
			if (nssProvider == null) {
				...
				try {
					...
					// NSSLibDir).getBytes()));
					Constructor<?> sunPKCS11Constructor = Class.forName(
							"sun.security.pkcs11.SunPKCS11").getConstructor(
							InputStream.class);
					nssProvider = (Provider) sunPKCS11Constructor
							.newInstance(new ByteArrayInputStream(
									MozillaKeyStoreUtilities
											.createPKCS11NSSConfigFile(
													mozillaProfileDir,
													nssLibDir).getBytes()));
				} catch (final Exception e) {
					throw new AOException(
							"No se ha podido instanciar la clase SunPKCS11, se abortara la operacion",
							e);
				}
			...
			try {
				...
				throw new AOException(
						"No se ha podido instanciar el proveedor 'com.apple.crypto.provider.Apple'",
						e);
			}
			...
			try {
				...
				throw new AOKeyStoreManagerException(
						"No se ha podido obtener el KeyStore de nombre '"
								+ type.getName()
								+ "' del Provider 'com.apple.crypto.provider.Apple'",
						e);
			}
	...
	public KeyStore.PrivateKeyEntry getKeyEntry(final String alias,
			PasswordCallback pssCallback) throws AOCancelledOperationException,
		...
		if (ks == null)
			throw new NullPointerException(
					"Se han pedido claves a un almacen no inicializado");
		...
		if ("KeychainStore".equals(ks.getType())) {
			try {
				Logger.getLogger("es.gob.afirma")
						.info("Detectado almacen Llavero de Mac OS X, se trataran directamente las claves privadas");
				...
				if (certChain == null) {
					...
					certChain = new Certificate[] { ks.getCertificate(alias) };
				}

				keyEntry = new KeyStore.PrivateKeyEntry((PrivateKey) ks.getKey(
						alias, "dummy".toCharArray()), certChain);
			} catch (final Exception e) {
				throw new AOCertificateKeyException(
						"Error intentando obtener la clave privada en Mac OS X",
						e);
			}
		} else {
			try {
				keyEntry = (KeyStore.PrivateKeyEntry) ks.getEntry(
						alias,
						new KeyStore.PasswordProtection(pssCallback
								.getPassword()));
			} catch (final AOCancelledOperationException e) {
				throw e;
			} catch (final Exception e) {
				throw new AOCertificateKeyException(
						"Error intentando obtener la clave privada", e);
			}
		}
	...
	public X509Certificate getCertificate(
		...
		Logger.getLogger("es.gob.afirma")
				.severe("El certificado solicitado no era de tipo X509Certificate, se devolvera null");
		return null;
	}
	...
	public X509Certificate getCertificate(final String alias) {
		if (alias == null) {
			Logger.getLogger("es.gob.afirma").warning(
					"El alias del certificado es nulo, se devolvera null");
			return null;
		}
		...
		if (this.ks == null) {
			Logger.getLogger("es.gob.afirma")
					.warning(
							"No se ha podido recuperar el certificado con alias '"
									+ alias
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").warning(
					"No se ha podido recuperar el certificado con alias '"
							+ alias + "', se devolvera null: " + e);
			return null;
		}
		if (cert == null) {
			Logger.getLogger("es.gob.afirma").warning(
					"No se ha podido recuperar el certificado con alias '"
							+ alias + "', se devolvera null");
			return null;
		}
		if (cert instanceof X509Certificate)
			return (X509Certificate) cert;
		...
		Logger.getLogger("es.gob.afirma").warning(
				"El certificado con alias '" + alias
						+ "' no es de tipo X509Certificate, se devolvera null");
		return null;
	...
	public X509Certificate[] getCertificateChain(
		...
		Logger.getLogger("es.gob.afirma")
				.severe("No se ha podido obtener la cadena de certificados, se devolvera una cadena vacia");
	 ...
	 * @param alias
	 *            Alias del certificado.
	 * @return Certificados de la cadena de certificaci&oacute;n o {@code null}
	 *         si no se pudo recuperar.
	 */
	public X509Certificate[] getCertificateChain(final String alias) {
		if (ks == null) {
			Logger.getLogger("es.gob.afirma")
					.warning(
							"El KeyStore actual no esta inicializado, por lo que no se pudo recuperar el certificado para el alias '"
									+ alias + "'");
			return null;
		}
		try {
			...
			Logger.getLogger("es.gob.afirma").severe(
					"Error al obtener la cadena de certificados para el alias '"
							+ alias + "', se devolvera una cadena vacia: " + e);
			return new X509Certificate[0];
		}
		Logger.getLogger("es.gob.afirma").severe(
				"No se ha podido obtener la cadena de certificados para el alias '"
						+ alias + "', se devolvera una cadena vacia");
		return new X509Certificate[0];
	}
	...
	public String[] getAliases() {
		...
		if (ks == null)
			throw new NullPointerException(
					"Se han pedido los alias de un almacen no inicializado");

		Logger.getLogger("es.gob.afirma").info(
				"Solicitando los alias al KeyStore (" + ks.getProvider() + ")");
		...
		try {
			...
			Logger.getLogger("es.gob.afirma").severe(
					"Error intentando obtener los alias del almacen de claves, se devolvera "
		...
		Logger.getLogger("es.gob.afirma")
				.info("Componiendo el vector de alias");
	 ...
	 * existe un keystore con la descripci&oacute;n indicada, se devuleve
	 * <code>null</code>.
	 * 
	 * @param description
