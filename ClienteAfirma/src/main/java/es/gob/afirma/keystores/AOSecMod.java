...
final class AOSecMod {
	 ...
	 * 
	 */
	private static ModuleName processNames(final byte[] secmoddb,
		...
		if (libName.endsWith(".DLL") || libName.endsWith(".dll")
				|| libName.endsWith(".so") || libName.endsWith(".dylib")) {

			// namesRunningOffset += len + 2;

			String trueLibName = searchPathForFile(new String[] { libName },
					false);
	...
	static Vector<ModuleName> getModules(String profileDir) throws AOException {
		if (profileDir == null || "".equals(profileDir)) {
			throw new AOException(
					"El directorio del perfil de Mozilla era nulo");
		}
		...
		if (modules == null) {
			...
			if (!secmod.exists()) {
				throw new AOException(
						"El directorio del perfil de Mozilla proporcionado no contiene una base de datos de modulos (secmod.db)");
			}
			final byte[] secMod;
			try {
				secMod = AOUtil
						.getDataFromInputStream(AOUtil.loadFile(
								AOUtil.createURI(secmod.getAbsolutePath()),
								null, false));
			} catch (Exception e) {
				throw new AOException(
						"Error leyendo la base de datos de modulos (secmod.db)");
			}
	 ...
	 *            b&uacute;squeda
	 * @return Ruta completa del fichero encontrado en el PATH o
	 *         <code>null</code> si no se encontr&oacute; nada
	 */
	private final static String searchPathForFile(final String[] files,
			final boolean excludeAFirma) {
		...
		final StringTokenizer st = new StringTokenizer(
				Platform.getJavaLibraryPath(), File.pathSeparator);
		String libPath;
