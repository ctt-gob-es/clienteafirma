...
class AOJarVerifier {
	...
	void verifyJar(final String jarName, final X509Certificate caCert, final X509Certificate signerCert) throws SecurityException {
		...
		if (caCert == null) throw new SecurityException(
		...
		try {
							...
							if (cert instanceof  X509Certificate) {
								...
								if (signerCert != null) {
									...
									if (!Arrays.equals(signerCert.getPublicKey().getEncoded(), cert.getPublicKey().getEncoded())) 
										...
										"El certificado firmante era de una CA valida, pero no se corresponde con el indicado"
									);
								}
							}
							else throw new SecurityException(
								"El codigo se ha firmado con un certificado que no cumple la norma X.509"
							);
						}
					} // if isSigned
			...
			if (!anySigned) throw new SecurityException(
				"El fichero ZIP/JAR no esta firmado (faltan firmas o no son procesables)"
			);
			
			if (hasUnsignedEntry) throw new SecurityException(
				"Hay entradas sin firmar en el fichero ZIP/JAR"
			);
			
            if (hasExpiringCert) Logger.getLogger("es.gob.afirma").warning(
    			"El fichero ZIP/JAR contiene entradas firmadas con un certificado que caduca en los proximos meses"
			);
            
            if (hasExpiredCert) Logger.getLogger("es.gob.afirma").warning(
        		"El fichero ZIP/JAR contiene entradas firmadas con un certificado caducado"
    		);
            
            if (notYetValidCert) Logger.getLogger("es.gob.afirma").warning(
				"El fichero ZIP/JAR contiene entradas firmadas con un certificado aun no valido"
			);
