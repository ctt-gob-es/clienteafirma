...
/**
 * Testeo de accesibilidad para la clase Ensobrado.
 * @author lmerayo
 *
 */
public class EnsobradoAccessibilityTest {

	/**
	 * Log.
	 */
	static Logger logger = Logger.getLogger(EnsobradoAccessibilityTest.class.getName());
	...
	public void testNotDuplicatedLabelForProperty() {
		...
		try {
			...
			final Ensobrado ensobradoPanel = new Ensobrado();
			...
			final Component[] components = ensobradoPanel.getComponents();
	...
	public void testNotDuplicatedDisplayedMnemonic() {
		...
		try {
			...
			getKeyCodeList (ensobradoPanel, keyCodes);
	...
	public void testNotEmptyAccessibleName() {
		...
		try {
			//Instancia del panel que se va a analizar
			final Ensobrado ensobrado = new Ensobrado();
			//Se llama al metodo que comprueba que el nombre no sea vacio
			assertTrue(checkAccessibleName(ensobrado));
		}
		catch(final java.awt.HeadlessException e) {
			// Ignoramos este error, pero no otros, para evitar fallos en tests automaticos en servidor
		}
