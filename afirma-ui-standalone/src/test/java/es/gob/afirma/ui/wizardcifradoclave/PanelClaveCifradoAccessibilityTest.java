...
/**
 * Testeo de accesibilidad para la clase PanelClaveCifrado.
 * @author lmerayo
 *
 */
public class PanelClaveCifradoAccessibilityTest {

	/**
	 * Log.
	 */
	static Logger logger = Logger.getLogger(PanelClaveCifradoAccessibilityTest.class.getName());
	...
	public void testNotDuplicatedLabelForProperty() {
		...
		PanelClaveCifrado panelClaveCifrado = new PanelClaveCifrado("", ""); //$NON-NLS-1$ //$NON-NLS-2$
		...
		Component[] components = panelClaveCifrado.getComponents();
		...
		//Si el tamano de la lista y del conjunto son iguales, no hay duplicados
		assertTrue(componentList.size() == componentSet.size());
