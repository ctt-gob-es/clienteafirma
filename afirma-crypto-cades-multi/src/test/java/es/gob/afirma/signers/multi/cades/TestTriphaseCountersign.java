...
public class TestTriphaseCountersign {
	...
	public void prueba_contrafirma_de_arbol_de_firma_explicita() throws Exception {
		...
		final InputStream is = AOUtil.getCleanClassLoader().getResourceAsStream(EXPLICIT_SHA1_COUNTERSIGN_FILE);
	...
	public void prueba_contrafirma_de_arbol_de_firma_implicita() throws Exception {
		...
		final InputStream is = AOUtil.getCleanClassLoader().getResourceAsStream(IMPLICIT_SHA1_COUNTERSIGN_FILE);
		final byte[] sign = AOUtil.getDataFromInputStream(is);
		is.close();
