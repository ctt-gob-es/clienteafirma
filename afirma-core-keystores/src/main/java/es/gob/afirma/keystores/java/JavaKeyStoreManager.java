...
package es.gob.afirma.keystores.java;
...
public final class JavaKeyStoreManager extends AOKeyStoreManager {
	...
	/** Inicializa un almac&eacute;n de claves y certificados de tipo JKS.
	...
	private KeyStore init(final InputStream store,
		...
		if (store == null) {
            throw new IllegalArgumentException("Es necesario proporcionar el fichero JKS"); //$NON-NLS-1$
        }
        ...
        try {
            ...
            throw new AOKeyStoreManagerException("No se ha podido obtener el almacen JKS", e); //$NON-NLS-1$
        }
        ...
        try {
            ...
            throw new AOKeyStoreManagerException("No se han podido cargar los certificados del almacen JKS.", e); //$NON-NLS-1$
        }
        catch (final NoSuchAlgorithmException e) {
            throw new AOKeyStoreManagerException("No se ha podido verificar la integridad del almacen JKS.", e); //$NON-NLS-1$
		}
        try {
            store.close();
        }
	...
	public AOKeyStore getType() {
        return AOKeyStore.JAVA;
    }

    /** Obtiene la clave privada de un certificado en un almac&eacute;n JKS.
