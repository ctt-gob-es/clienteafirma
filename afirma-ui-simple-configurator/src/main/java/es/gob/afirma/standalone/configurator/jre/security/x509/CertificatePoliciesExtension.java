...
implements CertAttrSet<String> {
    ...
    private void encodeThis() throws IOException {
        if (this.certPolicies == null || this.certPolicies.isEmpty()) {
            ...
            for (final PolicyInformation info : this.certPolicies) {
                info.encode(tmp);
            }
     ...
     */
    public CertificatePoliciesExtension(final List<PolicyInformation> certPolicies)
    throws IOException {
     ...
     * @param certPolicies the List of PolicyInformation.
     */
    public CertificatePoliciesExtension(final Boolean critical,
            final List<PolicyInformation> certPolicies) throws IOException {
     ...
     */
    public CertificatePoliciesExtension(final Boolean critical, final Object value)
    throws IOException {
        this.extensionId = PKIXExtensions.CertificatePolicies_Id;
        this.critical = critical.booleanValue();
        ...
        while (val.data.available() != 0) {
            ...
            final PolicyInformation policy = new PolicyInformation(seq);
    ...
    public void set(final String name, final Object obj) throws IOException {
        if (name.equalsIgnoreCase(POLICIES)) {
            ...
            this.certPolicies = (List<PolicyInformation>)obj;
        } else {
     ...
     */
    @Override
	public List<PolicyInformation> get(final String name) throws IOException {
        if (name.equalsIgnoreCase(POLICIES)) {
