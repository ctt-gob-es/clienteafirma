...
public class GeneralSubtrees implements Cloneable {
    ...
    private GeneralSubtrees(final GeneralSubtrees source) {
        this.trees = new ArrayList<GeneralSubtree>(source.trees);
    }
     ...
     */
    public GeneralSubtrees(final DerValue val) throws IOException {
        this();
        ...
        while (val.data.available() != 0) {
            ...
            final GeneralSubtree tree = new GeneralSubtree(opt);
            add(tree);
        }
    }

    public GeneralSubtree get(final int index) {
    ...
    public void add(final GeneralSubtree tree) {
        if (tree == null) {
    ...
    public boolean contains(final GeneralSubtree tree) {
        if (tree == null) {
            throw new NullPointerException();
        }
	...
	public String toString() {
        final String s = "   GeneralSubtrees:\n" + this.trees.toString() + "\n";
        return s;
    }
	...
	public boolean equals(final Object obj) {
        ...
        final GeneralSubtrees other = (GeneralSubtrees)obj;
     ...
     */
    private GeneralNameInterface getGeneralNameInterface(final int ndx) {
        return getGeneralNameInterface(get(ndx));
    }

    private static GeneralNameInterface getGeneralNameInterface(final GeneralSubtree gs) {
        final GeneralName gn = gs.getName();
        final GeneralNameInterface gni = gn.getName();
        return gni;
    }
    ...
    private void minimize() {
        ...
        for (int i = 0; i < size(); i++) {
            final GeneralNameInterface current = getGeneralNameInterface(i);
            boolean remove1 = false;

            /* compare current to subsequent elements */
            for (int j = i + 1; j < size(); j++) {
                final GeneralNameInterface subsequent = getGeneralNameInterface(j);
     ...
     */
    private GeneralSubtree createWidestSubtree(final GeneralNameInterface name) {
        try {
            GeneralName newName;
            switch (name.getType()) {
            case GeneralNameInterface.NAME_ANY:
                ...
                // empty value
                final ObjectIdentifier otherOID = ((OtherName)name).getOID();
     ...
     */
    public GeneralSubtrees intersect(final GeneralSubtrees other) {

        if (other == null) {
            throw new NullPointerException("other GeneralSubtrees must not be null");
        }

        final GeneralSubtrees newThis = new GeneralSubtrees();
        ...
        // same type in this.
        for (int i = 0; i < size(); i++) {
            final GeneralNameInterface thisEntry = getGeneralNameInterface(i);
            final boolean removeThisEntry = false;
            ...
            for (int j = 0; j < other.size(); j++) {
                final GeneralSubtree otherEntryGS = other.get(j);
                final GeneralNameInterface otherEntry =
            ...
            if (sameType) {
                ...
                for (int j = 0; j < size(); j++) {
                    final GeneralNameInterface thisAltEntry = getGeneralNameInterface(j);

                    if (thisAltEntry.getType() == thisEntry.getType()) {
                        for (int k = 0; k < other.size(); k++) {
                            final GeneralNameInterface othAltEntry =
                ...
                if (intersection == false) {
                    ...
                    final GeneralSubtree widestSubtree =
        ...
        for (int i = 0; i < other.size(); i++) {
            final GeneralSubtree otherEntryGS = other.get(i);
            final GeneralNameInterface otherEntry = getGeneralNameInterface(otherEntryGS);
            boolean diffType = false;
            for (int j = 0; j < size(); j++) {
                final GeneralNameInterface thisEntry = getGeneralNameInterface(j);
                switch (thisEntry.constrains(otherEntry)) {
                    case NAME_DIFF_TYPE:
                        diffType = true;
     ...
     */
    public void union(final GeneralSubtrees other) {
        if (other != null) {
     ...
     */
    public void reduce(final GeneralSubtrees excluded) {
        if (excluded == null) {
            return;
        }
        for (int i = 0, n = excluded.size(); i < n; i++) {
            final GeneralNameInterface excludedName = excluded.getGeneralNameInterface(i);
            for (int j = 0; j < size(); j++) {
                final GeneralNameInterface permitted = getGeneralNameInterface(j);
