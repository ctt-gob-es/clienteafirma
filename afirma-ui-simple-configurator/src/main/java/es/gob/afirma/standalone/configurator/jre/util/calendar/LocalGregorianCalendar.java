...
public class LocalGregorianCalendar extends BaseCalendar {
    ...
    public static class Date extends BaseCalendar.Date {
        ...
        public Date setEra(final Era era) {
            if (getEra() != era) {
                super.setEra(era);
                this.gregorianYear = FIELD_UNDEFINED;
            }
            return this;
        }

        @Override
        public Date addYear(final int localYear) {
        ...
        @Override
        public Date setYear(final int localYear) {
        ...
        public String toString() {
            ...
            final Era era = getEra();
            if (era != null) {
                final String abbr = era.getAbbreviation();
                if (abbr != null) {
                    sb.append(abbr);
                }
            }
    ...
    static LocalGregorianCalendar getLocalGregorianCalendar(final String name) {
        ...
        // Parse calendar.*.eras
        final String props = calendarProps.getProperty("calendar." + name + ".eras");
        if (props == null) {
            return null;
        }
        final List<Era> eras = new ArrayList<>();
        final StringTokenizer eraTokens = new StringTokenizer(props, ";");
        while (eraTokens.hasMoreTokens()) {
            final String items = eraTokens.nextToken().trim();
            final StringTokenizer itemTokens = new StringTokenizer(items, ",");
            String eraName = null;
            ...
            while (itemTokens.hasMoreTokens()) {
                final String item = itemTokens.nextToken();
                final int index = item.indexOf('=');
                // it must be in the key=value form.
                if (index == -1) {
                    return null;
                }
                final String key = item.substring(0, index);
                final String value = item.substring(index + 1);
                if ("name".equals(key)) {
                    eraName = value;
            ...
            final Era era = new Era(eraName, abbr, since, localTime);
            eras.add(era);
        }
        final Era[] eraArray = new Era[eras.size()];
    ...
    public Date getCalendarDate(final long millis, final CalendarDate date) {
        final Date ldate = (Date) super.getCalendarDate(millis, date);
        return adjustYear(ldate, millis, ldate.getZoneOffset());
    }

    private Date adjustYear(final Date ldate, final long millis, final int zoneOffset) {
        int i;
        for (i = this.eras.length - 1; i >= 0; --i) {
            ...
            if (millis >= since) {
                ...
                final int y = ldate.getNormalizedYear() - era.getSinceDate().getYear() + 1;
    ...
    public boolean validate(final CalendarDate date) {
        final Date ldate = (Date) date;
        final Era era = ldate.getEra();
        if (era != null) {
            if (!validateEra(era)) {
                return false;
            }
            ldate.setNormalizedYear(era.getSinceDate().getYear() + ldate.getYear() - 1);
            final Date tmp = newCalendarDate(date.getZone());
    ...
    public boolean normalize(final CalendarDate date) {
        ...
        long millis = 0;
        final int year = ldate.getNormalizedYear();
        int i;
        Era era = null;
        for (i = this.eras.length - 1; i >= 0; --i) {
            era = this.eras[i];
            if (era.isLocalTime()) {
                ...
                final int sinceYear = sinceDate.getYear();
                if (year > sinceYear) {
                    break;
                }
                if (year == sinceYear) {
                    final int month = ldate.getMonth();
                    final int sinceMonth = sinceDate.getMonth();
                    if (month > sinceMonth) {
                        break;
                    }
                    if (month == sinceMonth) {
                        final int day = ldate.getDayOfMonth();
                        final int sinceDay = sinceDate.getDayOfMonth();
                        if (day > sinceDay) {
                            break;
                        }
                        if (day == sinceDay) {
                            final long timeOfDay = ldate.getTimeOfDay();
                            final long sinceTimeOfDay = sinceDate.getTimeOfDay();
                ...
                final long since = era.getSince(date.getZone());
                if (millis >= since) {
                    break;
                }
            }
    ...
    void normalizeYear(final CalendarDate date) {
        final Date ldate = (Date) date;
     ...
     */
    @Override
    public boolean isLeapYear(final int gregorianYear) {
        return CalendarUtils.isGregorianLeapYear(gregorianYear);
    }

    public boolean isLeapYear(final Era era, final int year) {
        if (era == null) {
            return isLeapYear(year);
        }
        final int gyear = era.getSinceDate().getYear() + year - 1;
        return isLeapYear(gyear);
    }

    @Override
    public void getCalendarDateFromFixedDate(final CalendarDate date, final long fixedDate) {
        final Date ldate = (Date) date;
