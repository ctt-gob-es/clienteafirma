...
public class CertificateX509Key implements CertAttrSet<String> {
     ...
     */
    public CertificateX509Key(final DerInputStream in) throws IOException {
     ...
     * @exception IOException on decoding errors.
     */
    public CertificateX509Key(final InputStream in) throws IOException {
     ...
     */
    @Override
	public PublicKey get(final String name) throws IOException {
        if (name.equalsIgnoreCase(KEY)) {
            return(this.key);
        } else {
            throw new IOException("Attribute name not recognized by " +
