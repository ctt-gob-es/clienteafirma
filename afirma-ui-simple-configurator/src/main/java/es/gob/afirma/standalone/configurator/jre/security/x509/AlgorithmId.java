...
public class AlgorithmId implements Serializable, DerEncoder {
     ...
     */
    public AlgorithmId(final ObjectIdentifier oid) {
        this.algid = oid;
    }
     ...
     */
    public AlgorithmId(final ObjectIdentifier oid, final AlgorithmParameters algparams) {
        this.algid = oid;
        this.algParams = algparams;
        this.constructedFromDer = false;
    }

    private AlgorithmId(final ObjectIdentifier oid, final DerValue params)
            throws IOException {
        this.algid = oid;
        this.params = params;
        if (this.params != null) {
    ...
    protected void decodeParams() throws IOException {
        final String algidString = this.algid.toString();
        try {
	...
	public void derEncode (final OutputStream out) throws IOException {
        ...
        if (this.constructedFromDer == false) {
    ...
    public String getName() {
        ...
        if ((this.params != null) && this.algid.equals((Object)specifiedWithECDSA_oid)) {
            try {
                final AlgorithmId paramsId =
                        AlgorithmId.parse(new DerValue(getEncodedParams()));
                final String paramsName = paramsId.getName();
     ...
     */
    public boolean equals(final AlgorithmId other) {
        final boolean paramsEqual =
     ...
     */
    public final boolean equals(final ObjectIdentifier id) {
     ...
     */
    public static AlgorithmId parse(final DerValue val) throws IOException {
        ...
        DerValue                params;
        final DerInputStream          in = val.toDerInputStream();
     ...
     */
    @Deprecated
    public static AlgorithmId getAlgorithmId(final String algname)
     ...
     * @exception NoSuchAlgorithmException on error.
     */
    public static AlgorithmId get(final String algname)
            throws NoSuchAlgorithmException {
     ...
     * @exception NoSuchAlgorithmException on error.
     */
    public static AlgorithmId get(final AlgorithmParameters algparams)
            throws NoSuchAlgorithmException {
        ObjectIdentifier oid;
        final String algname = algparams.getAlgorithm();
        try {
            oid = algOID(algname);
        } catch (final IOException ioe) {
            throw new NoSuchAlgorithmException
     ...
     */
    private static ObjectIdentifier algOID(final String name) throws IOException {
        ...
        if (!initOidTable) {
            ...
            for (final Provider prov : provs) {
                for (final Enumeration<Object> enum_ = prov.keys();
                     enum_.hasMoreElements(); ) {
                    final String alias = (String)enum_.nextElement();
                    final String upperCaseAlias = alias.toUpperCase(Locale.ENGLISH);
                    int index;
