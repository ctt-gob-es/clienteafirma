...
public abstract class CalendarSystem {
    ...
    private static final String PACKAGE_NAME = "es.gob.afirma.standalone.configurator.jre.util.calendar."; //$NON-NLS-1$

    private static final String[] namePairs = {
        "gregorian", "Gregorian", //$NON-NLS-1$ //$NON-NLS-2$
        "japanese", "LocalGregorianCalendar", //$NON-NLS-1$ //$NON-NLS-2$
        "julian", "JulianCalendar", //$NON-NLS-1$ //$NON-NLS-2$
        /*
    ...
    private static void initNames() {
        final ConcurrentMap<String,String> nameMap = new ConcurrentHashMap<>();

        // Associate a calendar name with its class name and the
        // calendar class name with its date class name.
        final StringBuilder clName = new StringBuilder();
        for (int i = 0; i < namePairs.length; i += 2) {
            clName.setLength(0);
            final String cl = clName.append(PACKAGE_NAME).append(namePairs[i+1]).toString();
     ...
     */
    public static CalendarSystem forName(final String calendarName) {
        if ("gregorian".equals(calendarName)) { //$NON-NLS-1$
            return GREGORIAN_INSTANCE;
        }
        ...
        final String className = names.get(calendarName);
        if (className == null) {
            return null; // Unknown calendar name
        }

        if (className.endsWith("LocalGregorianCalendar")) { //$NON-NLS-1$
        ...
        final CalendarSystem cs =  calendars.putIfAbsent(calendarName, cal);
    ...
    public static Properties getCalendarProperties() throws IOException {
        ...
        try {
            final String homeDir = AccessController.doPrivileged(
                new GetPropertyAction("java.home")); //$NON-NLS-1$
            final String fname = homeDir + File.separator + "lib" + File.separator //$NON-NLS-1$
                                 + "calendars.properties"; //$NON-NLS-1$
