...
public class GeneralName {
     ...
     */
    public GeneralName(final DerValue encName) throws IOException {
        this(encName, false);
    }
     ...
     */
    public GeneralName(final DerValue encName, final boolean nameConstraint)
        throws IOException {
        final short tag = (byte)(encName.tag & 0x1f);
        ...
        case GeneralNameInterface.NAME_URI:
            if (encName.isContextSpecific() && !encName.isConstructed()) {
                encName.resetTag(DerValue.tag_IA5String);
                this.name = (nameConstraint ? URIName.nameConstraint(encName) :
                        new URIName(encName));
            } else {
	...
	public boolean equals(final Object other) {
        ...
        final GeneralNameInterface otherGNI = ((GeneralName)other).name;
        try {
            return this.name.constrains(otherGNI) == GeneralNameInterface.NAME_MATCH;
    ...
    public void encode(final DerOutputStream out) throws IOException {
        ...
        final int nameType = this.name.getType();
