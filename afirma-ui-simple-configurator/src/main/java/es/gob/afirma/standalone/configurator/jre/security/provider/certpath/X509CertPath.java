...
import es.gob.afirma.standalone.configurator.jre.security.pkcs.ContentInfo;
import es.gob.afirma.standalone.configurator.jre.security.pkcs.PKCS7;
import es.gob.afirma.standalone.configurator.jre.security.pkcs.SignerInfo;
...
public class X509CertPath extends CertPath {
    ...
    public X509CertPath(final List<? extends Certificate> certs) throws CertificateException {
        super("X.509");
     ...
     */
    public X509CertPath(final InputStream is) throws CertificateException {
     ...
     */
    public X509CertPath(final InputStream is, final String encoding)
     ...
     */
    private static List<X509Certificate> parsePKIPATH(final InputStream is)
            throws CertificateException {
        ...
        try {
            final DerInputStream dis = new DerInputStream(readAllBytes(is));
            final DerValue[] seq = dis.getSequence(3);
            if (seq.length == 0) {
    ...
    private static List<X509Certificate> parsePKCS7(InputStream is)
        ...
        try {
            ...
            final PKCS7 pkcs7 = new PKCS7(is);

            final X509Certificate[] certArray = pkcs7.getCertificates();
    ...
    private byte[] encodePKIPATH() throws CertificateEncodingException {
        ...
        final ListIterator<X509Certificate> li = this.certs.listIterator(this.certs.size());
        try {
            ...
            while (li.hasPrevious()) {
                final X509Certificate cert = li.previous();
    ...
    private byte[] encodePKCS7() throws CertificateEncodingException {
        final PKCS7 p7 = new PKCS7(new AlgorithmId[0],
                             new ContentInfo(ContentInfo.DATA_OID, null),
                             this.certs.toArray(new X509Certificate[this.certs.size()]),
                             new SignerInfo[0]);
        final DerOutputStream derout = new DerOutputStream();
        try {
