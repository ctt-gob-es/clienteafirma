...
package es.gob.afirma.standalone.configurator.jre.security.provider;
...
import es.gob.afirma.standalone.configurator.jre.security.pkcs.ParsingException;
import es.gob.afirma.standalone.configurator.jre.security.provider.certpath.X509CertPath;
import es.gob.afirma.standalone.configurator.jre.security.provider.certpath.X509CertificatePair;
...
public class X509Factory extends CertificateFactorySpi {
    ...
    public static final String BEGIN_CERT = "-----BEGIN CERTIFICATE-----"; //$NON-NLS-1$
    public static final String END_CERT = "-----END CERTIFICATE-----"; //$NON-NLS-1$
     ...
     */
    @Override
    public Certificate engineGenerateCertificate(final InputStream is)
        throws CertificateException
    {
        if (is == null) {
            ...
            throw new CertificateException("Missing input stream"); //$NON-NLS-1$
        }
        try {
            final byte[] encoding = readOneBlock(is);
            if (encoding != null) {
                ...
                throw new IOException("Empty input"); //$NON-NLS-1$
            }
        } catch (final IOException ioe) {
            throw new CertificateException("Could not parse certificate: " + //$NON-NLS-1$
                    ioe.toString(), ioe);
        }
    }
     ...
     */
    private static int readFully(final InputStream in, final ByteArrayOutputStream bout,
            int length) throws IOException {
        int read = 0;
        final byte[] buffer = new byte[2048];
        while (length > 0) {
            final int n = in.read(buffer, 0, length<2048?length:2048);
            if (n <= 0) {
                break;
            }
     ...
     */
    public static synchronized X509CertImpl intern(final X509Certificate c)
            throws CertificateException {
        if (c == null) {
            return null;
        }
        final boolean isImpl = c instanceof X509CertImpl;
        byte[] encoding;
        if (isImpl) {
     ...
     */
    public static synchronized X509CRLImpl intern(final X509CRL c)
            throws CRLException {
        if (c == null) {
            return null;
        }
        final boolean isImpl = c instanceof X509CRLImpl;
        byte[] encoding;
        if (isImpl) {
     ...
     */
    private static synchronized <K,V> V getFromCache(final Cache<K,V> cache,
            final byte[] encoding) {
        final Object key = new Cache.EqualByteArray(encoding);
     ...
     */
    private static synchronized <V> void addToCache(final Cache<Object, V> cache,
            final byte[] encoding, final V value) {
     ...
     */
    @Override
    public CertPath engineGenerateCertPath(final InputStream inStream)
        throws CertificateException
    {
        if (inStream == null) {
            throw new CertificateException("Missing input stream"); //$NON-NLS-1$
        ...
        try {
            final byte[] encoding = readOneBlock(inStream);
            if (encoding != null) {
    ...
    @Override
    public CertPath engineGenerateCertPath(final InputStream inStream,
        final String encoding) throws CertificateException
    {
        if (inStream == null) {
            throw new CertificateException("Missing input stream"); //$NON-NLS-1$
        }
        try {
            final byte[] data = readOneBlock(inStream);
            if (data != null) {
    ...
    @Override
    public CertPath
        engineGenerateCertPath(final List<? extends Certificate> certificates)
        throws CertificateException
    {
    ...
    public Collection<? extends java.security.cert.Certificate>
            engineGenerateCertificates(final InputStream is)
            throws CertificateException {
        if (is == null) {
            throw new CertificateException("Missing input stream"); //$NON-NLS-1$
        }
     ...
     */
    @Override
    public CRL engineGenerateCRL(final InputStream is)
        throws CRLException
    {
        if (is == null) {
            // clear the cache (for debugging)
            crlCache.clear();
            throw new CRLException("Missing input stream"); //$NON-NLS-1$
        }
        try {
            final byte[] encoding = readOneBlock(is);
            if (encoding != null) {
    ...
    public Collection<? extends java.security.cert.CRL> engineGenerateCRLs(
            final InputStream is) throws CRLException
    {
        if (is == null) {
            throw new CRLException("Missing input stream"); //$NON-NLS-1$
        }
    ...
    private Collection<? extends java.security.cert.Certificate>
        parseX509orPKCS7Cert(final InputStream is)
        throws CertificateException, IOException
    {
        int peekByte;
        byte[] data;
        final PushbackInputStream pbis = new PushbackInputStream(is);
        final Collection<X509CertImpl> coll = new ArrayList<>();
        ...
        // data has been found.
        if (data == null) {
            throw new CertificateException("No certificate data found"); //$NON-NLS-1$
        }

        try {
            final PKCS7 pkcs7 = new PKCS7(data);
            final X509Certificate[] certs = pkcs7.getCertificates();
    ...
    private Collection<? extends java.security.cert.CRL>
        parseX509orPKCS7CRL(final InputStream is)
        throws CRLException, IOException
    {
        int peekByte;
        byte[] data;
        final PushbackInputStream pbis = new PushbackInputStream(is);
        final Collection<X509CRLImpl> coll = new ArrayList<>();
        ...
        // data has been found.
        if (data == null) {
            throw new CRLException("No CRL data found"); //$NON-NLS-1$
        }

        try {
            final PKCS7 pkcs7 = new PKCS7(data);
            final X509CRL[] crls = pkcs7.getCRLs();
     ...
     */
    private static byte[] readOneBlock(final InputStream is) throws IOException {
        ...
        if (c == DerValue.tag_Sequence) {
            ...
            int end;
            final StringBuilder header = new StringBuilder("-----"); //$NON-NLS-1$
            while (true) {
                int next = is.read();
                if (next == -1) {
                    throw new IOException("Incomplete data"); //$NON-NLS-1$
                }
                if (next == '\n') {
                    end = '\n';
                    break;
            ...
            final StringBuilder footer = new StringBuilder("-"); //$NON-NLS-1$
            while (true) {
                final int next = is.read();
    ...
    private static void checkHeaderFooter(final String header,
            final String footer) throws IOException {
        if (header.length() < 16 || !header.startsWith("-----BEGIN ") || //$NON-NLS-1$
                !header.endsWith("-----")) { //$NON-NLS-1$
            throw new IOException("Illegal header: " + header); //$NON-NLS-1$
        }
        if (footer.length() < 14 || !footer.startsWith("-----END ") || //$NON-NLS-1$
                !footer.endsWith("-----")) { //$NON-NLS-1$
            throw new IOException("Illegal footer: " + footer); //$NON-NLS-1$
        }
        final String headerType = header.substring(11, header.length()-5);
        final String footerType = footer.substring(9, footer.length()-5);
        if (!headerType.equals(footerType)) {
            throw new IOException("Header and footer do not match: " + //$NON-NLS-1$
                    header + " " + footer); //$NON-NLS-1$
        }
    }
     ...
     */
    private static int readBERInternal(final InputStream is,
            final ByteArrayOutputStream bout, int tag) throws IOException {

        if (tag == -1) {        // Not read before the call, read now
            tag = is.read();
            if (tag == -1) {
                throw new IOException("BER/DER tag info absent"); //$NON-NLS-1$
            }
            if ((tag & 0x1f) == 0x1f) {
                throw new IOException("Multi octets tag not supported"); //$NON-NLS-1$
            }
            bout.write(tag);
        }
        ...
        final int n = is.read();
        if (n == -1) {
            throw new IOException("BER/DER length info absent"); //$NON-NLS-1$
        }
        bout.write(n);
        ...
        if (n == 0x80) {        // Indefinite-length encoding
            if ((tag & 0x20) != 0x20) {
                throw new IOException(
                        "Non constructed encoding must have definite length"); //$NON-NLS-1$
            }
            while (true) {
                final int subTag = readBERInternal(is, bout, -1);
            ...
            if (n < 0x80) {
                ...
                length = is.read();
                if (length == -1) {
                    throw new IOException("Incomplete BER/DER length info"); //$NON-NLS-1$
                }
                bout.write(length);
            } else if (n == 0x82) {
                final int highByte = is.read();
                final int lowByte = is.read();
                if (lowByte == -1) {
                    throw new IOException("Incomplete BER/DER length info"); //$NON-NLS-1$
                }
                bout.write(highByte);
                ...
                if (highByte > 127) {
                    throw new IOException("Invalid BER/DER data (a little huge?)"); //$NON-NLS-1$
                }
                bout.write(highByte);
                bout.write(nextByte);
                bout.write(midByte);
                ...
                throw new IOException("Invalid BER/DER data (too huge?)"); //$NON-NLS-1$
            }
            if (readFully(is, bout, length) != length) {
                throw new IOException("Incomplete BER/DER data"); //$NON-NLS-1$
            }
        }
        return tag;
    }
