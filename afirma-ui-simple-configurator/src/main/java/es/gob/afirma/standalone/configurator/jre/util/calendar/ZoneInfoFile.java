...
public final class ZoneInfoFile {
    ...
    public static String[] getZoneIds() {
        ...
        final String[] ids = Arrays.copyOf(regions, len);
        ...
        for (final String[] oldMapping : oldMappings) {
            ids[i++] = oldMapping[0];
        }
        return ids;
    }
     ...
     */
    public static String[] getZoneIds(final int rawOffset) {
        final List<String> ids = new ArrayList<>();
        for (final String id : getZoneIds()) {
            final ZoneInfo zi = getZoneInfo(id);
        ...
        // compatibility.
        final String[] list = ids.toArray(new String[ids.size()]);
        Arrays.sort(list);
        return list;
    }

    public static ZoneInfo getZoneInfo(final String zoneId) {
        if (zoneId == null) {
            return null;
        }
    ...
    private static ZoneInfo getZoneInfo0(final String zoneId) {
        try {
            ...
            final int index = Arrays.binarySearch(regions, zid);
            if (index < 0) {
                return null;
            }
            final byte[] bytes = ruleArray[indices[index]];
            final DataInputStream dis = new DataInputStream(new ByteArrayInputStream(bytes));
            zi = getZoneInfo(dis, zid);
            zones.put(zoneId, zi);
            return zi;
        } catch (final Exception ex) {
            throw new RuntimeException("Invalid binary time-zone data: TZDB:" + //$NON-NLS-1$
                zoneId + ", version: " + versionId, ex); //$NON-NLS-1$
        }
    }
     ...
     */
    public static ZoneInfo getCustomTimeZone(final String originalId, final int gmtOffset) {
        final String id = toCustomID(gmtOffset);
        return new ZoneInfo(id, gmtOffset);
    }

    public static String toCustomID(final int gmtOffset) {
        char sign;
        ...
        final int hh = offset / 60;
        final int mm = offset % 60;

        final char[] buf = new char[] { 'G', 'M', 'T', sign, '0', '0', ':', '0', '0' };
        if (hh >= 10) {
            buf[4] += hh / 10;
        }
        buf[5] += hh % 10;
    ...
    private static String[][] oldMappings = new String[][] {
        { "ACT", "Australia/Darwin" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "AET", "Australia/Sydney" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "AGT", "America/Argentina/Buenos_Aires" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "ART", "Africa/Cairo" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "AST", "America/Anchorage" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "BET", "America/Sao_Paulo" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "BST", "Asia/Dhaka" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "CAT", "Africa/Harare" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "CNT", "America/St_Johns" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "CST", "America/Chicago" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "CTT", "Asia/Shanghai" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "EAT", "Africa/Addis_Ababa" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "ECT", "Europe/Paris" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "IET", "America/Indiana/Indianapolis" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "IST", "Asia/Kolkata" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "JST", "Asia/Tokyo" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "MIT", "Pacific/Apia" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "NET", "Asia/Yerevan" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "NST", "Pacific/Auckland" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "PLT", "Asia/Karachi" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "PNT", "America/Phoenix" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "PRT", "America/Puerto_Rico" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "PST", "America/Los_Angeles" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "SST", "Pacific/Guadalcanal" }, //$NON-NLS-1$ //$NON-NLS-2$
        { "VST", "Asia/Ho_Chi_Minh" }, //$NON-NLS-1$ //$NON-NLS-2$
    };

    static {
        final String oldmapping = AccessController.doPrivileged(
            new GetPropertyAction("sun.timezone.ids.oldmapping", "false")).toLowerCase(Locale.ROOT); //$NON-NLS-1$ //$NON-NLS-2$
        USE_OLDMAPPING = (oldmapping.equals("yes") || oldmapping.equals("true")); //$NON-NLS-1$ //$NON-NLS-2$
        AccessController.doPrivileged(new PrivilegedAction<Object>() {
            @Override
			public Object run() {
                try {
                    final String libDir = System.getProperty("java.home") + File.separator + "lib"; //$NON-NLS-1$ //$NON-NLS-2$
                    try (DataInputStream dis = new DataInputStream(
                             new BufferedInputStream(new FileInputStream(
                                 new File(libDir, "tzdb.dat"))))) { //$NON-NLS-1$
                        load(dis);
                    }
    ...
    private static void addOldMapping() {
        for (final String[] alias : oldMappings) {
            aliases.put(alias[0], alias[1]);
        }
        if (USE_OLDMAPPING) {
            aliases.put("EST", "America/New_York"); //$NON-NLS-1$ //$NON-NLS-2$
            aliases.put("MST", "America/Denver"); //$NON-NLS-1$ //$NON-NLS-2$
            aliases.put("HST", "Pacific/Honolulu"); //$NON-NLS-1$ //$NON-NLS-2$
        } else {
            zones.put("EST", new ZoneInfo("EST", -18000000)); //$NON-NLS-1$ //$NON-NLS-2$
            zones.put("MST", new ZoneInfo("MST", -25200000)); //$NON-NLS-1$ //$NON-NLS-2$
            zones.put("HST", new ZoneInfo("HST", -36000000)); //$NON-NLS-1$ //$NON-NLS-2$
        }
    }
     ...
     */
    static void load(final DataInputStream dis) throws ClassNotFoundException, IOException {
        if (dis.readByte() != 1) {
            throw new StreamCorruptedException("File format not recognised"); //$NON-NLS-1$
        }
        // group
        final String groupId = dis.readUTF();
        if ("TZDB".equals(groupId) == false) { //$NON-NLS-1$
            throw new StreamCorruptedException("File format not recognised"); //$NON-NLS-1$
        }
        // versions, only keep the last one
        final int versionCount = dis.readShort();
        ...
        final String[] regionArray = new String[regionCount];
        for (int i = 0; i < regionCount; i++) {
            regionArray[i] = dis.readUTF();
        }
        // rules
        final int ruleCount = dis.readShort();
        ...
        zones.remove("ROC"); //$NON-NLS-1$
        for (int i = 0; i < versionCount; i++) {
            final int aliasCount = dis.readShort();
            aliases.clear();
            for (int j = 0; j < aliasCount; j++) {
                final String alias = regionArray[dis.readShort()];
                final String region = regionArray[dis.readShort()];
    ...
    /////////////////////////Ser/////////////////////////////////
    public static ZoneInfo getZoneInfo(final DataInput in, final String zoneId) throws Exception {
        final byte type = in.readByte();
        // TBD: assert ZRULES:
        final int stdSize = in.readInt();
        final long[] stdTrans = new long[stdSize];
        for (int i = 0; i < stdSize; i++) {
            stdTrans[i] = readEpochSec(in);
        }
        final int [] stdOffsets = new int[stdSize + 1];
        for (int i = 0; i < stdOffsets.length; i++) {
            stdOffsets[i] = readOffset(in);
        }
        final int savSize = in.readInt();
        final long[] savTrans = new long[savSize];
        for (int i = 0; i < savSize; i++) {
            savTrans[i] = readEpochSec(in);
        }
        final int[] savOffsets = new int[savSize + 1];
        for (int i = 0; i < savOffsets.length; i++) {
            savOffsets[i] = readOffset(in);
        }
        final int ruleSize = in.readByte();
        final ZoneOffsetTransitionRule[] rules = new ZoneOffsetTransitionRule[ruleSize];
    ...
    public static int readOffset(final DataInput in) throws IOException {
        final int offsetByte = in.readByte();
        return offsetByte == 127 ? in.readInt() : offsetByte * 900;
    }

    static long readEpochSec(final DataInput in) throws IOException {
        final int hiByte = in.readByte() & 255;
        if (hiByte == 255) {
            return in.readLong();
        } else {
            final int midByte = in.readByte() & 255;
            final int loByte = in.readByte() & 255;
            final long tot = ((hiByte << 16) + (midByte << 8) + loByte);
     ...
     */
    private static ZoneInfo getZoneInfo(final String zoneId,
                                        final long[] standardTransitions,
                                        final int[] standardOffsets,
                                        final long[] savingsInstantTransitions,
                                        final int[] wallOffsets,
        ...
        if (savingsInstantTransitions.length != 0) {
            ...
            for (; i < savingsInstantTransitions.length; i++) {
                final long trans = savingsInstantTransitions[i];
                if (trans > UTC2037) {
                ...
                while (k < standardTransitions.length) {
                    ...
                    final long trans_s = standardTransitions[k];
                    if (trans_s >= UTC1900) {
                        if (trans_s > trans) {
							break;
						}
            ...
            while (k < standardTransitions.length) {
                final long trans = standardTransitions[k];
                if (trans >= UTC1900) {
                    final int offset = wallOffsets[i];
                    final int offsetIndex = indexOf(offsets, 0, nOffsets, offset);
            ...
            if (lastRules.length > 1) {
                ...
                while (lastyear++ < LASTYEAR) {
                    for (final ZoneOffsetTransitionRule zotr : lastRules) {
                        final long trans = zotr.getTransitionEpochSecond(lastyear);
                        if (nOffsets + 2 >= offsets.length) {
                            offsets = Arrays.copyOf(offsets, offsets.length + 100);
                        }
                        if (nTrans + 1 >= transitions.length) {
                ...
                if (params[2] == 6 && params[3] == 0 &&
                    (zoneId.equals("Asia/Amman") || //$NON-NLS-1$
                     zoneId.equals("Asia/Gaza") || //$NON-NLS-1$
                     zoneId.equals("Asia/Hebron"))) { //$NON-NLS-1$
                    params[2] = 5;
                    params[3] = 86400000;
                }
                //endDayOfWeek and endTime workaround
                if (params[7] == 6 && params[8] == 0 &&
                    (zoneId.equals("Africa/Cairo"))) { //$NON-NLS-1$
                    params[7] = 5;
                    params[8] = 86400000;
                }
                ...
                if (lastyear < LASTYEAR) {
                    ...
                    //                           .toEpochSecond();
                    final long trans = LDT2037 - rawOffset/1000;

                    final int offsetIndex = indexOf(offsets, 0, nOffsets, rawOffset/1000);
                    if (offsetIndex == nOffsets) {
						nOffsets++;
					}
                    transitions[nTrans++] = (trans * 1000) << TRANSITION_NSHIFT |
                    ...
                    final long startTrans = savingsInstantTransitions[m - 2];
                    final int startOffset = wallOffsets[m - 2 + 1];
                    final int startStd = getStandardOffset(standardTransitions, standardOffsets, startTrans);
                    final long endTrans =  savingsInstantTransitions[m - 1];
                    final int endOffset = wallOffsets[m - 1 + 1];
                    final int endStd = getStandardOffset(standardTransitions, standardOffsets, endTrans);
            ...
            if (transitions != null) {
                final Checksum sum = new Checksum();
                for (i = 0; i < transitions.length; i++) {
                    final long val = transitions[i];
                    final int dst = (int)((val >>> DST_NSHIFT) & 0xfL);
                    final int saving = (dst == 0) ? 0 : offsets[dst];
                    final int index = (int)(val & OFFSET_MASK);
                    final int offset = offsets[index];
                    final long second = (val >> TRANSITION_NSHIFT);
    ...
    private static int getStandardOffset(final long[] standardTransitions,
                                         final int[] standardOffsets,
                                         final long epochSec) {
    ...
    private static int getYear(final long epochSecond, final int offset) {
        final long second = epochSecond + offset;  // overflow caught later
        final long epochDay = Math.floorDiv(second, SECONDS_PER_DAY);
        ...
        if (zeroDay < 0) {
            ...
            final long adjustCycles = (zeroDay + 1) / DAYS_PER_CYCLE - 1;
    ...
    private static int indexOf(final int[] offsets, int from, final int nOffsets, int offset) {
        offset *= 1000;
    ...
    private static int addTrans(final long transitions[], final int nTrans,
                                final int offsets[], int nOffsets,
                                final long trans, final int offset, final int stdOffset) {
        final int offsetIndex = indexOf(offsets, 0, nOffsets, offset);
        if (offsetIndex == nOffsets) {
			nOffsets++;
		}
    ...
    private static class ZoneOffsetTransitionRule {
        ...
        ZoneOffsetTransitionRule(final DataInput in) throws IOException {
            final int data = in.readInt();
            final int dowByte = (data & (7 << 19)) >>> 19;
            final int timeByte = (data & (31 << 14)) >>> 14;
            final int stdByte = (data & (255 << 4)) >>> 4;
            final int beforeByte = (data & (3 << 2)) >>> 2;
            final int afterByte = (data & 3);
        ...
        long getTransitionEpochSecond(final int year) {
            long epochDay = 0;
            if (this.dom < 0) {
            ...
            return epochDay * 86400 + this.secondOfDay + difference;
        }

        static final boolean isLeapYear(final int year) {
            return ((year & 3) == 0) && ((year % 100) != 0 || (year % 400) == 0);
        }

        static final int lengthOfMonth(final int year, final int month) {
            switch (month) {
                case 2:        //FEBRUARY:
        ...
        static final long toEpochDay(final int year, final int month, final int day) {
            final long y = year;
            final long m = month;
            long total = 0;
            total += 365 * y;
        ...
        static final long previousOrSame(final long epochDay, final int dayOfWeek) {
            return adjust(epochDay, dayOfWeek, 1);
        }

        static final long nextOrSame(final long epochDay, final int dayOfWeek) {
           return adjust(epochDay, dayOfWeek, 0);
        }

        static final long adjust(final long epochDay, final int dow, final int relative) {
            final int calDow = (int)Math.floorMod(epochDay + 3, 7L) + 1;
            if (relative < 2 && calDow == dow) {
                return epochDay;
            }
            if ((relative & 1) == 0) {
                final int daysDiff = calDow - dow;
                return epochDay + (daysDiff >= 0 ? 7 - daysDiff : -daysDiff);
            } else {
                final int daysDiff = dow - calDow;
                return epochDay - (daysDiff >= 0 ? 7 - daysDiff : -daysDiff);
            }
        }
    }
