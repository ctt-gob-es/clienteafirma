...
public class X500Name implements GeneralNameInterface, Principal {
     ...
     */
    public X500Name(final String dname) throws IOException {
     ...
     */
    public X500Name(final String dname, final Map<String, String> keywordMap)
        throws IOException {
     ...
     */
    public X500Name(final String dname, final String format) throws IOException {
        if (dname == null) {
     ...
     */
    public X500Name(final String commonName, final String organizationUnit,
                     final String organizationName, final String country)
    throws IOException {
        ...
        this.names[3].assertion[0] = new AVA(commonName_oid,
                new DerValue(commonName));
        this.names[2] = new RDN(1);
        this.names[2].assertion[0] = new AVA(orgUnitName_oid,
                new DerValue(organizationUnit));
        this.names[1] = new RDN(1);
        this.names[1].assertion[0] = new AVA(orgName_oid,
                new DerValue(organizationName));
        this.names[0] = new RDN(1);
        this.names[0].assertion[0] = new AVA(countryName_oid,
     ...
     */
    public X500Name(final String commonName, final String organizationUnit,
                    final String organizationName, final String localityName,
    ...
    throws IOException {
        ...
        this.names[5].assertion[0] = new AVA(commonName_oid,
                new DerValue(commonName));
        this.names[4] = new RDN(1);
        this.names[4].assertion[0] = new AVA(orgUnitName_oid,
                new DerValue(organizationUnit));
        this.names[3] = new RDN(1);
        this.names[3].assertion[0] = new AVA(orgName_oid,
                new DerValue(organizationName));
        this.names[2] = new RDN(1);
        this.names[2].assertion[0] = new AVA(localityName_oid,
                new DerValue(localityName));
        this.names[1] = new RDN(1);
        this.names[1].assertion[0] = new AVA(stateName_oid,
     ...
     */
    public X500Name(final RDN[] rdnArray) throws IOException {
        if (rdnArray == null) {
            this.names = new RDN[0];
        } else {
            this.names = rdnArray.clone();
            for (final RDN name : this.names) {
                if (name == null) {
     ...
     */
    public X500Name(final DerValue value) throws IOException {
     ...
     */
    public X500Name(final DerInputStream in) throws IOException {
        parseDER(in);
    }
     ...
     */
    public X500Name(final byte[] name) throws IOException {
        final DerInputStream in = new DerInputStream(name);
        parseDER(in);
    }
	...
	public boolean equals(final Object obj) {
        ...
        final X500Name other = (X500Name)obj;
        ...
        for (int i = 0; i < n; i++) {
            final RDN r1 = this.names[i];
            final RDN r2 = other.names[i];
            if (r1.assertion.length != r2.assertion.length) {
                return false;
            }
        }
        // definite check via canonical form
        final String thisCanonical = this.getRFC2253CanonicalName();
        final String otherCanonical = other.getRFC2253CanonicalName();
     ...
     */
    private String getString(final DerValue attribute) throws IOException {
        if (attribute == null) {
			return null;
		}
        final String  value = attribute.getAsString();
    ...
    public String getCountry() throws IOException {
        final DerValue attr = findAttribute(countryName_oid);
    ...
    public String getOrganization() throws IOException {
        final DerValue attr = findAttribute(orgName_oid);
    ...
    public String getOrganizationalUnit() throws IOException {
        final DerValue attr = findAttribute(orgUnitName_oid);
    ...
    public String getCommonName() throws IOException {
        final DerValue attr = findAttribute(commonName_oid);
    ...
    public String getLocality() throws IOException {
        final DerValue attr = findAttribute(localityName_oid);
    ...
    public String getState() throws IOException {
      final DerValue attr = findAttribute(stateName_oid);
    ...
    public String getDomain() throws IOException {
        final DerValue attr = findAttribute(DOMAIN_COMPONENT_OID);
    ...
    public String getDNQualifier() throws IOException {
        final DerValue attr = findAttribute(DNQUALIFIER_OID);
    ...
    public String getSurname() throws IOException {
        final DerValue attr = findAttribute(SURNAME_OID);
    ...
    public String getGivenName() throws IOException {
       final DerValue attr = findAttribute(GIVENNAME_OID);
    ...
    public String getInitials() throws IOException {
        final DerValue attr = findAttribute(INITIALS_OID);
    ...
    public String getGeneration() throws IOException {
        final DerValue attr = findAttribute(GENERATIONQUALIFIER_OID);
    ...
    public String getIP() throws IOException {
        final DerValue attr = findAttribute(ipAddress_oid);
     ...
     */
    public String getRFC1779Name(final Map<String, String> oidMap)
     ...
     * keywords contained in the OID/keyword map.
     */
    public String getRFC2253Name(final Map<String, String> oidMap) {
    ...
    private String generateRFC2253DN(final Map<String, String> oidMap) {
        /*
         ...
         */
        final StringBuilder fullname = new StringBuilder(48);
     ...
     */
    private DerValue findAttribute(final ObjectIdentifier attribute) {
        if (this.names != null) {
            for (final RDN name : this.names) {
                final DerValue value = name.findAttribute(attribute);
                if (value != null) {
                    return value;
                }
            }
     ...
     * type.
     */
    public DerValue findMostSpecificAttribute(final ObjectIdentifier attribute) {
        if (this.names != null) {
            for (int i = this.names.length - 1; i >= 0; i--) {
                final DerValue value = this.names[i].findAttribute(attribute);
                if (value != null) {
                    return value;
                }
            }
    ...
    private void parseDER(final DerInputStream in) throws IOException {
        //
        ...
        try {
            ...
            if (derBytes == null) {
                ...
                final DerValue derVal = new DerValue(DerValue.tag_Sequence,
                                           derBytes);
     ...
     */
    @Deprecated
    public void emit(final DerOutputStream out) throws IOException {
        encode(out);
    }
     ...
     */
    private void parseDN(final String input, final Map<String, String> keywordMap)
        throws IOException {
        ...
        final List<RDN> dnVector = new ArrayList<>();
        int dnOffset = 0;
        int rdnEnd;
        String rdnString;
        int quoteCount = 0;

        final String dnString = input;
        ...
        while (nextComma >=0 || nextSemiColon >=0) {
            ...
            if (rdnEnd >= 0 && quoteCount != 1 &&
                ...
                final RDN rdn = new RDN(rdnString, keywordMap);
                dnVector.add(rdn);
    ...
    private void parseRFC2253DN(final String dnString) throws IOException {
        if (dnString.length() == 0) {
         ...
         while (rdnEnd >=0) {
             ...
             if (rdnEnd > 0 && !escaped(rdnEnd, searchOffset, dnString)) {
                 ...
                 // Parse RDN, and store it in vector
                 final RDN rdn = new RDN(rdnString, "RFC2253");
                 dnVector.add(rdn);
     ...
     */
    static int countQuotes(final String string, final int from, final int to) {
        int count = 0;
    ...
    private static boolean escaped
                (int rdnEnd, final int searchOffset, final String dnString) {
     ...
     */
    private String generateRFC1779DN(final Map<String, String> oidMap) {
        if (this.names.length == 1) {
     ...
     */
    static ObjectIdentifier intern(final ObjectIdentifier oid) {
        final ObjectIdentifier interned = internedOIDs.get(oid);
        if (interned != null) {
            return interned;
        }
	...
	public int constrains(final GeneralNameInterface inputName)
        ...
        if (inputName == null) {
            ...
            final X500Name inputX500 = (X500Name)inputName;
     ...
     */
    private boolean isWithinSubtree(final X500Name other) {
        if (this == other) {
            return true;
        }
        if (other == null) {
     ...
     */
    public X500Name commonAncestor(final X500Name other) {

        if (other == null) {
            return null;
        }
        final int otherLen = other.names.length;
        final int thisLen = this.names.length;
        if (thisLen == 0 || otherLen == 0) {
            return null;
        }
        final int minLen = (thisLen < otherLen) ? thisLen: otherLen;
     ...
     */
    static {
        final PrivilegedExceptionAction<Object[]> pa =
                new PrivilegedExceptionAction<Object[]>() {
            @Override
			public Object[] run() throws Exception {
                final Class<X500Principal> pClass = X500Principal.class;
                final Class<?>[] args = new Class<?>[] { X500Name.class };
                final Constructor<X500Principal> cons = pClass.getDeclaredConstructor(args);
                cons.setAccessible(true);
                final Field field = pClass.getDeclaredField("thisX500Name");
                field.setAccessible(true);
                return new Object[] {cons, field};
            }
        };
        try {
            final Object[] result = AccessController.doPrivileged(pa);
     ...
     */
    public static X500Name asX500Name(final X500Principal p) {
        try {
            final X500Name name = (X500Name)principalField.get(p);
