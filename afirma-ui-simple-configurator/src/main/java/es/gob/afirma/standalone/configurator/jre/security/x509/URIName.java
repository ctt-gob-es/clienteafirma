...
public class URIName implements GeneralNameInterface {
     ...
     */
    public URIName(final String name) throws IOException {
        try {
            this.uri = new URI(name);
        } catch (final URISyntaxException use) {
            throw new IOException("invalid URI name:" + name, use); //$NON-NLS-1$
        }
        if (this.uri.getScheme() == null) {
            throw new IOException("URI name must include scheme:" + name); //$NON-NLS-1$
        }
        ...
        if (this.host != null) {
            if (this.host.charAt(0) == '[') {
                ...
                final String ipV6Host = this.host.substring(1, this.host.length()-1);
                try {
                    this.hostIP = new IPAddressName(ipV6Host);
                } catch (final IOException ioe) {
                    throw new IOException("invalid URI name (host " + //$NON-NLS-1$
                        "portion is not a valid IPv6 address):" + name); //$NON-NLS-1$
                }
            } else {
                try {
                    ...
                    } catch (final Exception ioe2) {
                        throw new IOException("invalid URI name (host " + //$NON-NLS-1$
                            "portion is not a valid DNS name, IPv4 address," + //$NON-NLS-1$
                            " or IPv6 address):" + name); //$NON-NLS-1$
                    }
                }
            }
        }
     ...
     */
    public static URIName nameConstraint(final DerValue value) throws IOException {
        URI uri;
        final String name = value.getIA5String();
        try {
            uri = new URI(name);
        } catch (final URISyntaxException use) {
            throw new IOException("invalid URI name constraint:" + name, use); //$NON-NLS-1$
        }
        if (uri.getScheme() == null) {
            final String host = uri.getSchemeSpecificPart();
            try {
                DNSName hostDNS;
                if (host.charAt(0) == '.') {
                ...
                throw new IOException("invalid URI name constraint:" + name, ioe); //$NON-NLS-1$
            }
        } else {
            throw new IOException("invalid URI name constraint (should not " + //$NON-NLS-1$
                "include scheme):" + name); //$NON-NLS-1$
        }
    }
	...
	public String toString() {
        return "URIName: " + this.uri.toString(); //$NON-NLS-1$
    }
	...
	public boolean equals(final Object obj) {
        ...
        final URIName other = (URIName) obj;
	...
	public int constrains(final GeneralNameInterface inputName)
        ...
        if (inputName == null) {
            ...
            final String otherHost = ((URIName)inputName).getHost();

            // Quick check for equality
            if (otherHost.equalsIgnoreCase(this.host)) {
                constraintType = NAME_MATCH;
            } else {
                final Object otherHostObject = ((URIName)inputName).getHostObject();

                if ((this.hostDNS == null) ||
                    ...
                    final boolean thisDomain = (this.host.charAt(0) == '.');
                    final boolean otherDomain = (otherHost.charAt(0) == '.');
                    final DNSName otherDNS = (DNSName) otherHostObject;
