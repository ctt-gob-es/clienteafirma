...
public class DerValue {
    ...
    public DerValue(final byte tag, final byte[] data) {
        this.tag = tag;
        this.buffer = new DerInputBuffer(data.clone());
        ...
        if (this.length == -1) {  // indefinite length encoding found
            ...
            if (this.tag != inbuf.read()) {
				throw new IOException
                        ("Indefinite length encoding not supported"); //$NON-NLS-1$
			}
    ...
    private DerInputStream init(final byte stringTag, final String value) throws IOException {
        String enc = null;
        ...
        case tag_GeneralString:
            ...
            break;
        case tag_T61String:
            enc = "ISO-8859-1"; //$NON-NLS-1$
            break;
        case tag_BMPString:
            enc = "UnicodeBigUnmarked"; //$NON-NLS-1$
            break;
        case tag_UTF8String:
            enc = "UTF8"; //$NON-NLS-1$
            break;
            ...
            // be handled.
        default:
            throw new IllegalArgumentException("Unsupported DER string type"); //$NON-NLS-1$
        }
    ...
    private DerInputStream init(final boolean fullyBuffered, InputStream in)
        ...
        if (fullyBuffered && in.available() != this.length) {
			throw new IOException("extra data given to DerValue constructor"); //$NON-NLS-1$
		}

        final byte[] bytes = IOUtils.readFully(in, this.length, true);
        ...
        if (this.length > 0) {
            ...
            synchronized (this.data) {
                ...
                if (this.buffer.read(value) != this.length) {
                    throw new IOException("short DER value read (encode)"); //$NON-NLS-1$
                }
                out.write(value);
            }
        }
    ...
    public boolean getBoolean() throws IOException {
        if (this.tag != tag_Boolean) {
            throw new IOException("DerValue.getBoolean, not a BOOLEAN " + this.tag); //$NON-NLS-1$
        }
        if (this.length != 1) {
            throw new IOException("DerValue.getBoolean, invalid length " //$NON-NLS-1$
                                        + this.length);
        }
    ...
    public ObjectIdentifier getOID() throws IOException {
        if (this.tag != tag_ObjectId) {
			throw new IOException("DerValue.getOID, not an OID " + this.tag); //$NON-NLS-1$
		}
    ...
    public byte[] getOctetString() throws IOException {
        ...
        if (this.tag != tag_OctetString && !isConstructed(tag_OctetString)) {
            throw new IOException(
                "DerValue.getOctetString, not an Octet String: " + this.tag); //$NON-NLS-1$
        }
        ...
        if (this.buffer.read(bytes) != this.length) {
			throw new IOException("short read on DerValue buffer"); //$NON-NLS-1$
		}
        if (isConstructed()) {
            final DerInputStream in = new DerInputStream(bytes);
            bytes = null;
    ...
    public int getInteger() throws IOException {
        if (this.tag != tag_Integer) {
            throw new IOException("DerValue.getInteger, not an int " + this.tag); //$NON-NLS-1$
        }
    ...
    public BigInteger getBigInteger() throws IOException {
        if (this.tag != tag_Integer) {
			throw new IOException("DerValue.getBigInteger, not an int " + this.tag); //$NON-NLS-1$
		}
    ...
    public int getEnumerated() throws IOException {
        if (this.tag != tag_Enumerated) {
            throw new IOException("DerValue.getEnumerated, incorrect tag: " //$NON-NLS-1$
                                  + this.tag);
        }
        return this.buffer.getInteger(this.data.available());
    }
    ...
    public byte[] getBitString() throws IOException {
        if (this.tag != tag_BitString) {
			throw new IOException(
                "DerValue.getBitString, not a bit string " + this.tag); //$NON-NLS-1$
		}
    ...
    public byte[] getBitString(final boolean tagImplicit) throws IOException {
        if (!tagImplicit) {
            if (this.tag != tag_BitString) {
				throw new IOException("DerValue.getBitString, not a bit string " //$NON-NLS-1$
                                       + this.tag);
			}
            }
        return this.buffer.getBitString();
    ...
    throws IOException {
        if (this.tag != tag_PrintableString) {
			throw new IOException(
                "DerValue.getPrintableString, not a string " + this.tag); //$NON-NLS-1$
		}

        return new String(getDataBytes(), "ASCII"); //$NON-NLS-1$
    }
    ...
    public String getT61String() throws IOException {
        if (this.tag != tag_T61String) {
			throw new IOException(
                "DerValue.getT61String, not T61 " + this.tag); //$NON-NLS-1$
		}

        return new String(getDataBytes(), "ISO-8859-1"); //$NON-NLS-1$
    }
    ...
    public String getIA5String() throws IOException {
        if (this.tag != tag_IA5String) {
			throw new IOException(
                "DerValue.getIA5String, not IA5 " + this.tag); //$NON-NLS-1$
		}
    ...
    public String getBMPString() throws IOException {
        if (this.tag != tag_BMPString) {
			throw new IOException(
                "DerValue.getBMPString, not BMP " + this.tag); //$NON-NLS-1$
		}

        // BMPString is the same as Unicode in big endian, unmarked
        // format.
        return new String(getDataBytes(), "UnicodeBigUnmarked"); //$NON-NLS-1$
    }
    ...
    public String getUTF8String() throws IOException {
        if (this.tag != tag_UTF8String) {
			throw new IOException(
                "DerValue.getUTF8String, not UTF-8 " + this.tag); //$NON-NLS-1$
		}

        return new String(getDataBytes(), "UTF8"); //$NON-NLS-1$
    }
    ...
    public String getGeneralString() throws IOException {
        if (this.tag != tag_GeneralString) {
			throw new IOException(
                "DerValue.getGeneralString, not GeneralString " + this.tag); //$NON-NLS-1$
		}
    ...
    public Date getUTCTime() throws IOException {
        if (this.tag != tag_UtcTime) {
            throw new IOException("DerValue.getUTCTime, not a UtcTime: " + this.tag); //$NON-NLS-1$
        }
    ...
    public Date getGeneralizedTime() throws IOException {
        if (this.tag != tag_GeneralizedTime) {
            throw new IOException(
                "DerValue.getGeneralizedTime, not a GeneralizedTime: " + this.tag); //$NON-NLS-1$
        }
	...
	public String toString() {
        try {
            ...
            if (this.tag == tag_Null) {
				return "[DerValue, null]"; //$NON-NLS-1$
			}
            if (this.tag == tag_ObjectId) {
				return "OID." + getOID(); //$NON-NLS-1$
			} else {
				return "[DerValue, tag = " + this.tag //$NON-NLS-1$
                        + ", length = " + this.length + "]"; //$NON-NLS-1$ //$NON-NLS-2$
			}
        } catch (final IOException e) {
            throw new IllegalArgumentException("misformatted DER value"); //$NON-NLS-1$
        }
    }
    ...
    public DerInputStream toDerInputStream() throws IOException {
        ...
        throw new IOException("toDerInputStream rejects tag type " + this.tag); //$NON-NLS-1$
    }
