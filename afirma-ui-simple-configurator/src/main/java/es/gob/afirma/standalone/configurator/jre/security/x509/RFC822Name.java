...
public class RFC822Name implements GeneralNameInterface
{
     ...
     */
    public void parseName(final String name) throws IOException {
        if (name == null || name.length() == 0) {
            throw new IOException("RFC822Name may not be null or empty"); //$NON-NLS-1$
        }
        // See if domain is a valid domain name
        final String domain = name.substring(name.indexOf('@')+1);
        if (domain.length() == 0) {
            throw new IOException("RFC822Name may not end with @"); //$NON-NLS-1$
        }
		//An RFC822 NameConstraint could start with a ., although
		//a DNSName may not
		if (domain.startsWith(".")) { //$NON-NLS-1$
		    if (domain.length() == 1) {
				throw new IOException("RFC822Name domain may not be just ."); //$NON-NLS-1$
			}
		}
    }
	...
	public String toString() {
        return ("RFC822Name: " + this.name); //$NON-NLS-1$
    }
	...
	public boolean equals(final Object obj) {
        ...
        final RFC822Name other = (RFC822Name)obj;
	...
	public int constrains(final GeneralNameInterface inputName) throws UnsupportedOperationException {
        ...
        if (inputName == null) {
            ...
            final String thisName = this.name.toLowerCase(Locale.ENGLISH);
            if (inName.equals(thisName)) {
                ...
                } else if (inName.startsWith(".")) { //$NON-NLS-1$
                    constraintType = NAME_WIDENS;
                } else {
                    final int inNdx = thisName.lastIndexOf(inName);
                ...
                if (thisName.indexOf('@') != -1) {
                    constraintType = NAME_SAME_TYPE;
                } else if (thisName.startsWith(".")) { //$NON-NLS-1$
                    constraintType = NAME_NARROWS;
                } else {
                    final int ndx = inName.lastIndexOf(thisName);
	...
	public int subtreeDepth() throws UnsupportedOperationException {
        ...
        final int atNdx = subtree.lastIndexOf('@');
        if (atNdx >= 0) {
            i++;
