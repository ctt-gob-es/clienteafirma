...
public class ZoneInfo extends TimeZone {
     ...
     */
    public ZoneInfo(final String ID, final int rawOffset) {
     ...
     */
    ZoneInfo(final String ID,
             final int rawOffset,
             final int dstSavings,
             final int checksum,
             final long[] transitions,
             final int[] offsets,
             final int[] simpleTimeZoneParams,
             final boolean willGMTOffsetChange) {
        setID(ID);
    ...
    public int getOffsets(final long utc, final int[] offsets) {
        return getOffsets(utc, offsets, UTC_TIME);
    }

    public int getOffsetsByStandard(final long standard, final int[] offsets) {
        return getOffsets(standard, offsets, STANDARD_TIME);
    }

    public int getOffsetsByWall(final long wall, final int[] offsets) {
        return getOffsets(wall, offsets, WALL_TIME);
    }

    private int getOffsets(long date, final int[] offsets, final int type) {
        // if dst is never observed, there is no transition.
        if (this.transitions == null) {
            final int offset = getLastRawOffset();
            if (offsets != null) {
                offsets[0] = offset;
                offsets[1] = 0;
            }
        ...
        final int index = getTransitionIndex(date, type);
        ...
        if (index < this.transitions.length) {
            final long val = this.transitions[index];
            final int offset = this.offsets[(int)(val & OFFSET_MASK)] + this.rawOffsetDiff;
            if (offsets != null) {
                final int dst = (int)((val >>> DST_NSHIFT) & 0xfL);
                final int save = (dst == 0) ? 0 : this.offsets[dst];
        ...
        final SimpleTimeZone tz = getLastRule();
        if (tz != null) {
            ...
            int dstoffset = tz.getOffset(msec) - this.rawOffset;
    ...
    private int getTransitionIndex(final long date, final int type) {
        int low = 0;
        int high = this.transitions.length - 1;

        while (low <= high) {
            final int mid = (low + high) / 2;
            final long val = this.transitions[mid];
            long midVal = val >> TRANSITION_NSHIFT; // sign extended
            if (type != UTC_TIME) {
                midVal += this.offsets[(int)(val & OFFSET_MASK)]; // wall time
            }
            if (type == STANDARD_TIME) {
                final int dstIndex = (int)((val >>> DST_NSHIFT) & 0xfL);
                if (dstIndex != 0) {
                    midVal -= this.offsets[dstIndex]; // make it standard time
                }
            }
   ...
   /**
     ...
     */
    @Override
	public int getOffset(final int era, int year, final int month, final int day,
        ...
        final CalendarDate date = gcal.newCalendarDate(null);
        ...
        dateInMillis -= this.rawOffset; // make it UTC
     ...
     */
    @Override
	public synchronized void setRawOffset(final int offsetMillis) {
    ...
    public boolean observesDaylightTime() {
        ...
        // any future.
        final long utc = System.currentTimeMillis() - this.rawOffsetDiff;
        final int index = getTransitionIndex(utc, UTC_TIME);
	...
	public boolean inDaylightTime(final Date date) {
        ...
        final long utc = date.getTime() - this.rawOffsetDiff;
        final int index = getTransitionIndex(utc, UTC_TIME);
        ...
        // the time is in the table range.
        if (index < this.transitions.length) {
            return (this.transitions[index] & DST_MASK) != 0;
        }
...
//    }
	...
	public String toString() {
        return getClass().getName() +
            ...
            ",transitions=" + ((this.transitions != null) ? this.transitions.length : 0) +
            ",lastRule=" + (this.lastRule == null ? getLastRuleInstance() : this.lastRule) +
            "]";
    }
    ...
    public SimpleTimeZone getLastRuleInstance() {
        ...
        if (this.simpleTimeZoneParams.length == 10) {
            return new SimpleTimeZone(getLastRawOffset(), getID(),
                                      this.simpleTimeZoneParams[0],
                                      this.simpleTimeZoneParams[1],
                                      this.simpleTimeZoneParams[2],
                                      this.simpleTimeZoneParams[3],
                                      this.simpleTimeZoneParams[4],
                                      this.simpleTimeZoneParams[5],
                                      this.simpleTimeZoneParams[6],
                                      this.simpleTimeZoneParams[7],
                                      this.simpleTimeZoneParams[8],
                                      this.simpleTimeZoneParams[9],
                                      this.dstSavings);
        }
        return new SimpleTimeZone(getLastRawOffset(), getID(),
                                  this.simpleTimeZoneParams[0],
    ...
    @Override
	public Object clone() {
        final ZoneInfo zi = (ZoneInfo) super.clone();
        zi.lastRule = null;
        return zi;
    }
	...
	public boolean equals(final Object obj) {
        ...
        final ZoneInfo that = (ZoneInfo) obj;
