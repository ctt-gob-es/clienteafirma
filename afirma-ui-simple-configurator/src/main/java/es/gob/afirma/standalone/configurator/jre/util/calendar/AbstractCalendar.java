...
public abstract class AbstractCalendar extends CalendarSystem {
    ...
    @Override
	public Era getEra(final String eraName) {
        if (this.eras != null) {
            for (final Era era : this.eras) {
                if (era.equals(eraName)) {
                    return era;
                }
            }
        }
    ...
    @Override
	public void setEra(final CalendarDate date, final String eraName) {
        if (this.eras == null) {
            return; // should report an error???
        }
        for (final Era e : this.eras) {
    ...
    @Override
	public CalendarDate getCalendarDate(final long millis) {
        return getCalendarDate(millis, newCalendarDate());
    }

    @Override
	public CalendarDate getCalendarDate(final long millis, final TimeZone zone) {
        final CalendarDate date = newCalendarDate(zone);
        return getCalendarDate(millis, date);
    }

    @Override
	public CalendarDate getCalendarDate(final long millis, final CalendarDate date) {
        ...
        final TimeZone zi = date.getZone();
        if (zi != null) {
    ...
    @Override
	public long getTime(final CalendarDate date) {
        final long gd = getFixedDate(date);
    ...
    protected long getTimeOfDay(final CalendarDate date) {
    ...
    public long getTimeOfDayValue(final CalendarDate date) {
    ...
    @Override
	public CalendarDate setTimeOfDay(final CalendarDate cdate, final int fraction) {
        if (fraction < 0) {
            throw new IllegalArgumentException();
        }
        final boolean normalizedState = cdate.isNormalized();
        int time = fraction;
        final int hours = time / HOUR_IN_MILLIS;
        time %= HOUR_IN_MILLIS;
        final int minutes = time / MINUTE_IN_MILLIS;
        time %= MINUTE_IN_MILLIS;
        final int seconds = time / SECOND_IN_MILLIS;
    ...
    @Override
	public CalendarDate getNthDayOfWeek(final int nth, final int dayOfWeek, final CalendarDate date) {
        final CalendarDate ndate = (CalendarDate) date.clone();
        normalize(ndate);
        final long fd = getFixedDate(ndate);
        long nfd;
        if (nth > 0) {
     ...
     */
    static long getDayOfWeekDateBefore(final long fixedDate, final int dayOfWeek) {
     ...
     * @return the calculated date
     */
    static long getDayOfWeekDateAfter(final long fixedDate, final int dayOfWeek) {
    ...
    public static long getDayOfWeekDateOnOrBefore(final long fixedDate, final int dayOfWeek) {
        final long fd = fixedDate - (dayOfWeek - 1);
        if (fd >= 0) {
    ...
    public boolean validateTime(final CalendarDate date) {
        int t = date.getHours();
        if (t < 0 || t >= 24) {
            return false;
        }
    ...
    int normalizeTime(final CalendarDate date) {
