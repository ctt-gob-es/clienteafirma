...
import es.gob.afirma.standalone.configurator.jre.util.calendar.CalendarDate;
import es.gob.afirma.standalone.configurator.jre.util.calendar.CalendarSystem;
...
class DerInputBuffer extends ByteArrayInputStream implements Cloneable {
    ...
    DerInputBuffer(final byte[] buf) { super(buf); }

    DerInputBuffer(final byte[] buf, final int offset, final int len) {
    ...
    int peek() throws IOException {
        if (this.pos >= this.count) {
			throw new IOException("out of data"); //$NON-NLS-1$
		} else {
    ...
    void truncate(final int len) throws IOException {
        if (len > available()) {
			throw new IOException("insufficient data"); //$NON-NLS-1$
		}
    ...
    BigInteger getBigInteger(final int len, final boolean makePositive) throws IOException {
        if (len > available()) {
			throw new IOException("short read of integer"); //$NON-NLS-1$
		}

        if (len == 0) {
            throw new IOException("Invalid encoding: zero length Int value"); //$NON-NLS-1$
        }
    ...
    public int getInteger(final int len) throws IOException {
        ...
        if (result.compareTo(BigInteger.valueOf(Integer.MIN_VALUE)) < 0) {
            throw new IOException("Integer below minimum valid value"); //$NON-NLS-1$
        }
        if (result.compareTo(BigInteger.valueOf(Integer.MAX_VALUE)) > 0) {
            throw new IOException("Integer exceeds maximum valid value"); //$NON-NLS-1$
        }
    ...
    public byte[] getBitString(final int len) throws IOException {
        if (len > available()) {
			throw new IOException("short read of bit string"); //$NON-NLS-1$
		}

        if (len == 0) {
            throw new IOException("Invalid encoding: zero length bit string"); //$NON-NLS-1$
        }

        final int numOfPadBits = this.buf[this.pos];
        if ((numOfPadBits < 0) || (numOfPadBits > 7)) {
            throw new IOException("Invalid number of padding bits"); //$NON-NLS-1$
        }
    ...
    BitArray getUnalignedBitString() throws IOException {
        ...
        if (unusedBits > 7 ) {
            throw new IOException("Invalid value for unused bits: " + unusedBits); //$NON-NLS-1$
        }
    ...
    public Date getUTCTime(final int len) throws IOException {
        if (len > available()) {
			throw new IOException("short read of DER UTC Time"); //$NON-NLS-1$
		}

        if (len < 11 || len > 17) {
			throw new IOException("DER UTC Time length error"); //$NON-NLS-1$
		}
    ...
    public Date getGeneralizedTime(final int len) throws IOException {
        if (len > available()) {
			throw new IOException("short read of DER Generalized Time"); //$NON-NLS-1$
		}

        if (len < 13 || len > 23) {
			throw new IOException("DER Generalized Time length error"); //$NON-NLS-1$
		}
    ...
    private Date getTime(int len, final boolean generalized) throws IOException {
        ...
        if (len > 2 && len < 12) {
            ...
            if (this.buf[this.pos] == '.' || this.buf[this.pos] == ',') {
                ...
                case 1:
                    ...
                    break;
                default:
                        throw new IOException("Parse " + type + //$NON-NLS-1$
                            " time, unsupported precision for seconds value"); //$NON-NLS-1$
                }
                len -= precision;
            }
        } else {
        ...
        if (month == 0 || day == 0
			...
			throw new IOException("Parse " + type + " time, invalid format"); //$NON-NLS-1$ //$NON-NLS-2$
		}
         ...
         */
        if (! (len == 1 || len == 5)) {
			throw new IOException("Parse " + type + " time, invalid offset"); //$NON-NLS-1$ //$NON-NLS-2$
		}
        ...
        case '+':
            ...
            if (hr >= 24 || min >= 60) {
				throw new IOException("Parse " + type + " time, +hhmm"); //$NON-NLS-1$ //$NON-NLS-2$
			}
        ...
        default:
            throw new IOException("Parse " + type + " time, garbage offset"); //$NON-NLS-1$ //$NON-NLS-2$
        }
