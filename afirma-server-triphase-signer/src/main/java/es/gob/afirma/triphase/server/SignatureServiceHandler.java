...
public final class SignatureServiceHandler {
	...
	public static void processRequest(final Map<String, String> requestParameters, final PrintWriter responseWriter) {


		// Obtenemos el codigo de operacion
		final String operation = requestParameters.get(PARAM_NAME_OPERATION);
		if (operation == null) {
			LOGGER.severe("No se ha indicado la operacion trifasica a realizar"); //$NON-NLS-1$
			responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.OPERATION_ARGUMENT_NOT_FOUND));
			responseWriter.flush();
			return;
		}

		// Obtenemos el codigo de operacion
		final String subOperation = requestParameters.get(PARAM_NAME_SUB_OPERATION);
		if (subOperation == null || !PARAM_VALUE_SUB_OPERATION_SIGN.equalsIgnoreCase(subOperation)
			...
			responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.INVALID_SUBOPERATION));
			responseWriter.flush();
			return;
		}
		...
		if (format == null) {
			...
			responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.FORMAT_ARGUMENT_NOT_FOUND));
			responseWriter.flush();
			return;
		}
		...
		try {
			if (requestParameters.containsKey(PARAM_NAME_EXTRA_PARAM)) {
				extraParams = AOUtil.base642Properties(requestParameters.get(PARAM_NAME_EXTRA_PARAM));
			}
		}
		catch (final Exception e) {
			LOGGER.severe("El formato de los parametros adicionales suministrado es erroneo: " +  e); //$NON-NLS-1$
			responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.INVALID_EXTRAPARAMS_FORMAT) + ": " + e); //$NON-NLS-1$);
			responseWriter.flush();
			return;
		}
		...
		try {
			if (requestParameters.containsKey(PARAM_NAME_SESSION_DATA)) {
				sessionData = Base64.decode(requestParameters.get(PARAM_NAME_SESSION_DATA).trim(), true);
			}
		}
		catch (final Exception e) {
		...
		final String cert = requestParameters.get(PARAM_NAME_CERT);
		if (cert == null) {
			LOGGER.warning("No se ha indicado certificado de firma"); //$NON-NLS-1$
			responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.CERTIFICATE_ARGUMENT_NOT_FOUND));
			responseWriter.flush();
			return;
		}
		...
		for (int i = 0; i<receivedCerts.length; i++) {
			try {
				...
				responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.INVALID_CERTIFICATE_FORMAT));
				responseWriter.flush();
				return;
			}
		}
		...
		final String docId = requestParameters.get(PARAM_NAME_DOCID);
		if (docId != null && docBytes == null) {
			try {
				...
				responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.RECOVERING_DOCUMENT_ERROR, ex.getErrorCode().getCode()));
				responseWriter.flush();
				return;
			}
		...
		String algorithm = requestParameters.get(PARAM_NAME_ALGORITHM);
		if (algorithm == null) {
			...
			responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.UNSUPPORTED_ALGORITHM));
			responseWriter.flush();
			return;
		}
		...
		if (AOSignConstants.SIGN_FORMAT_AUTO.equalsIgnoreCase(format)) {
			...
			try {
				...
				responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.UNSUPPORTED_FORMAT));
				responseWriter.flush();
				return;
			}
		}
		...
		if (PARAM_VALUE_OPERATION_PRESIGN.equalsIgnoreCase(operation)) {
			...
			try {
				...
				if (e.isDenied()) {
					...
					responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.CONFIGURATION_NEEDED, e.getRequestorText()) + ": " + e); //$NON-NLS-1$
				}
				responseWriter.flush();
				return;
			}
				...
				responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.PRESIGN_ERROR) + ": " + e); //$NON-NLS-1$
				responseWriter.flush();
				return;
			}
			...
			if (ConfigManager.getHMacKey() != null) {
				try {
					...
					responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.GENERATING_CSV_ERROR) + ": " + e); //$NON-NLS-1$
					responseWriter.flush();
					return;
				}
			}
			...
			try {
				...
				responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.INVALID_DATA_OPERATION_FORMAT) + ": " + e); //$NON-NLS-1$
				responseWriter.flush();
				return;
			}
			...
			if (ConfigManager.getHMacKey() != null) {
				try {
					...
					responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.CHECKING_CSV_ERROR) + ": " + e); //$NON-NLS-1$
					responseWriter.flush();
					return;
				}
				catch (final Exception e) {
			...
			try {
				...
				responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.POSTSIGN_ERROR) + ": " + e); //$NON-NLS-1$
				responseWriter.flush();
				return;
			}
			...
			try {
				...
				responseWriter.print(ErrorManager.getErrorMessage(ErrorManager.SAVING_DOCUMENT_ERROR) + ": " + e); //$NON-NLS-1$
				responseWriter.flush();
				return;
			}
			LOGGER.info("Documento almacenado"); //$NON-NLS-1$

			responseWriter.println(SUCCESS + newDocId);
			responseWriter.flush();

			LOGGER.info("== FIN POSTFIRMA"); //$NON-NLS-1$
		}
		else {
			responseWriter.println(ErrorManager.getErrorMessage(ErrorManager.UNSUPPORTED_OPERATION));
		}
	}
