...
final class SingleSignPreProcessor {
	...
	private static TriphaseData getPreSign(final SingleSign sSign,
		...
		try {
			...
			throw new AOException("No se pudo decodificar el certificado de firma del lote XML", e, ErrorCode.Internal.ENCODING_SIGNING_CERTIFICATE); //$NON-NLS-1$
		} catch (final Exception e) {
			throw new AOException("No se pudo agregar le codigo de verificacion de firmas", e, BatchServiceErrorCode.Internal.INTERNAL_XML_BATCH_ERROR); //$NON-NLS-1$
		}
