...
public class Crossing {
	...
	/**
	 * Excludes double roots. Roots are double if they lies enough close with
	 * each other.
	 * 
	 * @param res
	 *            - the roots
	...
	/**
	 * QuadCurve class provides basic functionality to find curve crossing and
	 * calculating bounds
	 */
	...
	/**
	 * CubicCurve class provides basic functionality to find curve crossing and
	 * calculating bounds
	 */
	...
	public static int crossPath(PathIterator p, double x, double y) {
		...
		while (!p.isDone()) {
			...
			case PathIterator.SEG_CUBICTO:
				cross += crossCubic(cx, cy, coords[0], coords[1], coords[2], coords[3], cx = coords[4], cy = coords[5],
						x, y);
				break;
			...
			// checks if the point (x,y) is the vertex of shape with
			// PathIterator p
			if (x == cx && y == cy) {
				cross = 0;
				cy = my;
	...
	public static int intersectPath(PathIterator p, double x, double y, double w, double h) {
		...
		while (!p.isDone()) {
			...
			case PathIterator.SEG_QUADTO:
				...
				break;
			case PathIterator.SEG_CUBICTO:
				count = intersectCubic(cx, cy, coords[0], coords[1], coords[2], coords[3], cx = coords[4],
						cy = coords[5], rx1, ry1, rx2, ry2);
				break;
			case PathIterator.SEG_CLOSE:
				if (cy != my || cx != mx) {
					count = intersectLine(cx, cy, mx, my, rx1, ry1, rx2, ry2);
