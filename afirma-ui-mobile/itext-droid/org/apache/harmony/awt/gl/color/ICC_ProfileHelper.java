...
/**
 * Includes utility methods for reading ICC profile data. Created to provide
...
public class ICC_ProfileHelper {
	...
	public static int getIntFromByteArray(byte[] byteArray, int idx) {
		return (byteArray[idx] & 0xFF) | ((byteArray[idx + 1] & 0xFF) << 8) | ((byteArray[idx + 2] & 0xFF) << 16)
	...
	public static short getShortFromByteArray(byte[] byteArray, int idx) {
		return (short) ((byteArray[idx] & 0xFF) | ((byteArray[idx + 1] & 0xFF) << 8));
	}
	...
	public static int getRenderingIntent(ICC_Profile profile) {
		return getIntFromByteArray(profile.getData(ICC_Profile.icSigHead), // pf
																			// header
