...
public class LUTColorConverter {
	 ...
	 * values from Linear RGB Color Space to sRGB and vice versa. Conversion is
	 * producing according to sRGB Color Space definition.
	 * "A Standard Default Color Space for the Internet - sRGB", Michael Stokes
	 * (Hewlett-Packard), Matthew Anderson (Microsoft), Srinivasan Chandrasekar
	 * (Microsoft), Ricardo Motta (Hewlett-Packard) Version 1.10, November 5,
	 * 1996 This document is available: http://www.w3.org/Graphics/Color/sRGB
	 */
	public static byte[] getFrom8lRGBtosRGB_LUT() {
		if (from8lRGBtosRGB_LUT == null) {
			...
			for (int i = 0; i < 256; i++) {
				v = (float) i / 255;
				v = (v <= 0.04045f) ? v / 12.92f : (float) Math.pow((v + 0.055) / 1.055, 2.4);
	...
	public static byte[] getFromsRGBto8lRGB_LUT() {
		if (fromsRGBto8lRGB_LUT == null) {
			...
			for (int i = 0; i < 256; i++) {
				v = (float) i / 255;
				v = (v <= 0.0031308f) ? v * 12.92f : ((float) Math.pow(v, 1.0 / 2.4)) * 1.055f - 0.055f;
