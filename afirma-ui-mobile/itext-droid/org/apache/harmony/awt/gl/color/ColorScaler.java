...
/**
 * This class provides functionality for scaling color data when ranges of the
...
public class ColorScaler {
	...
	/**
	 * Loads scaling data for raster. Note, if profile pf is null, for
	 * non-integral data types multipliers are not initialized.
	 * 
	 * @param r
	 *            - raster
	 * @param pf
	 *            - profile which helps to determine the ranges of the color
	 *            data
	 */
	...
	/**
	 * Use this method only for integral transfer types. Extracts min/max values
	...
	/**
	 * Use this method only for double of float transfer types. Extracts scaling
	...
	public void loadScalingData(ColorSpace cs) {
		...
		for (int i = 0; i < nColorChannels; i++) {
			...
			channelMulipliers[i] = MAX_SHORT / (cs.getMaxValue(i) - channelMinValues[i]);

			invChannelMulipliers[i] = (cs.getMaxValue(i) - channelMinValues[i]) / MAX_SHORT;
		}
	}

	/**
	 * Scales and normalizes the whole raster and returns the result in the
	 * float array
	 * 
	 * @param r
	 *            - source raster
	 ...
	 * @param chanData
	 *            - output buffer
	 * @param chanDataOffset
	 *            - output buffer offset
	 */
	public void scale(float[] pixelData, short[] chanData, int chanDataOffset) {
		for (int chan = 0; chan < nColorChannels; chan++) {
			chanData[chanDataOffset + chan] = (short) ((pixelData[chan] - channelMinValues[chan])
					* channelMulipliers[chan] + 0.5f);
		}
	}
	...
	public void unscale(float[] pixelData, short[] chanData, int chanDataOffset) {
		for (int chan = 0; chan < nColorChannels; chan++) {
			pixelData[chan] = (chanData[chanDataOffset + chan] & 0xFFFF) * invChannelMulipliers[chan]
					+ channelMinValues[chan];
		}
	}
}