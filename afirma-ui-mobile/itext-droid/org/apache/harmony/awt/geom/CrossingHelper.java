...
public class CrossingHelper {
	...
	private void intersectShape(List<Edge> edges, double[] coords1, int length1, double[] coords2, int length2,
			Edge initEdge) {
		...
		for (Iterator<Edge> iter = edges.iterator(); iter.hasNext();) {
			...
			if ((areaOfEdge1 != areaOfEdge2)
				...
				isectPoints.add(new IntersectPoint(initBegin, initEnd, addBegin, addEnd, point[0], point[1]));
			}
		}
	}
