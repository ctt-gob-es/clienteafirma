...
public class GeometryUtil {
	...
	public static int intersectLinesWithParams(double x1, double y1, double x2, double y2, double x3, double y3,
			double x4, double y4, double[] params) {
		double dx = x4 - x3;
		double dy = y4 - y3;
	...
	public static int intersectQuads(double x1, double y1, double x2, double y2, double x3, double y3, double qx1,
			double qy1, double qx2, double qy2, double qx3, double qy3, double[] params) {
	...
	/**
	 * It checks up if the quad (x1, y1) - (x2, y2) - (x3, y3) and the cubic
	 ...
	 *         in the interval
	 */
	public static int intersectCubics(double x1, double y1, double x2, double y2, double x3, double y3, double x4,
			double y4, double cx1, double cy1, double cx2, double cy2, double cx3, double cy3, double cx4, double cy4,
			double[] params) {
	...
	private static void cubicNewton(double xCoefs1[], double yCoefs1[], double xCoefs2[], double yCoefs2[],
			double[] params) {
		double t = 0.0, s = 0.0;
		double t1 = params[0];
		double s1 = params[1];
		...
		while (Math.sqrt((t - t1) * (t - t1) + (s - s1) * (s - s1)) > EPSILON) {
			...
			dt = (t * t * t * xCoefs1[0] + t * t * xCoefs1[1] + t * xCoefs1[2] + xCoefs1[3] - s * s * s * xCoefs2[0]
					- s * s * xCoefs2[1] - s * xCoefs2[2] - xCoefs2[3])
					* (-3 * s * s * yCoefs2[0] - 2 * s * yCoefs2[1] - yCoefs2[2])
					+ (t * t * t * yCoefs1[0] + t * t * yCoefs1[1] + t * yCoefs1[2] + yCoefs1[3] - s * s * s
							* yCoefs2[0] - s * s * yCoefs2[1] - s * yCoefs2[2] - yCoefs2[3])
					* (3 * s * s * xCoefs2[0] + 2 * s * xCoefs2[1] + xCoefs2[2]);
			...
			ds = (3 * t * t * xCoefs1[0] + 2 * t * xCoefs1[1] + xCoefs1[2])
					* (t * t * t * yCoefs1[0] + t * t * yCoefs1[1] + t * yCoefs1[2] + yCoefs1[3] - s * s * s
							* yCoefs2[0] - s * s * yCoefs2[1] - s * yCoefs2[2] - yCoefs2[3])
					- (3 * t * t * yCoefs1[0] + 2 * t * yCoefs1[1] + yCoefs1[2])
					* (t * t * t * xCoefs1[0] + t * t * xCoefs1[1] + t * xCoefs1[2] + xCoefs1[3] - s * s * s
							* xCoefs2[0] - s * s * xCoefs2[1] - s * xCoefs2[2] - xCoefs2[3]);
	...
	private static void quadAndCubicNewton(double xCoefs1[], double yCoefs1[], double xCoefs2[], double yCoefs2[],
		...
		while (Math.sqrt((t - t1) * (t - t1) + (s - s1) * (s - s1)) > EPSILON) {
			d = -(2 * t * xCoefs1[0] + xCoefs1[1]) * (3 * s * s * yCoefs2[0] + 2 * s * yCoefs2[1] + yCoefs2[2])
					+ (2 * t * yCoefs1[0] + yCoefs1[1]) * (3 * s * s * xCoefs2[0] + 2 * s * xCoefs2[1] + xCoefs2[2]);

			dt = (t * t * xCoefs1[0] + t * xCoefs1[1] + xCoefs1[2] + -s * s * s * xCoefs2[0] - s * s * xCoefs2[1] - s
					* xCoefs2[2] - xCoefs2[3])
					* (-3 * s * s * yCoefs2[0] - 2 * s * yCoefs2[1] - yCoefs2[2])
					+ (t * t * yCoefs1[0] + t * yCoefs1[1] + yCoefs1[2] - s * s * s * yCoefs2[0] - s * s * yCoefs2[1]
							- s * yCoefs2[2] - yCoefs2[3]) * (3 * s * s * xCoefs2[0] + 2 * s * xCoefs2[1] + xCoefs2[2]);

			ds = (2 * t * xCoefs1[0] + xCoefs1[1])
					* (t * t * yCoefs1[0] + t * yCoefs1[1] + yCoefs1[2] - s * s * s * yCoefs2[0] - s * s * yCoefs2[1]
							- s * yCoefs2[2] - yCoefs2[3])
					- (2 * t * yCoefs1[0] + yCoefs1[1])
					* (t * t * xCoefs1[0] + t * xCoefs1[1] + xCoefs1[2] - s * s * s * xCoefs2[0] - s * s * xCoefs2[1]
							- s * xCoefs2[2] - xCoefs2[3]);
	...
	private static void quadNewton(double xCoefs1[], double yCoefs1[], double xCoefs2[], double yCoefs2[],
			double params[]) {
		double t = 0.0, s = 0.0;
		double t1 = params[0];
		double s1 = params[1];
		...
		while (Math.sqrt((t - t1) * (t - t1) + (s - s1) * (s - s1)) > EPSILON) {
			...
			dt = -(t * t * xCoefs1[0] + t * xCoefs1[1] + xCoefs1[1] - s * s * xCoefs2[0] - s * xCoefs2[1] - xCoefs2[2])
					* (2 * s * yCoefs2[0] + yCoefs2[1])
					+ (2 * s * xCoefs2[0] + xCoefs2[1])
					* (t * t * yCoefs1[0] + t * yCoefs1[1] + yCoefs1[2] - s * s * yCoefs2[0] - s * yCoefs2[1] - yCoefs2[2]);

			ds = (2 * t * xCoefs1[0] + xCoefs1[1])
					* (t * t * yCoefs1[0] + t * yCoefs1[1] + yCoefs1[2] - s * s * yCoefs2[0] - s * yCoefs2[1] - yCoefs2[2])
					- (2 * t * yCoefs1[0] + yCoefs1[1])
					* (t * t * xCoefs1[0] + t * xCoefs1[1] + xCoefs1[2] - s * s * xCoefs2[0] - s * xCoefs2[1] - xCoefs2[2]);
