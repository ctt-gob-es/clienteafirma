		...
		try {
			...
			if (args.length >= 2) {
				if (args[1].compareToIgnoreCase("stdout") != 0) {
					...
					writer = new PrintWriter(new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + File.separator + "droidtext" + File.separator + new File(args[1])));
				}
			}
