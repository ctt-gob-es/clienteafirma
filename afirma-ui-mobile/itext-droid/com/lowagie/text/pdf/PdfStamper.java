...
/**
 * Applies extra content to the pages of a PDF document. This extra content can
 * be all the objects allowed in PdfContentByte including pages from other Pdfs.
 * The original PDF will keep all the interactive elements including bookmarks,
 * links and form fields.
 * <p>
 * It is also possible to change the field values and to flatten them. New
 * fields can be added but not flattened.
 * 
...
public class PdfStamper implements PdfViewerPreferences, PdfEncryptionSettings {
	 ...
	 *            the output stream
	 * @param pdfVersion
	 *            the new pdf version or '\0' to keep the same version as the
	 *            original document
	 * @throws DocumentException
	 *             on error
	 * @throws IOException
	 ...
	 *            original document
	 * @param append
	 *            if <CODE>true</CODE> appends the document changes as a new
	 *            revision. This is only useful for multiple signatures as
	 *            nothing is gained in speed or memory
	 * @throws DocumentException
	 *             on error
	 * @throws IOException
	 *             on error
	...
	/**
	 * Gets the optional <CODE>String</CODE> map to add or change values in the
	 * info dictionary.
	 * 
	...
	/**
	 * An optional <CODE>String</CODE> map to add or change values in the info
	 * dictionary. Entries with <CODE>null</CODE> values delete the key in the
	 * original info dictionary
	 * 
	 * @param moreInfo
	...
	/**
	 * Replaces a page from this document with a page from other document. Only
	 * the content is replaced not the fields and annotations. This method must
	 * be called before getOverContent() or getUndercontent() are called for the
	 * same page.
	 * 
	 * @param r
	 *            the <CODE>PdfReader</CODE> from where the new page will be
	 *            imported
	 * @param pageImported
	...
	/**
	 * Inserts a blank page. All the pages above and including
	 * <CODE>pageNumber</CODE> will be shifted up. If <CODE>pageNumber</CODE> is
	 * bigger than the total number of pages the new page will be the last one.
	 * 
	 * @param pageNumber
	...
	/**
	 * Gets the signing instance. The appearances and other parameters can the
	 * be set.
	 * 
	 ...
	 * closed.
	 * <p>
	 * If closing a signed document with an external signature the closing must
	 * be done in the <CODE>PdfSignatureAppearance</CODE> instance.
	 * 
	 * @throws DocumentException
	 *             on error
	 * @throws IOException
	...
	/**
	 * Gets a <CODE>PdfContentByte</CODE> to write under the page of the
	 * original document.
	 * 
	 * @param pageNum
	 *            the page number where the extra content is written
	 * @return a <CODE>PdfContentByte</CODE> to write under the page of the
	 *         original document
	 */
	...
	/**
	 * Gets a <CODE>PdfContentByte</CODE> to write over the page of the original
	 * document.
	 * 
	 * @param pageNum
	 *            the page number where the extra content is written
	 * @return a <CODE>PdfContentByte</CODE> to write over the page of the
	 *         original document
	 */
	...
	/**
	 * Checks if the content is automatically adjusted to compensate the
	...
	/**
	 * Flags the content to be automatically adjusted to compensate the original
	 * page rotation. The default is <CODE>true</CODE>.
	 * 
	 * @param rotateContents
	 ...
	 * ownerPassword can be null or have zero length. In this case the
	 * ownerPassword is replaced by a random string. The open permissions for the
	 * document can be AllowPrinting, AllowModifyContents, AllowCopy,
	 * AllowModifyAnnotations, AllowFillIn, AllowScreenReaders, AllowAssembly
	 * and AllowDegradedPrinting. The permissions can be combined by ORing them.
	 * 
	 * @param userPassword
	 ...
	 *            <code>true</code> for 128 bit key length, <code>false</code>
	...
	/**
	 * Sets the certificate encryption options for this document. An array of
	 * one or more public certificates must be provided together with an array
	 * of the same size for the permissions for each certificate. The open
	 * permissions for the document can be AllowPrinting, AllowModifyContents,
	 * AllowCopy, AllowModifyAnnotations, AllowFillIn, AllowScreenReaders,
	 * AllowAssembly and AllowDegradedPrinting. The permissions can be combined
	 * by ORing them. Optionally DO_NOT_ENCRYPT_METADATA can be ored to output
	 * the metadata in cleartext
	 * 
	 * @param certs
	...
	/**
	 * Gets a page from other PDF document. Note that calling this method more
	 * than once with the same parameters will retrieve the same object.
	 * 
	 * @param reader
	...
	/**
	 * Gets the <CODE>AcroFields</CODE> object that allows to get and set field
	 * values and to merge FDF forms.
	 * 
	 ...
	 * 
	 * @param flat
	 *            <CODE>true</CODE> to flatten the fields, <CODE>false</CODE> to
	 *            keep the fields
	 */
	 ...
	 * 
	 * @param flat
	 *            <CODE>true</CODE> to flatten the FreeText annotations,
	 *            <CODE>false</CODE> (the default) to keep the FreeText
	 *            annotations as active content.
	 */
	...
	/**
	 * Adds an annotation of form field in a specific page. This page number can
	 * be overridden with {@link PdfAnnotation#setPlaceInPage(int)}.
	 * 
	 * @param annot
	 *            the annotation
	 * @param page
	 ...
	 * 
	 * @param outlines
	 *            the bookmarks or <CODE>null</CODE> to remove any
	 */
	...
	/**
	 * Adds <CODE>name</CODE> to the list of fields that will be flattened on
	 * close, all the other fields will remain. If this method is never called
	 * or is called with invalid field names, all the fields will be flattened.
	 * <p>
	 * Calling <CODE>setFormFlattening(true)</CODE> is needed to have any kind
	 * of flattening.
	 * 
	 * @param name
	 *            the field name
	...
	/**
	 * Adds a JavaScript action at the document level. When the document opens
	 * all this JavaScript runs. The existing JavaScript will be replaced.
	 * 
	 * @param js
	 *            the JavaScript code
	 */
	...
	/**
	 * Adds a file attachment at the document level. Existing attachments will
	 * be kept.
	 * 
	 * @param description
	 *            the file description
	 * @param fileStore
	 *            an array with the file. If it's <CODE>null</CODE> the file
	 ...
	 * <li>PdfName.H (hidden)
	 * </ul>
	 * Pass this name as a parameter and your PDF will be a portable collection
	 * with all the embedded and attached files as entries.
	 * 
	 * @param initialView
	 ...
	 * @param value
	 *            the value for the viewer preference
	...
	/**
	 * Sets the document's compression to the new 1.5 mode with object streams
	 * and xref streams. It can be set at any time but once set it can't be
	 * unset.
	 */
	 ...
	 * 
	 * @param seconds
	 *            the number of seconds to display the page. A negative value
	 *            removes the entry
	 * @param page
	 *            the page where the duration will be applied. The first page is
	 *            1
	 */
	 ...
	 * 
	 * @param transition
	 *            the transition object. A <code>null</code> removes the
	 *            transition
	 * @param page
	 *            the page where the transition will be applied. The first page
	 *            is 1
	 */
	...
	/**
	 * Applies a digital signature to a document, possibly as a new revision,
	 * making possible multiple signatures. The returned PdfStamper can be used
	 * normally as the signature is only applied when closing.
	 * <p>
	 * A possible use for adding a signature without invalidating an existing
	 * one is:
	 * <p>
	 * 
	 * <pre>
	 ...
	 * FileOutputStream fout = new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + File.separator + "droidtext" + File.separator + &quot;signed.pdf&quot;);
	 ...
	 * @param os
	 *            the output stream or <CODE>null</CODE> to keep the document in
	 *            the temporary file
	 * @param pdfVersion
	 *            the new pdf version or '\0' to keep the same version as the
	 *            original document
	 * @param tempFile
	 *            location of the temporary file. If it's a directory a
	 *            temporary file will be created there. If it's a file it will
	 *            be used directly. The file will be deleted on exit unless
	 *            <CODE>os</CODE> is null. In that case the document can be
	 *            retrieved directly from the temporary file. If it's
	 *            <CODE>null</CODE> no temporary file will be created and memory
	 *            will be used
	 * @param append
	 *            if <CODE>true</CODE> the signature and all the other content
	 *            will be added as a new revision thus not invalidating existing
	 *            signatures
	...
	/**
	 * Applies a digital signature to a document. The returned PdfStamper can be
	 * used normally as the signature is only applied when closing.
	 * <p>
	...
	/**
	 * Gets the PdfLayer objects in an existing document as a Map with the
	 * names/titles of the layers as keys.
	 * 
	 * @return a Map with all the PdfLayers in the document (and the name/title
	 *         of the layer as key)
	 * @since 2.1.2
	 */
