...
public class PdfAnnotation extends PdfDictionary {
	...
	public static PdfAnnotation createFileAttachment(PdfWriter writer, Rectangle rect, String contents,
		...
		return createFileAttachment(writer, rect, contents, PdfFileSpecification.fileEmbedded(writer, file,
				fileDisplay, fileStore));
	}
	...
	/**
	 * Places the annotation in a specified page that must be greater or equal
	 * to the current one. With <code>PdfStamper</code> the page can be any. The
	 * first page is 1.
	 * 
	 * @param placeInPage
	...
	/**
	 * Sets the name of the annotation. With this name the annotation can be
	 * identified among all the annotations on a page (it has to be unique).
	 */
	...
	/**
	 * This class processes links from imported pages so that they may be
	 * active. The following example code reads a group of files and places them
	 * all on the output PDF, four pages in a single page, keeping the links
	 * active.
	 * 
	 * <pre>
	 ...
	 * PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(Environment.getExternalStorageDirectory() + File.separator + "droidtext" + File.separator + outputFile));
