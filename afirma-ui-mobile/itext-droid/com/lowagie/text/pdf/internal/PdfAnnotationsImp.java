...
public class PdfAnnotationsImp {
    ...
    private void addFormFieldRaw(final PdfFormField field) {
    ...
    public PdfArray rotateAnnotations(final PdfWriter writer, final Rectangle pageSize) {
        ...
        for (int k = 0; k < this.annotations.size(); ++k) {
            final PdfAnnotation dic = (PdfAnnotation)this.annotations.get(k);
    ...
    public static PdfAnnotation convertAnnotation(final PdfWriter writer, final Annotation annot, final Rectangle defaultRect) throws IOException {
        switch(annot.annotationType()) {
           ...
           case Annotation.SCREEN:
               final boolean sparams[] = (boolean[])annot.attributes().get(Annotation.PARAMETERS);
               final String fname = (String) annot.attributes().get(Annotation.FILE);
               final String mimetype = (String) annot.attributes().get(Annotation.MIMETYPE);
               ...
               final PdfAnnotation ann = PdfAnnotation.createScreen(writer, new Rectangle(annot.llx(), annot.lly(), annot.urx(), annot.ury()),
