...
class PdfEncodings {
    ...
    static {
        for (int k = 128; k < 161; ++k) {
            final char c = winansiByteToChar[k];
            if (c != 65533) {
				winansi.put(c, k);
			}
        }
        ...
        for (int k = 128; k < 161; ++k) {
            final char c = pdfEncodingByteToChar[k];
            if (c != 65533) {
				pdfEncoding.put(c, k);
			}
        }
    ...
    public static final byte[] convertToBytes(final String text, final String encoding) {
        ...
        final ExtraEncoding extra = (ExtraEncoding)extraEncodings.get(encoding.toLowerCase());
        if (extra != null) {
        ...
        if (encoding.equals(PdfObject.TEXT_UNICODE)) {
            ...
            final char cc[] = text.toCharArray();
            final int len = cc.length;
            final byte b[] = new byte[cc.length * 2 + 2];
            b[0] = -2;
            b[1] = -1;
            int bptr = 2;
            for (int k = 0; k < len; ++k) {
     ...
     */
    private static String convertCmap(final String name, final byte seq[], final int start, final int length) {
        try {
            char planes[][] = null;
            synchronized (cmaps) {
    ...
    private static char[][] readCmap(final String name, final byte newline[][]) throws IOException {
        ...
        if (newline != null) {
            for (final byte[] element : newline) {
				encodeSequence(element.length, element, BaseFont.CID_NEWLINE, planes);
			}
        }
        ...
        while ((line = rd.readLine()) != null) {
            ...
            switch (state) {
                ...
                case CIDCHAR: {
                    ...
                    final long start = Long.parseLong(t.substring(1, t.length() - 1), 16);
                    t = tk.nextToken();
                    final int cid = Integer.parseInt(t);
    ...
    private static void encodeSequence(int size, final byte seqs[], final char cid, final ArrayList planes) {
        ...
        for (int idx = 0; idx < size; ++idx) {
            final char plane[] = (char[])planes.get(nextPlane);
        ...
        final char plane[] = (char[])planes.get(nextPlane);
        final int one = seqs[size] & 0xff;
        final char c = plane[one];
        if ((c & 0x8000) != 0) {
			throw new RuntimeException("Inconsistent mapping.");
		}
        plane[one] = cid;
    ...
    private static class WingdingsConversion implements ExtraEncoding {
		...
		public byte[] charToByte(final char char1, final String encoding) {
            if (char1 == ' ') {
                ...
                final byte v = table[char1 - 0x2700];
                if (v != 0) {
					return new byte[]{v};
				}
            }
    ...
    private static class Cp437Conversion implements ExtraEncoding {
		...
		public byte[] charToByte(final char char1, final String encoding) {
            if (char1 < 128) {
                ...
                final byte v = (byte)c2b.get(char1);
                if (v != 0) {
					return new byte[]{v};
				} else {
					return new byte[0];
    ...
    private static class SymbolConversion implements ExtraEncoding {
        ...
        static {
            for (int k = 0; k < table1.length; ++k) {
                final int v = table1[k];
                if (v != 0) {
					t1.put(v, k + 32);
				}
            }
            for (int k = 0; k < table2.length; ++k) {
                final int v = table2[k];
                if (v != 0) {
					t2.put(v, k + 32);
				}
            }
