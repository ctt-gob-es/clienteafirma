...
final class UtilActions {
	...
	static void doChangeKeyStore(final int keyStoreType, final CertificateSelectionDialog selectionDialog, final Component parent, final String ksLibPath) {
		if (parent != null) {
			parent.setCursor(new Cursor(Cursor.WAIT_CURSOR));
		}

		selectionDialog.changeKeyStore(keyStoreType, ksLibPath);
