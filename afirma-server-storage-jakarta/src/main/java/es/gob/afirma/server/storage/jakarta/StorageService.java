...
package es.gob.afirma.server.storage.jakarta;
...
import es.gob.afirma.server.storage.StorageServiceHandler;
...
public final class StorageService extends HttpServlet {
	...
	private static Map<String, String> extractParameters(final HttpServletRequest request) throws IOException {
		...
		try (InputStream is = request.getInputStream()) {
			...
			LOGGER.log(Level.SEVERE, "La codificacion por defecto no es compatible con la JRE: " + URL_DEFAULT_CHARSET, e); //$NON-NLS-1$
		}
