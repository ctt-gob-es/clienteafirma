 ...
 * entries.
 * <p>
 * The format of the file doesn't seem to be officially documented however it's not complicated and can be easily derived from the Mozilla sources
...
final class NSRegistry {
    ...
    NSRegistry open(final File regFile) {
        ...
        try {
            ...
            if (in.skipBytes(4) != 4) {
                LOGGER
                      .warning("Se solicito ignorar 4 bytes en el registro de Netscape pero se ignoro un numero distinto, " + "el resto analisis podria ser incoherente");
            }
            rootOffset = readInt();
            return this;
        }
        catch (final IOException e) {
            LOGGER.severe("Error abriendo el registro de Mozilla, se devolvera null: " + e);
            return null;
        }
    }
    ...
    void close() {
        if (in != null) {
            try {
                ...
                LOGGER.warning("Error cerrando el flujo de lectura del registro de Mozilla ");
            }
            in = null;
        }
    }
