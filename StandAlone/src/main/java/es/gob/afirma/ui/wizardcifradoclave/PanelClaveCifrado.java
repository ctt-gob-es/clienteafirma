...
public class PanelClaveCifrado extends JDialogWizard {
    ...
    private String getKeyFromCipherKeyStore() throws AOException {
    	...
    	try {
    		cKs = new AOCipherKeyStoreHelper(
    				AOUIManager.getPassword(Messages.getString("WizardCifrado.almacen.claves.contrasenia"), this)
    		);
    	} catch (AOCancelledOperationException e) {
    		throw e;
    	} catch (Exception e) {
    ...
    private void autogenerarActionPerformed() {
        ...
        campoClave.setText(AOCryptoUtil.encodeBase64(cipherKey.getEncoded(), false));
        checkGuardar.setEnabled(true);
    }
	...
	private Boolean cifrarFichero() {
		...
		if (campoClave.getText() == null || campoClave.getText().equals("")){
			...
			try {
				...
				logger.severe("No se cumplen con los requisitos de clave del algoritmo: " + ex);
	...
	private void guardarClaveCifrado() {
		try {
			...
			do {
				...
				try {
					if (!AOCipherKeyStoreHelper.storeExists()) {
						PasswordDialog passDialog = new PasswordDialog(this, Messages.getString("Cifrado.almacen.claves"));
						passDialog.setDescriptionText(Messages.getString("Cifrado.introducir.pass"));
						if (!passDialog.showDialog()) 
							throw new AOException("No se ha introducido contrase\u00F1a para el almac\u00E9n de claves. No se almacenar\u00E1 la clave de cifrado.");

						cksh = new AOCipherKeyStoreHelper(passDialog.getPass());
					} else {
